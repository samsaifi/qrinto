...
{
    /**
     * Show the custom print page.
     */
    public function index()
    {
        $sizes = CustomPrintSize::active()->get()
            ->map(fn($s) => $s->toFrontendArray())
            ->values();

        return view('products.custom-print', compact('sizes'));
    }
    ...
    public function upload(Request $request)
    {
        ...
        $path = $file->store('custom-prints/' . date('Y/m'), 'public');
    ...
    public function addToCart(Request $request, CartService $cartService)
    {
        ...
        $sizeRecord = CustomPrintSize::where('key', $sizeKey)->where('is_active', true)->firstOrFail();
        ...
        // Use null product_id for custom print items (column is nullable).
