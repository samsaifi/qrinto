...
@section('content')
    <div x-data="cartPage()" @cart-updated.window="updateFromResponse($event.detail)" class="space-y-5 pb-48">
        <div class="space-y-1">
                ...
                @foreach ($cart->items as $item)
                    ...
                    <div class="bg-white border-2 border-slate-50 rounded-[2rem] shadow-premium overflow-hidden cart-item-enter"
                        ...
                        <div class="p-4 flex gap-4">
                            {{-- Thumbnail --}}
                            <div
                                class="w-20 h-20 rounded-xl overflow-hidden bg-slate-100 flex-shrink-0 border border-slate-100">
                                @if ($thumbUrl)
                                    <img src="{{ $thumbUrl }}" alt="{{ $item->product->name ?? 'Design' }}"
                                ...
                                <div class="flex items-start justify-between gap-2">
                                    <div class="min-w-0">
                                        @if (!empty($customization['type_name']))
                                            <span
                                                class="text-[9px] font-black text-brand-600 uppercase tracking-[0.15em]">{{ $customization['type_name'] }}</span>
                                        @endif
        ...
        function cartPage() {
            return {
                ...
                async applyCoupon() {
                    ...
                    try {
                        const res = await fetch(
                            '{{ route((str_contains(Route::currentRouteName(), 'flow-pc') ? 'flow-pc' : 'flow') . '.cart.apply-coupon') }}', {
                                method: 'POST',
                                headers: {
                ...
                async removeCoupon() {
                    try {
                        await fetch(
                            '{{ route((str_contains(Route::currentRouteName(), 'flow-pc') ? 'flow-pc' : 'flow') . '.cart.remove-coupon') }}', {
                                method: 'POST',
                                headers: {
                                    'Content-Type': 'application/json',
                                    'Accept': 'application/json',
        ...
        function cartItem(id, initialQty, unitPrice) {
            return {
                ...
                async changeQty(newQty) {
                    ...
                    try {
                        const res = await fetch(
                            `{{ url(str_contains(Route::currentRouteName(), 'flow-pc') ? 'pc/cart/update' : 'cart/update') }}/${this.id}`, {
                                method: 'POST',
                                headers: {
                                    'Content-Type': 'application/json',
                                    'Accept': 'application/json',
                ...
                async removeItem() {
                    ...
                    try {
                        const res = await fetch(
                            `{{ url(str_contains(Route::currentRouteName(), 'flow-pc') ? 'pc/cart/remove' : 'cart/remove') }}/${this.id}`, {
                                method: 'DELETE',
                                headers: {
                                    'Content-Type': 'application/json',
                                    'Accept': 'application/json',
