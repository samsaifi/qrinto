...
@section('content')
    <div class="w-full bg-[#fafcf9] min-h-screen py-4 px-4 md:py-10 md:px-6 lg:px-16 font-sans">
        <div class="max-w-[1240px] mx-auto">
            ...
            {{-- Back + Title --}}
            <div class="flex items-center gap-2 mb-3 md:flex-col md:items-start md:gap-0 md:mb-10">
                ...
                <div class="min-w-0">
                    ...
                    @if ($type->title || $type->short_description)
                        <p class="text-[10px] md:text-sm text-slate-400 font-normal truncate md:mt-1.5">
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-6">
                ...
                @forelse ($groupedSubTypes as $groupTitle => $items)
                    ...
                    <div x-data="{
                        ...
                        {{-- Top Image Area --}}
                        <div
                            class="bg-[#f2f7f2] h-28 md:h-48 flex items-center justify-center p-4 md:p-6 border-b border-slate-100 relative">
                            ...
                            @elseif ($isFolded)
                                <svg class="w-14 h-14 md:w-28 md:h-28 stroke-emerald-700 stroke-[1.25] fill-none group-hover:scale-105 transition-transform"
                                    viewBox="0 0 90 90">
                            ...
                            @else
                                <svg class="w-24 h-24 stroke-emerald-700 stroke-[1.25] fill-none group-hover:scale-105 transition-transform"
                                    viewBox="0 0 80 90">
                                    <rect x="20" y="12" width="40" height="60" rx="4" stroke="currentColor"
                                        fill="none" />
                                    <rect x="27" y="20" width="26" height="30" rx="2" stroke="currentColor"
                                        fill="none" />
                        ...
                        <div class="px-3 py-3 md:p-6 flex-1 flex flex-col justify-between">
                            <div>
                                {{-- Card Title from Product Title --}}
                                <h3
                                    class="font-extrabold text-slate-900 text-base md:text-xl group-hover:text-emerald-700 transition-colors">
                                    {{ $displayTitle }}
                                </h3>
                                @if ($desc)
                                ...
                                <div class="mt-2.5 md:mt-4">
                                        ...
                                        <div class="flex items-center gap-1.5 flex-wrap">
                                            @foreach ($items as $item)
                                                @php
                                                    ...
                                                    $itemPrice = \App\Services\CurrencyService::formatWithCurrency(
                                                        $item->price,
                                                        $item->currency ?? 'USD',
                                                    );
                                                @endphp
                                    ...
                                    @else
                                        @php
                                            $singleDim =
                                                $firstItem->width + 0 . ' × ' . ($firstItem->height + 0) . ' in';
                                            $normTitle = str_replace(
                                                [' ', 'in', '×'],
                                                ['', '', 'x'],
                            ...
                            <div
                                class="mt-3 md:mt-6 pt-2 md:pt-4 border-t border-slate-100 flex flex-col md:flex-row gap-1.5 md:gap-2 items-start md:items-center justify-between">
                                <span class="text-[11px] md:text-xs text-slate-500 font-normal">
                                    <strong class="font-extrabold text-slate-900 text-sm" x-text="selectedPrice"></strong>
                                    each
                                </span>
                                ...
                                @if (Str::contains(strtolower($type->slug ?? ''), 'magnet') || Str::contains(strtolower($type->name ?? ''), 'magnet'))
                                    <a :href="'{{ url('/') }}/magnets/' + selectedSizeCode + '/design'"
                                        class="bg-[#287d3c] hover:bg-emerald-800 text-white font-bold px-3 py-1.5 md:px-4 md:py-2 rounded-lg md:rounded-xl text-[11px] md:text-xs transition-all shadow-2xs inline-flex items-center justify-center gap-1 active:scale-95 w-full md:w-auto">
                                        <span> Start Design →</span>
                                    </a>
                                @else
                                    <a :href="'{{ url('/') }}/{{ $type->slug }}/' + selectedTitleSlug + '/' +
                                        selectedSizeCode + '/templates'"
                                        class="bg-[#287d3c] hover:bg-emerald-800 text-white font-bold px-3 py-1.5 md:px-4 md:py-2 rounded-lg md:rounded-xl text-[11px] md:text-xs transition-all shadow-2xs inline-flex items-center justify-center gap-1 active:scale-95 w-full md:w-auto">
                                        <span>Start Design →</span>
                                    </a>
                                @endif
                ...
                @empty
                    <div class="col-span-3 text-center py-12 bg-white rounded-3xl border border-slate-200">
                        <p class="text-slate-500 text-sm font-medium">No sub-categories available for {{ $type->name }}.
                        </p>
                    </div>
                @endforelse
