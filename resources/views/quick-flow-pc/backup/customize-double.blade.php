    ...
    <div id="customizer-app" class="pb-16">
                    ...
                    <div
                        ...
                        @foreach ($imageTypes as $key => $img)
                            ...
                            <button type="button" onclick="customizer.switchCanvas('{{ $key }}')"
                                ...
                                <span
                                    class="text-[10px] {{ $isFirst ? 'font-black text-brand-600' : 'font-bold text-slate-500 group-hover:text-brand-hover' }} transition-colors">
                                    Page {{ $loop->iteration }}
                                </span>
                            </button>
                        @endforeach
        ...
        <section class="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-10 mt-8 mb-6">
            <div
                ...
                <div
                    class="absolute -bottom-20 -left-20 w-80 h-80 bg-gradient-to-tr from-brand-400/15 via-brand-500/15 to-purple-400/15 rounded-full blur-3xl pointer-events-none">
                </div>
                ...
                <div class="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                    <div class="flex items-center gap-5">
                        ...
                        <div class="space-y-1">
                            <div
                                class="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-white/90 border border-brand-200/80 text-brand-600 text-[11px] font-black shadow-2xs backdrop-blur-md tracking-wider uppercase">
            ...
            updateUI() {
                ...
                Object.keys(this.imageTypes).forEach((k) => {
                    ...
                    if (btnIcon) {
                        ...
                        if (k === key) {
                            btnIcon.className =
                                `w-12 h-12 rounded-2xl bg-white text-brand-600 shadow-md border-2 border-brand-500 flex items-center justify-center scale-105 transition-all duration-200 relative ${!isEnabled ? 'opacity-70' : ''}`;
                            if (btnWrapper) {
                            ...
                            btnIcon.className =
                                `w-12 h-12 rounded-2xl bg-white text-slate-500 shadow-2xs border border-slate-200/90 flex items-center justify-center group-hover:text-brand-hover group-hover:border-brand-hover group-hover:scale-105 transition-all duration-200 relative ${!isEnabled ? 'opacity-60' : ''}`;
                            if (btnWrapper) {
                                const txt = btnWrapper.querySelector('span');
                                if (txt) txt.className =
                                    'text-[10px] font-bold text-slate-500 group-hover:text-brand-hover transition-colors';
                            }
                        }
                    }
                });
