        ...
        <div class="max-w-[1360px] mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <div
                            ...
                            <i data-lucide="clock" class="w-3.5 h-3.5"></i> Placed on
                            {{ $order->created_at->format('M d, Y &bull; h:i A') }}
                        </div>
                ...
                <div class="flex items-center gap-3 self-end sm:self-center">
                    <span
                        class="inline-flex items-center gap-2 text-sm font-extrabold text-brand-700 bg-brand-50 border border-brand-200/80 px-4 py-2 rounded-2xl shadow-2xs">
                    ...
                    @if ($order->store)
                        <div class="glass-card border border-slate-200/90 rounded-3xl p-6 lg:p-7 shadow-sm">
                            <div class="flex items-center gap-3.5 mb-5 pb-4 border-b border-slate-200/80">
                                ...
                                <div>
                                    <h3 class="text-base font-black text-slate-900 tracking-tight">Fulfillment Store
                                        Location</h3>
                            ...
                            <div
                                class="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-2xs flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                                <div>
                                ...
                                @if ($order->store->phone)
                                    <a href="tel:{{ $order->store->phone }}"
                                        class="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-slate-100 hover:bg-brand-50 text-slate-700 hover:text-brand-600 text-xs font-bold transition-all border border-slate-200/80 shrink-0">
                    ...
                    <div
                        ...
                        <div class="divide-y divide-slate-100">
                            <div class="py-3 flex justify-between items-center text-sm">
                                ...
                                <span
                                    class="font-extrabold text-slate-900 text-right max-w-[200px] truncate">{{ $order->items->first()?->product_name ?? 'Custom Print' }}</span>
                            </div>
                            <div class="py-3 flex justify-between items-center text-sm">
                                <span class="text-slate-500 font-semibold">Quantity</span>
                                <span
                                    class="font-extrabold text-slate-900">{{ $order->items->first()?->quantity ?? 1 }}</span>
                            </div>
                            @if ($order->store)
                                <div class="py-3 flex justify-between items-center text-sm">
                                    <span class="text-slate-500 font-semibold">Fulfillment Store</span>
                                    <span
                                        class="font-extrabold text-slate-900 text-right max-w-[180px] truncate">{{ $order->store->store_name }}</span>
                                </div>
                            @endif
                            <div class="py-3 flex justify-between items-center text-sm">
                                ...
                                @else
                                    <span
                                        class="inline-flex items-center gap-1.5 text-xs font-extrabold text-amber-700 bg-amber-50 px-3 py-1 rounded-full border border-amber-200/60">
                        ...
                        <div
                            ...
                            <div class="flex justify-between items-baseline mb-1 relative z-10">
                                ...
                                <span
                                    class="text-3xl font-black text-white tracking-tight">{{ \App\Services\CurrencyService::formatWithCurrency($order->total, $order->currency) }}</span>
                            </div>
