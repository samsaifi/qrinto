        ...
        if (empty($itemPages)) {
            if ($uploadedImages && is_array($uploadedImages)) {
                foreach($uploadedImages as $p) {
                    if ($p) $itemPages[] = str_starts_with($p, 'http') ? $p : asset('storage/' . $p);
                }
            }
    ...
    <div class="max-w-[1360px] mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4 bg-white/90 backdrop-blur-md px-5 py-2.5 rounded-2xl border border-slate-200/80 shadow-2xs">
            <div class="flex items-center gap-3">
                ...
                <span class="text-xs font-extrabold text-emerald-700 bg-emerald-50 px-2.5 py-0.5 rounded-full border border-emerald-200/60 flex items-center gap-1">
            ...
            <div class="flex items-center gap-3">
                <div class="hidden md:flex items-center gap-2 text-[11px] font-bold">
                    ...
                    <i data-lucide="chevron-right" class="w-3 h-3 text-slate-300"></i>
                    <span class="text-emerald-700 bg-emerald-50 px-2.5 py-0.5 rounded-full border border-emerald-200/80">3. Confirmed!</span>
                </div>
        ...
        <div class="bg-emerald-50/90 border border-emerald-200/90 text-slate-900 rounded-2xl p-4 sm:p-5 mb-4 shadow-2xs relative overflow-hidden">
            <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
                <div class="flex items-center gap-3.5">
                    <div class="w-11 h-11 bg-emerald-500 text-white rounded-xl flex items-center justify-center shrink-0 shadow-sm">
                ...
                <div class="flex flex-wrap items-center gap-2 shrink-0">
                    <span class="inline-flex items-center gap-1.5 bg-white border border-emerald-200 rounded-xl px-3 py-1.5 text-slate-900 text-xs font-black shadow-2xs">
                        <i data-lucide="hash" class="w-3.5 h-3.5 text-emerald-600"></i>
                        <span>{{ $order->order_number }}</span>
                    </span>
                    @if($order->store)
                    <span class="inline-flex items-center gap-1.5 bg-white border border-slate-200 rounded-xl px-3 py-1.5 text-slate-700 text-xs font-bold shadow-2xs">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-5 items-start">
            ...
            <div class="lg:col-span-7 space-y-4">
                ...
                <div class="glass-card border border-slate-200/90 rounded-2xl p-4 shadow-2xs" x-data="{ activePreviewId: {{ $firstItemId }} }">
                    ...
                    <div class="pb-3 border-b border-slate-200/80 mb-3 space-y-2">
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-2">
                                ...
                                <h2 class="text-sm font-black text-slate-900 tracking-tight">Order Design Previews</h2>
                            </div>
                        ...
                        <div class="flex items-center gap-2 overflow-x-auto pb-1">
                            @foreach($itemPreviews as $pIdx => $preview)
                                <button type="button" @click="activePreviewId = {{ $preview['id'] }}"
                                    :class="activePreviewId === {{ $preview['id'] }} ? 'bg-brand-500 text-white border-brand-500 shadow-2xs' : 'bg-white text-slate-700 hover:bg-slate-50 border-slate-200'"
                                    ...
                                    <span class="w-4 h-4 rounded-full flex items-center justify-center text-[10px] font-black"
                                          :class="activePreviewId === {{ $preview['id'] }} ? 'bg-white/20 text-white' : 'bg-slate-100 text-slate-600'">
                                        {{ $pIdx + 1 }}
                                    </span>
                        ...
                        <div x-show="activePreviewId === {{ $preview['id'] }}" x-transition>
                            @if(count($preview['pages']) >= 2)
                                ...
                                <div class="book-container py-2">
                                    <div class="book" style="--book-w: {{ min($preview['base_width'], 160) }}px; --book-h: {{ min($preview['base_height'], 210) }}px;">
                                        ...
                                        <div class="page" style="--i: {{ $i / 2 }}" onclick="this.classList.toggle('flipped')">
                                            <div class="page-face front">
                                                <img src="{{ $preview['pages'][$i] }}" alt="Page {{ $i + 1 }}">
                                            </div>
                                            <div class="page-face back">
                                                @if(isset($preview['pages'][$i+1]))
                                                <img src="{{ $preview['pages'][$i+1] }}" alt="Page {{ $i + 2 }}">
                                                @else
                                ...
                                <p class="text-center text-[11px] text-slate-500 font-bold flex items-center justify-center gap-1.5 mt-2 bg-slate-100/70 py-1.5 rounded-xl border border-slate-200/60">
                            ...
                            @elseif(count($preview['pages']) === 1)
                                ...
                                <div class="flex flex-col items-center justify-center py-2">
                                    <div class="rounded-xl overflow-hidden bg-white border border-slate-200 shadow-md p-1" style="max-width: {{ min($preview['base_width'], 180) }}px; max-height: {{ min($preview['base_height'], 240) }}px;">
                                        <img src="{{ $preview['pages'][0] }}" alt="{{ $preview['name'] }}" class="w-full h-full object-cover rounded-lg">
                                    </div>
                ...
                <div class="glass-card border border-slate-200/90 rounded-2xl p-4 shadow-2xs">
                    <div class="flex items-center gap-2 mb-4 pb-2.5 border-b border-slate-200/80">
                        ...
                        <h2 class="text-sm font-black text-slate-900 tracking-tight">Live Order Fulfillment Progress</h2>
                    </div>
                ...
                <div class="glass-card border border-slate-200/90 rounded-2xl p-4 shadow-2xs">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                        <div class="bg-white p-3 rounded-xl border border-slate-200/80 shadow-2xs">
                            <div class="w-7 h-7 bg-brand-50 border border-brand-100 rounded-lg flex items-center justify-center text-brand-600 mb-2">
                        ...
                        <div class="bg-white p-3 rounded-xl border border-slate-200/80 shadow-2xs">
                            <div class="w-7 h-7 bg-emerald-50 border border-emerald-100 rounded-lg flex items-center justify-center text-emerald-600 mb-2">
                        ...
                        <div class="bg-white p-3 rounded-xl border border-slate-200/80 shadow-2xs">
                            <div class="w-7 h-7 bg-amber-50 border border-amber-100 rounded-lg flex items-center justify-center text-amber-600 mb-2">
                ...
                <div class="glass-card border border-slate-200/90 rounded-2xl p-4 lg:p-5 shadow-lg shadow-slate-200/40 relative overflow-hidden space-y-4">
                    ...
                    <div class="divide-y divide-slate-100 text-xs">
                        <div class="py-2.5 flex justify-between items-center">
                            ...
                            <span class="font-extrabold text-slate-900 text-right max-w-[180px] truncate">{{ $item?->product_name ?? 'Custom Print' }}</span>
                        </div>
                        <div class="py-2.5 flex justify-between items-center">
                        ...
                        <div class="py-2.5 flex justify-between items-center">
                            ...
                            <span class="inline-flex items-center gap-1 text-[10px] font-extrabold text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded-full border border-emerald-200/60">
                                <i data-lucide="check-circle-2" class="w-3 h-3 text-emerald-500"></i> Paid Online
                            </span>
                            @else
                            <span class="inline-flex items-center gap-1 text-[10px] font-extrabold text-amber-700 bg-amber-50 px-2 py-0.5 rounded-full border border-amber-200/60">
                        ...
                        <div class="py-2.5 flex justify-between items-center">
                            ...
                            <span class="inline-flex items-center gap-1 text-[10px] font-extrabold text-brand-700 bg-brand-50 px-2 py-0.5 rounded-full border border-brand-200/60">
                                <i data-lucide="printer" class="w-3 h-3 text-brand-500"></i> {{ ucfirst($order->status) }}
                            </span>
                        </div>
                    </div>
                    ...
                    <div class="my-3.5 bg-brand-50/90 rounded-xl p-3.5 border border-brand-200/80 shadow-2xs">
                        <div class="flex justify-between items-baseline mb-0.5">
                            <span class="text-xs font-extrabold text-slate-700">{{ $order->payment_status === 'paid' ? 'Total Amount Paid' : 'Total Amount Due' }}</span>
                            <span class="text-2xl font-black text-slate-900 tracking-tight">{{ \App\Services\CurrencyService::formatWithCurrency($order->total, $order->currency) }}</span>
                        </div>
                        <p class="text-[10px] font-bold text-slate-500 text-right">Includes taxes & priority print processing</p>
                    </div>
