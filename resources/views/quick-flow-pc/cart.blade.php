        ...
        <div class="max-w-[1360px] mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div
                ...
                <div class="flex items-center gap-3">
                    ...
                    <span
                        class="text-xs font-extrabold text-brand-700 bg-brand-50 px-2.5 py-0.5 rounded-full border border-brand-200/60">
            ...
            @if ($cart->items->count() > 0)
                <div class="flex flex-col lg:flex-row gap-5 items-start">
                    ...
                    <div class="w-full flex-1 min-w-0 space-y-3">
                        ...
                        <div
                            class="hidden lg:grid grid-cols-[1fr_130px_110px_36px] gap-4 px-5 py-2 bg-slate-100/70 backdrop-blur-xs rounded-xl border border-slate-200/80 text-[10px] font-black text-slate-500 uppercase tracking-widest">
                        ...
                        @foreach ($cart->items as $item)
                            ...
                            <div class="glass-card border border-slate-200/90 rounded-2xl p-3.5 sm:p-4 shadow-2xs hover:shadow-md hover:border-brand-300 transition-all duration-300 cart-item-enter relative group overflow-hidden"
                                ...
                                <div
                                    class="absolute top-0 left-0 right-0 h-0.5 bg-brand-500 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                                </div>

                                <div
                                    class="grid grid-cols-1 lg:grid-cols-[1fr_130px_110px_36px] gap-3 sm:gap-4 items-center">
                                    ...
                                    <div class="flex items-center gap-3 sm:gap-4">
                                        ...
                                        <div
                                            class="fanned-card-stage relative flex items-center justify-center w-24 h-24 sm:w-28 sm:h-24 shrink-0 select-none py-1 px-1">
                                            @if ($imageCount > 0)
                                            ...
                                            @else
                                                <div
                                                    class="w-16 h-16 sm:w-20 sm:h-20 rounded-xl overflow-hidden bg-slate-50 border border-slate-200/80 shrink-0 relative flex flex-col items-center justify-center text-slate-300 shadow-2xs">
                                        ...
                                        <div class="min-w-0 flex-1">
                                            @if (!empty($customization['type_name']))
                                                <span
                                                    class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-brand-50 text-brand-600 border border-brand-100 text-[9px] font-black uppercase tracking-wider mb-1">
                                            ...
                                            <div class="flex flex-wrap items-center justify-between gap-2">
                                                <h3
                                                    class="font-black text-slate-900 text-sm sm:text-base leading-snug group-hover:text-brand-600 transition-colors truncate">
                                            ...
                                            <div class="flex items-center gap-1.5 mt-1 flex-wrap">
                                                @if (!empty($customization['size_name']))
                                                    <span
                                                        class="inline-flex items-center gap-1 text-[11px] font-bold text-slate-700 bg-slate-100 px-2 py-0.5 rounded-md border border-slate-200/80">
                                                        ...
                                                        @if (!empty($customization['size_width']) && !empty($customization['size_height']))
                                                            <span
                                                                class="text-slate-500 font-medium">({{ $customization['size_width'] }}×{{ $customization['size_height'] }}{{ $customization['size_unit'] ?? '' }})</span>
                                                        @endif
                                                    </span>
                                                @endif
                                            ...
                                            <div class="mt-1 text-[11px] font-semibold text-slate-500">
                                                Unit Price: <strong class="text-slate-900 font-extrabold"
                                                    x-text="__price(price)"></strong>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    {{-- Quantity Stepper --}}
                                    <div
                                        class="flex flex-col items-center justify-center border-t lg:border-t-0 border-slate-100 pt-2 lg:pt-0">
                                        <span
                                            class="lg:hidden text-[10px] font-bold text-slate-400 uppercase mb-1">Quantity</span>
                                        <div
                                            class="inline-flex items-center bg-slate-100/80 p-0.5 rounded-xl border border-slate-200/80 shadow-inner">
                                            <button type="button" @click="changeQty(qty - 1)"
                                            ...
                                            <span class="w-9 text-center font-black text-slate-900 text-sm"
                                                x-text="qty"></span>
                                    ...
                                    {{-- Item Total Price --}}
                                    <div
                                        class="flex items-center justify-between lg:justify-end border-t lg:border-t-0 border-slate-100 pt-2 lg:pt-0">
                                        <span class="lg:hidden text-[10px] font-bold text-slate-400 uppercase">Item
                                            Total</span>
                                    ...
                                    <div class="flex justify-end lg:justify-center">
                                        <button type="button" @click="removeItem()"
                                            ...
                                            <i data-lucide="trash-2"
                                                class="w-3.5 h-3.5 group-hover/btn:scale-110 transition-transform"></i>
                                        </button>
                                    </div>
                    ...
                    <div class="w-full lg:w-[360px] shrink-0 sticky top-20">
                        <div
                            class="glass-card border border-slate-200/90 rounded-2xl p-4 lg:p-5 shadow-lg shadow-slate-200/40 relative overflow-hidden">
                            ...
                            <div class="bg-amber-50 border border-amber-200/80 rounded-xl p-2.5 mb-3.5 shadow-2xs">
                                <div
                                    class="flex items-center justify-between text-[11px] font-extrabold text-amber-900 mb-1">
                                    <span class="flex items-center gap-1">
                                        <i data-lucide="sparkles" class="w-3.5 h-3.5 text-amber-600"></i> Priority
                                        Delivery
                                    </span>
                                ...
                                <div class="flex items-center justify-between mb-1.5">
                                    ...
                                    <template x-if="appliedCoupon">
                                        <button type="button" @click="removeCoupon()"
                                            class="text-[10px] font-bold text-red-500 hover:text-red-600 transition-colors">
                                            Remove
                                        </button>
                                    </template>
                                </div>
            ...
            @else
                {{-- Empty Cart State --}}
                <div
                    class="max-w-md mx-auto text-center py-20 px-8 bg-white/90 backdrop-blur-md rounded-3xl border border-slate-200/90 shadow-2xl shadow-slate-200/40">
                    <div
                        class="w-28 h-28 rounded-3xl bg-brand-50 border border-brand-100 flex items-center justify-center mx-auto mb-6 shadow-inner relative">
                        <i data-lucide="shopping-bag" class="w-14 h-14 text-brand-500"></i>
                        <div
                            class="absolute -top-1 -right-1 w-7 h-7 rounded-full bg-brand-500 text-white flex items-center justify-center text-xs font-black shadow-md">
                            0</div>
                    </div>
                    <h2 class="font-black text-2xl text-slate-900 mb-2">Your cart is empty</h2>
                    <p class="text-slate-500 font-medium text-sm mb-8 leading-relaxed">You haven't added any custom print
                        templates to your cart yet. Explore our templates and start customizing!</p>
                    <a href="{{ route($routePrefix . 'index') }}"
