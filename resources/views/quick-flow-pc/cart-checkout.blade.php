    ...
    <div class="max-w-[1360px] mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4 bg-white/90 backdrop-blur-md px-5 py-2.5 rounded-2xl border border-slate-200/80 shadow-2xs">
            <div class="flex items-center gap-3">
                ...
                <span class="text-xs font-extrabold text-emerald-700 bg-emerald-50 px-2.5 py-0.5 rounded-full border border-emerald-200/60">
            ...
            <div class="flex items-center gap-3">
                <div class="hidden md:flex items-center gap-2 text-[11px] font-bold">
                    <span class="text-emerald-600 flex items-center gap-1"><i data-lucide="check-circle" class="w-3 h-3"></i> 1. Customize</span>
                    <i data-lucide="chevron-right" class="w-3 h-3 text-slate-300"></i>
                    <span class="text-emerald-600 flex items-center gap-1"><i data-lucide="check-circle" class="w-3 h-3"></i> 2. Cart</span>
                    <i data-lucide="chevron-right" class="w-3 h-3 text-slate-300"></i>
                    <span class="text-brand-600 bg-brand-50 px-2.5 py-0.5 rounded-full border border-brand-200/60">3. Checkout & Pay</span>
                </div>
                <div class="h-4 w-px bg-slate-200 hidden md:block"></div>
                <span class="text-xs font-bold text-slate-600 bg-slate-100 px-2.5 py-1 rounded-xl border border-slate-200/60">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-5 items-start">
            ...
            <div class="lg:col-span-7 space-y-4">
                ...
                <div class="glass-card border border-slate-200/90 rounded-2xl p-4 shadow-2xs">
                    <div class="flex items-center justify-between pb-3 border-b border-slate-200/80 mb-3.5">
                        <div class="flex items-center gap-2">
                            <div class="w-7 h-7 rounded-lg bg-brand-50 border border-brand-100 text-brand-600 flex items-center justify-center">
                                <i data-lucide="package" class="w-4 h-4"></i>
                            </div>
                            <h2 class="text-sm font-black text-slate-900 tracking-tight">Order Items Breakdown</h2>
                        </div>
                        <span class="text-[11px] font-extrabold text-slate-600 bg-slate-100 px-2.5 py-0.5 rounded-full border border-slate-200/60">
                            {{ $cart->item_count }} Item{{ $cart->item_count > 1 ? 's' : '' }}
                        </span>
                    </div>
                        ...
                        @foreach($cart->items as $item)
                            @php
                                ...
                                foreach ($activeSlots as $index => $slotKey) {
                                    ...
                                    } elseif ($index === 0 && is_scalar($itemUploadIds) && isset($uploads[$itemUploadIds]) && !empty($uploads[$itemUploadIds]->url)) {
                            ...
                            <div class="py-3 first:pt-0 last:pb-0 flex items-center gap-5 sm:gap-6">
                                ...
                                <div class="fanned-card-stage relative flex items-center justify-center w-24 h-20 sm:w-28 sm:h-22 shrink-0 select-none py-1 px-1">
                                    @if($imageCount > 0)
                                        ...
                                        @endforeach
                                    @else
                                        <div class="w-14 h-14 rounded-xl overflow-hidden bg-slate-50 border border-slate-200/80 shrink-0 relative flex items-center justify-center text-slate-300">
                                ...
                                <div class="flex-1 min-w-0">
                                    <div class="flex flex-wrap items-center justify-between gap-2 mb-1">
                                        <h4 class="font-black text-xs sm:text-sm text-slate-900 truncate mb-0.5">{{ $item->product->name ?? 'Custom Print' }}</h4>
                                    ...
                                    <div class="flex flex-wrap items-center gap-1.5 text-[11px] text-slate-500 font-semibold">
                                        <span class="bg-slate-100 px-1.5 py-0.5 rounded text-slate-700 font-bold">Qty: {{ $item->quantity }}</span>
                                        @if(!empty($customization['size_name']))
                                            <span class="bg-slate-100 px-1.5 py-0.5 rounded text-slate-700 font-bold">Size: {{ $customization['size_name'] }}</span>
                                        @endif
                                    </div>
                                </div>
                                <div class="text-right flex-shrink-0">
                                    <span class="font-black text-sm sm:text-base text-slate-900 block" x-text="__price({{ $item->unit_price }} * {{ $item->quantity }})"></span>
                                    <span class="text-[10px] font-semibold text-slate-400" x-text="__price({{ $item->unit_price }}) + ' ea'"></span>
                                </div>
                            </div>
                        @endforeach
                    </div>
                ...
                <div class="glass-card border border-slate-200/90 rounded-2xl p-4 shadow-2xs">
                    <div class="flex items-center gap-2.5 mb-3.5 pb-2.5 border-b border-slate-200/80">
                        <div class="w-8 h-8 bg-brand-50 border border-brand-100 rounded-lg flex items-center justify-center text-brand-600 flex-shrink-0">
                            <i data-lucide="user-check" class="w-4 h-4"></i>
                        </div>
                        <div>
                            <h3 class="text-sm font-black text-slate-900 tracking-tight">Pickup Information</h3>
                            <p class="text-[10px] text-slate-500 font-medium">Details of the person collecting this print order</p>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                        ...
                        <div class="md:col-span-2">
                            <label class="block text-[10px] font-extrabold text-slate-600 uppercase tracking-wider mb-1">Pickup Full Name <span class="text-red-500">*</span></label>
                            ...
                            <p x-show="errors.pickupName" x-text="errors.pickupName" class="text-[10px] font-bold text-red-500 mt-1 ml-1 flex items-center gap-1" style="display:none">
                        ...
                        {{-- Email Address --}}
                        <div>
                            <label class="block text-[10px] font-extrabold text-slate-600 uppercase tracking-wider mb-1">Email Address <span class="text-red-500">*</span></label>
                            <div class="relative group">
                                <div class="absolute left-3 top-1/2 -translate-y-1/2 transition-colors"
                            ...
                            <p x-show="errors.pickupEmail" x-text="errors.pickupEmail" class="text-[10px] font-bold text-red-500 mt-1 ml-1 flex items-center gap-1" style="display:none">
                                <i data-lucide="alert-circle" class="w-3 h-3 flex-shrink-0"></i>
                            </p>
                        </div>
                        ...
                        <div>
                            <label class="block text-[10px] font-extrabold text-slate-600 uppercase tracking-wider mb-1">Contact Phone <span class="text-red-500">*</span></label>
                            <div class="relative group">
                                <div class="absolute left-3 top-1/2 -translate-y-1/2 transition-colors"
                            ...
                            <p x-show="errors.contactNumber" x-text="errors.contactNumber" class="text-[10px] font-bold text-red-500 mt-1 ml-1 flex items-center gap-1" style="display:none">
                                <i data-lucide="alert-circle" class="w-3 h-3 flex-shrink-0"></i>
                            </p>
                        </div>
                    </div>
                ...
                <div class="glass-card border border-slate-200/90 rounded-2xl p-4 shadow-2xs">
                    <div class="flex items-center gap-2.5 mb-2.5">
                        <div class="w-7 h-7 bg-slate-100 border border-slate-200/80 rounded-lg flex items-center justify-center text-slate-700 flex-shrink-0">
                            <i data-lucide="message-square" class="w-3.5 h-3.5"></i>
                        </div>
                        <div>
                            <h3 class="text-xs font-black text-slate-900 tracking-tight">Special Instructions</h3>
                            <p class="text-[10px] text-slate-500 font-medium">Optional print notes or store pickup requests</p>
                        </div>
                    </div>
                    <div class="relative group">
            ...
            <div class="lg:col-span-5 sticky top-20">
                <div class="glass-card border border-slate-200/90 rounded-2xl p-4 lg:p-5 shadow-lg shadow-slate-200/40 relative overflow-hidden space-y-4">
                    ...
                    <div class="space-y-1.5">
                        <div class="flex items-center justify-between">
                            ...
                            <template x-if="appliedCoupon">
                                <button type="button" @click="removeCoupon()" class="text-[10px] font-bold text-red-500 hover:underline uppercase">Remove</button>
                            </template>
                        </div>
                    ...
                    <div class="space-y-2 pt-1 pb-3.5 border-b border-slate-200/80 text-xs">
                        ...
                        <template x-if="discountAmount > 0">
                            <div class="flex justify-between items-center text-emerald-600 font-semibold">
                                <span x-text="'Discount (' + appliedCoupon + ')'" class="flex items-center gap-1"><i data-lucide="tag" class="w-3 h-3"></i></span>
                        ...
                        <div class="flex justify-between items-center">
                            ...
                            <span class="text-[10px] font-black text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded-full border border-emerald-200/60">FREE Pickup</span>
                        </div>
                    </div>
                    ...
                    <div class="my-3.5 bg-brand-50/90 rounded-xl p-3.5 border border-brand-200/80 shadow-2xs">
                        <div class="flex justify-between items-baseline mb-0.5">
                            ...
                            <span class="text-2xl font-black text-slate-900 tracking-tight" x-text="__price(calculateTotal())"></span>
                        </div>
                    ...
                    <div>
                        <div class="flex items-start gap-2.5 p-3 bg-slate-50 border transition-all rounded-xl"
                            :class="errors.terms ? 'border-red-400 bg-red-50/30' : (acceptedTerms ? 'border-emerald-300 bg-emerald-50/20' : 'border-slate-200')">
                            ...
                            <label for="terms-checkbox-pc" class="text-[11px] font-medium text-slate-700 cursor-pointer select-none leading-tight">
                        ...
                        <p x-show="errors.terms" x-text="errors.terms" class="text-[10px] font-bold text-red-500 mt-1 ml-1 flex items-center gap-1" style="display:none">
                            <i data-lucide="alert-circle" class="w-3 h-3 flex-shrink-0"></i>
                        </p>
                    </div>
                    ...
                    <div class="pt-2.5 border-t border-slate-100 grid grid-cols-2 gap-1.5 text-[10px] font-semibold text-slate-400">
            ...
            <div x-cloak>
                <div x-show="showPaypal" class="paypal-overlay" @click.self="showPaypal = false">
                    <div class="paypal-sheet" @click.stop>
                        ...
                        <div class="bg-brand-50/90 text-slate-900 rounded-xl p-3.5 mb-4 flex items-center justify-between border border-brand-200/80 shadow-2xs">
                            <div>
                                <p class="text-[10px] font-extrabold text-slate-500 uppercase tracking-widest">Amount to Pay</p>
                                <p class="text-xl font-black text-slate-900" x-text="__price(calculateTotal())"></p>
                            </div>
                            <div class="text-right">
                                <p class="text-[10px] font-extrabold text-slate-500 uppercase tracking-widest">Pickup For</p>
                                <p class="text-xs font-bold text-slate-800 truncate max-w-[140px]" x-text="pickupName"></p>
                            </div>
                        </div>
