...
@section('content')
    @php
        ...
        $bcPageSizeSide = $flowData['size_name'] 
            ...
            ?? ($product->no_of_pages ? ($product->no_of_pages == 1 ? 'Single Side' : ($product->no_of_pages == 2 ? 'Double Side' : $product->no_of_pages.' Pages')) : 'Standard Size');
        ...
        <section class="w-full relative py-8 px-4 sm:px-6 lg:px-10 border-b border-slate-200/80"
            ...
            <div class="max-w-[1400px] mx-auto">
                ...
                <div class="relative w-full min-h-[80vh] flex items-center justify-center">
                    ...
                    <div
                        class="absolute left-2 lg:left-6 top-1/2 -translate-y-1/2 flex flex-col items-center gap-3 shrink-0 z-30 py-2 px-1">
                        ...
                        @foreach ($imageTypes as $key => $img)
                            ...
                            <button type="button" onclick="customizer.switchCanvas('{{ $key }}')"
                                ...
                                <span
                                    class="text-[10px] {{ $isFirst ? 'font-black text-pink-600' : 'font-bold text-slate-500 group-hover:text-pink-600' }} transition-colors">
                                    Page {{ $loop->iteration }}
                                </span>
                            </button>
                        @endforeach
                        ...
                        <button type="button" onclick="toggleTemplatesDrawer()"
                            ...
                            <span
                                class="text-[10px] font-black text-slate-600 group-hover:text-indigo-600 transition-colors">Templates</span>
                        </button>
                        ...
                        <div class="w-full flex items-center justify-center min-h-[80vh] py-6 relative" id="canvas-stage">
                            ...
                            <div class="canvas-wrapper bg-white shadow-2xl rounded-2xl overflow-hidden relative mx-auto flex items-center justify-center transition-all duration-200"
                                ...
                                <div id="canvas-loading-overlay"
                                    ...
                                    <div
                                        class="w-12 h-12 rounded-2xl bg-indigo-50 border border-indigo-100 flex items-center justify-center shadow-lg shadow-indigo-500/10">
            ...
            <div class="hero-glass-card hero-cust-pattern relative overflow-hidden rounded-3xl p-6 sm:p-8 lg:p-10 border border-white/80 transition-all duration-300">
                ...
                <div class="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                    <div class="flex items-center gap-5">
                        ...
                        <div class="space-y-1">
                            <div class="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-white/90 border border-pink-200/80 text-pink-600 text-[11px] font-black shadow-2xs backdrop-blur-md tracking-wider uppercase">
                                <i data-lucide="wand-2" class="w-3.5 h-3.5 text-pink-500 animate-spin-slow"></i>
                                Interactive Design Studio
                            </div>
                            <h1 class="text-2xl lg:text-3xl xl:text-4xl font-black text-slate-900 tracking-tight leading-tight">
                                Customize <span class="bg-gradient-to-r from-brand-600 via-purple-600 to-violet-600 bg-clip-text text-transparent italic" style="font-family: 'Playfair Display', serif;">{{ $product->name }}</span>
                            </h1>
                            <p class="text-xs lg:text-sm text-slate-500 font-semibold flex items-center gap-2">
                                <span class="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-ping"></span>
                                Switch between pages from the left dock to design all four sides.
                            </p>
                        </div>
                    </div>
            ...
            updateUI() {
                ...
                Object.keys(this.imageTypes).forEach((k) => {
                    ...
                    if (btnIcon) {
                        ...
                        if (k === key) {
                            btnIcon.className =
                                'w-11 h-11 rounded-2xl bg-white text-pink-600 shadow-md border-2 border-pink-500 flex items-center justify-center scale-105 transition-all duration-200 relative ' +
                            ...
                            btnIcon.className =
                                'w-11 h-11 rounded-2xl bg-white text-slate-500 shadow-2xs border border-slate-200/90 flex items-center justify-center group-hover:text-pink-600 group-hover:border-pink-300 group-hover:scale-105 transition-all duration-200 relative ' +
                                (!isEnabled ? 'opacity-60' : '');
                            if (btnWrapper) {
                                const txt = btnWrapper.querySelector('span');
                                if (txt) txt.className =
                                    'text-[10px] font-bold text-slate-500 group-hover:text-pink-600 transition-colors';
                            }
                        }
                    }
                });
