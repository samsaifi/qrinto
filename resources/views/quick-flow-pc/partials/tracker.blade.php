...
<div class="relative flex justify-between items-start w-full pt-2">
    ...
    <div class="absolute left-5 top-[22px] h-[3px] bg-indigo-500 rounded-full z-0 transition-all duration-700 ease-out" style="width: calc({{ $progressWidth }}% - 10px)"></div>
    ...
    @foreach($statusArray as $index => $step)
        ...
        <div class="relative z-10 flex flex-col items-center" style="width: 20%">
            <div class="w-11 h-11 rounded-full flex items-center justify-center transition-all duration-300
                {{ $isCompleted ? 'bg-indigo-500 text-white shadow-md shadow-indigo-200' : '' }}
            ...
            <span class="mt-2.5 text-[11px] font-bold {{ $isCompleted || $isCurrent ? 'text-indigo-600' : 'text-slate-400' }} tracking-wide text-center">
                {{ ucfirst($step) }}
            </span>
        </div>
    @endforeach
