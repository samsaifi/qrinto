...
<div id="shapes-studio-drawer"
    ...
    <div class="grid grid-cols-3 gap-2.5">
        <button type="button" onclick="customizer.addShape('rectangle')" class="p-3 bg-slate-50 border border-slate-200 hover:border-brand-500 rounded-xl flex flex-col items-center gap-1.5 hover:bg-brand-50 transition-all">
            <div class="w-8 h-6 bg-brand-500 rounded-xs"></div>
            <span class="text-[10px] font-bold text-slate-700">Rectangle</span>
        </button>
        <button type="button" onclick="customizer.addShape('circle')" class="p-3 bg-slate-50 border border-slate-200 hover:border-brand-500 rounded-xl flex flex-col items-center gap-1.5 hover:bg-brand-50 transition-all">
            <div class="w-7 h-7 bg-brand-500 rounded-full"></div>
            <span class="text-[10px] font-bold text-slate-700">Circle</span>
        </button>
        <button type="button" onclick="customizer.addShape('triangle')" class="p-3 bg-slate-50 border border-slate-200 hover:border-brand-500 rounded-xl flex flex-col items-center gap-1.5 hover:bg-brand-50 transition-all">
            <div class="w-0 h-0 border-l-[14px] border-l-transparent border-r-[14px] border-r-transparent border-b-[24px] border-b-brand-500"></div>
            <span class="text-[10px] font-bold text-slate-700">Triangle</span>
        </button>
        <button type="button" onclick="customizer.addShape('star')" class="p-3 bg-slate-50 border border-slate-200 hover:border-brand-500 rounded-xl flex flex-col items-center gap-1.5 hover:bg-brand-50 transition-all">
            <i data-lucide="star" class="w-6 h-6 text-brand-500 fill-brand-500"></i>
            <span class="text-[10px] font-bold text-slate-700">Star</span>
        </button>
        <button type="button" onclick="customizer.addShape('heart')" class="p-3 bg-slate-50 border border-slate-200 hover:border-brand-500 rounded-xl flex flex-col items-center gap-1.5 hover:bg-brand-50 transition-all">
    ...
    <div class="pt-3 border-t border-slate-100 space-y-3">
        <label class="block text-[10px] font-black uppercase text-slate-400 tracking-wider">Customize Selected Shape</label>
        
        <div class="grid grid-cols-2 gap-2">
            {{-- 1. Change Color --}}
            <label class="relative w-full bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs py-2 px-3 rounded-xl flex items-center justify-center gap-1.5 transition-all cursor-pointer">
                <i data-lucide="palette" class="w-3.5 h-3.5 text-brand-600"></i> Fill Color
                <input type="color" id="shape-color-input" oninput="customizer.setShapeColor(this.value)" class="absolute inset-0 opacity-0 w-full h-full cursor-pointer">
            </label>

            {{-- 2. Remove Solid Fill (Transparent) --}}
            <button type="button" onclick="customizer.removeShapeFill()" class="w-full bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs py-2 px-3 rounded-xl flex items-center justify-center gap-1.5 transition-all">
        ...
        <div class="pt-2 space-y-2">
            <div class="flex items-center justify-between">
                ...
                <label class="flex items-center gap-1.5 cursor-pointer">
                    <input type="checkbox" id="shape-lib-border-show" onchange="customizer.setShapeBorderShow(this.checked)" class="w-4 h-4 rounded text-brand-600 focus:ring-brand-500 cursor-pointer">
            ...
            <div class="flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-xl p-1.5">
                <input type="color" id="shape-lib-border-color" value="#6FB63A" oninput="document.getElementById('shape-lib-border-color-val').innerText = this.value; customizer.setShapeBorderColor(this.value);" class="w-6 h-6 rounded-lg border-0 cursor-pointer p-0 bg-transparent shrink-0">
                <span id="shape-lib-border-color-val" class="text-[11px] font-bold text-slate-700 uppercase truncate">#6FB63A</span>
                <input type="range" id="shape-lib-border-width" min="0" max="50" value="3" step="1" oninput="document.getElementById('shape-lib-border-width-val').innerText = this.value + 'px'; customizer.setShapeBorderWidth(this.value);" class="flex-1 h-1.5 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-brand-600">
                <span id="shape-lib-border-width-val" class="text-[11px] font-extrabold text-brand-600 shrink-0 w-9 text-right">3px</span>
            </div>
        </div>

        {{-- 3. Add Image onto Shape --}}
        <div>
            <label for="shape-img-fill-input" class="w-full bg-brand-50 hover:bg-brand-100 border border-brand-200 text-brand-700 font-extrabold text-xs py-2.5 px-3 rounded-xl flex items-center justify-center gap-2 cursor-pointer transition-all shadow-2xs">
                <i data-lucide="image-plus" class="w-4 h-4 text-brand-600"></i> Fill Image into Shape
            </label>
            <input type="file" id="shape-img-fill-input" accept="image/*" onchange="customizer.fillShapeWithImage(this)" class="hidden">
        </div>

        {{-- 4. Reposition Image Inside Shape --}}
        <div id="reposition-shape-btn-wrap" class="hidden">
            <button type="button" onclick="customizer.enterImageRepositionMode()" class="w-full bg-purple-50 hover:bg-purple-100 border border-purple-200 text-purple-700 font-extrabold text-xs py-2.5 px-3 rounded-xl flex items-center justify-center gap-2 transition-all shadow-2xs">
...
<div id="qr-studio-drawer"
    ...
    <div>
        <label class="block text-[10px] font-black uppercase text-slate-400 tracking-wider mb-1.5">Website URL / Contact Data</label>
        <input type="text" id="qr-input-text" placeholder="https://example.com"
            class="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2.5 text-xs font-semibold text-slate-800 focus:bg-white focus:border-emerald-500 outline-none">
    </div>

    <button type="button" onclick="customizer.addQrCode(document.getElementById('qr-input-text').value)"
        class="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold text-xs py-3 rounded-xl shadow-lg shadow-emerald-500/20 transition-all flex items-center justify-center gap-2">
...
<div id="shape-mask-drawer"
    ...
    <div class="grid grid-cols-4 gap-2 max-h-72 overflow-y-auto custom-scrollbar p-1">
        <button type="button" data-shape="rect" onclick="customizer.applyShapeMaskToSelectedImage('rect');" class="shape-mask-btn relative flex flex-col items-center gap-1 p-2 rounded-xl bg-slate-50 hover:bg-emerald-50 border border-slate-200 hover:border-emerald-300 transition-all group cursor-pointer">
            <div class="w-8 h-8 rounded bg-slate-700 group-hover:bg-emerald-600 transition-colors"></div>
            <span class="text-[10px] font-bold text-slate-600 group-hover:text-emerald-700">Rectangle</span>
        </button>

        <button type="button" data-shape="rounded-rect" onclick="customizer.applyShapeMaskToSelectedImage('rounded-rect');" class="shape-mask-btn relative flex flex-col items-center gap-1 p-2 rounded-xl bg-slate-50 hover:bg-emerald-50 border border-slate-200 hover:border-emerald-300 transition-all group cursor-pointer">
            <div class="w-8 h-8 rounded-lg bg-slate-700 group-hover:bg-emerald-600 transition-colors"></div>
            <span class="text-[10px] font-bold text-slate-600 group-hover:text-emerald-700">Rounded</span>
        </button>

        <button type="button" data-shape="circle" onclick="customizer.applyShapeMaskToSelectedImage('circle');" class="shape-mask-btn relative flex flex-col items-center gap-1 p-2 rounded-xl bg-slate-50 hover:bg-emerald-50 border border-slate-200 hover:border-emerald-300 transition-all group cursor-pointer">
            <div class="w-8 h-8 rounded-full bg-slate-700 group-hover:bg-emerald-600 transition-colors"></div>
            <span class="text-[10px] font-bold text-slate-600 group-hover:text-emerald-700">Circle</span>
        </button>

        <button type="button" data-shape="oval" onclick="customizer.applyShapeMaskToSelectedImage('oval');" class="shape-mask-btn relative flex flex-col items-center gap-1 p-2 rounded-xl bg-slate-50 hover:bg-emerald-50 border border-slate-200 hover:border-emerald-300 transition-all group cursor-pointer">
            <div class="w-8 h-6 rounded-full bg-slate-700 group-hover:bg-emerald-600 transition-colors my-1"></div>
            <span class="text-[10px] font-bold text-slate-600 group-hover:text-emerald-700">Oval</span>
        </button>

        <button type="button" data-shape="triangle" onclick="customizer.applyShapeMaskToSelectedImage('triangle');" class="shape-mask-btn relative flex flex-col items-center gap-1 p-2 rounded-xl bg-slate-50 hover:bg-emerald-50 border border-slate-200 hover:border-emerald-300 transition-all group cursor-pointer">
            <div class="w-0 h-0 border-l-[14px] border-l-transparent border-r-[14px] border-r-transparent border-b-[24px] border-b-slate-700 group-hover:border-b-emerald-600 my-0.5"></div>
            <span class="text-[10px] font-bold text-slate-600 group-hover:text-emerald-700">Triangle</span>
        </button>

        <button type="button" data-shape="diamond" onclick="customizer.applyShapeMaskToSelectedImage('diamond');" class="shape-mask-btn relative flex flex-col items-center gap-1 p-2 rounded-xl bg-slate-50 hover:bg-emerald-50 border border-slate-200 hover:border-emerald-300 transition-all group cursor-pointer">
            <div class="w-6 h-6 rotate-45 bg-slate-700 group-hover:bg-emerald-600 transition-colors my-1"></div>
            <span class="text-[10px] font-bold text-slate-600 group-hover:text-emerald-700">Diamond</span>
        </button>

        <button type="button" data-shape="star" onclick="customizer.applyShapeMaskToSelectedImage('star');" class="shape-mask-btn relative flex flex-col items-center gap-1 p-2 rounded-xl bg-slate-50 hover:bg-emerald-50 border border-slate-200 hover:border-emerald-300 transition-all group cursor-pointer">
            <i data-lucide="star" class="w-7 h-7 text-slate-700 group-hover:text-emerald-600 fill-current my-0.5"></i>
            <span class="text-[10px] font-bold text-slate-600 group-hover:text-emerald-700">Star</span>
        </button>

        <button type="button" data-shape="heart" onclick="customizer.applyShapeMaskToSelectedImage('heart');" class="shape-mask-btn relative flex flex-col items-center gap-1 p-2 rounded-xl bg-slate-50 hover:bg-emerald-50 border border-slate-200 hover:border-emerald-300 transition-all group cursor-pointer">
            <i data-lucide="heart" class="w-7 h-7 text-slate-700 group-hover:text-emerald-600 fill-current my-0.5"></i>
            <span class="text-[10px] font-bold text-slate-600 group-hover:text-emerald-700">Heart</span>
        </button>
    </div>
    ...
    <div class="pt-3 border-t border-slate-100 space-y-3">
        <div class="flex items-center justify-between">
            ...
            <label class="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" id="shape-border-show-checkbox" onchange="customizer.setShapeBorderShow(this.checked)" class="w-4 h-4 rounded text-emerald-600 focus:ring-emerald-500 cursor-pointer">
        ...
        <div id="shape-border-options-wrap" class="space-y-2.5">
            ...
            <div class="grid grid-cols-2 gap-2">
                <div class="flex flex-col gap-1">
                    ...
                    <div class="flex items-center gap-2 bg-slate-50 border border-slate-200 rounded-xl p-1.5">
                        <input type="color" id="shape-border-color-input" value="#378ADD" oninput="document.getElementById('shape-border-color-val').innerText = this.value; customizer.setShapeBorderColor(this.value);" class="w-6 h-6 rounded-lg border-0 cursor-pointer p-0 bg-transparent">
                        <span id="shape-border-color-val" class="text-[11px] font-bold text-slate-700 uppercase truncate">#378ADD</span>
                    </div>
                </div>
                <div class="flex flex-col gap-1">
                    <span class="text-[10px] font-extrabold text-slate-500 uppercase">Style</span>
                    <select id="shape-border-style-select" onchange="customizer.setShapeBorderStyle(this.value)" class="text-xs font-bold bg-slate-50 border border-slate-200 rounded-xl p-2 text-slate-700 outline-none cursor-pointer">
            ...
            <div class="space-y-1">
                <div class="flex items-center justify-between text-[11px] font-bold text-slate-600">
                    <span>Thickness</span>
                    <span id="shape-border-width-val" class="font-extrabold text-emerald-600">3px</span>
                </div>
                <input type="range" id="shape-border-width-slider" min="0" max="50" value="3" step="1" oninput="document.getElementById('shape-border-width-val').innerText = this.value + 'px'; customizer.setShapeBorderWidth(this.value);" class="w-full h-1.5 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-emerald-600">
            </div>
        </div>
    </div>

    <div class="pt-2 border-t border-slate-100">
        <button type="button" onclick="customizer.removeShapeMaskFromImage(); toggleShapeMaskDrawer();" class="w-full bg-red-50 hover:bg-red-100 text-red-600 font-extrabold text-xs py-2 px-3 rounded-xl flex items-center justify-center gap-1.5 transition-all cursor-pointer">
