            ...
            <div id="text-toolbar" class="hidden text-toolbar flex flex-wrap items-center gap-3">
                <div class="relative flex-1 min-w-[140px]">
                    <input type="text" id="text-input" placeholder="Add text..."
                        class="w-full" oninput="customizer.onTextInputChange(this.value)" onkeydown="if(event.key==='Enter') { event.preventDefault(); this.blur(); }">
...
<script>
    const customizer = {
            ...
            Object.keys(this.imageTypes).forEach(key => {
                ...
                if (key === firstKey && Array.isArray(masks) && masks.length > 0) {
                    ...
                    const guide = new fabric.Rect({
                        left: m.left * scaleFactor,
                        top: m.top * scaleFactor,
                        width: m.width * (m.scaleX || 1) * scaleFactor,
                        height: m.height * (m.scaleY || 1) * scaleFactor,
                        angle: m.angle || 0,
                        fill: 'transparent',
                ...
                if (data.objects && data.objects.length > 0) {
                    fabric.util.enlivenObjects(data.objects, (enlivenedObjects) => {
                        enlivenedObjects.forEach(obj => {
                            ...
                            const firstKey = Object.keys(this.imageTypes)[0];
                            const mData = this.allMaskData[key] || {};
                            const masks = mData.masks || this.allMaskData.masks; // Support keyed or flat
                            if (key === firstKey && Array.isArray(masks) && masks.length > 0) {
                                const m = masks[0];
                                ...
                                obj.set('clipPath', new fabric.Rect({
                                    left: m.left * sf,
                                    top: m.top * sf,
                                    width: m.width * (m.scaleX || 1) * sf,
                                    height: m.height * (m.scaleY || 1) * sf,
                                    angle: m.angle || 0,
        ...
        addText(isLiveType = false) {
            ...
            const temp = new fabric.Text(textVal, {
                fontSize,
                fontFamily
            });
            const maxW = cv.fabricCanvas.width * 0.9;
            const desiredWidth = temp.width + (fontSize * 0.2);

            const t = new fabric.Textbox(textVal, {
                left: cv.fabricCanvas.width * 0.05,
            ...
            if (key === firstKey && Array.isArray(masks) && masks.length > 0) {
                const m = masks[0];
                const sf = cv.scaleFactor;
                t.set('clipPath', new fabric.Rect({
                    left: m.left * sf,
                    top: m.top * sf,
                    width: m.width * (m.scaleX || 1) * sf,
                    height: m.height * (m.scaleY || 1) * sf,
                ...
                cv.fabricCanvas.getObjects().forEach(o => {
                    ...
                    if (key === firstKey && Array.isArray(masks) && masks.length > 0 && o.clipPath) {
                        ...
                        o.clipPath.set({
                            left: m.left * newSf,
                            top: m.top * newSf,
                            width: m.width * (m.scaleX || 1) * newSf,
                            height: m.height * (m.scaleY || 1) * newSf,
                            angle: m.angle || 0
                        });
                    }
