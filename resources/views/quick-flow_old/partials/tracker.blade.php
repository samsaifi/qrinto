<div class="bg-white border-2 border-slate-50 rounded-[2rem] p-6 shadow-premium text-left mt-6">
    ...
    <div class="relative flex justify-between items-center w-full">
        ...
        @endphp
        <div class="absolute left-0 top-1/2 -translate-y-1/2 h-1 bg-pink-500 rounded-full z-0 transition-all duration-500 ease-in-out" style="width: {{ $progressWidth }}%"></div>
        ...
        @foreach($statusArray as $index => $step)
            ...
            <div class="relative z-10 flex flex-col items-center group">
                <div class="w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300 {{ $isCompleted || $isCurrent ? 'bg-pink-500 text-white shadow-lg shadow-pink-200 scale-110' : 'bg-slate-100 border-4 border-white text-slate-400' }}">
                    @if($isCompleted)
                ...
                <div class="absolute top-12 whitespace-nowrap">
                    <span class="text-[11px] font-bold {{ $isCompleted || $isCurrent ? 'text-pink-600' : 'text-slate-400' }} tracking-wide">{{ ucfirst($step) }}</span>
                </div>
            </div>
        @endforeach
    </div>
