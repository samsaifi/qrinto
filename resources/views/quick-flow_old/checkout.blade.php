...
<div x-data="checkoutFlow()" class="space-y-6 pb-32 font-sans text-slate-900">
    <div class="space-y-1">
        <h1 class="text-2xl font-extrabold">Final Review</h1>
        <p class="text-slate-500 font-medium text-sm">Review your custom design before payment</p>
    </div>
    ...
    <div class="bg-white border-2 border-slate-50 rounded-[2.5rem] shadow-premium overflow-hidden">
        ...
        <div class="p-6 space-y-3">
            ...
            <div class="flex justify-between items-center border-t border-slate-100 pt-4">
                ...
                <span class="text-2xl font-black text-brand-600" x-text="'$' + calculateTotal()"></span>
            </div>
        </div>
    </div>
                ...
                <div class="paypal-sheet" @click.stop>
                    ...
                    <div class="bg-slate-50 rounded-xl p-4 mb-4 flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-2xl font-black text-slate-900" x-text="'$' + calculateTotal()"></p>
                        </div>
    ...
    <div class="fixed bottom-0 left-0 right-0 max-w-md mx-auto p-4 glass border-t border-slate-100 safe-bottom z-50 space-y-3">
        ...
        <button type="button" 
            ...
            <span x-text="pickupName && contactNumber && pickupEmail && acceptedTerms ? 'Pay Now — $' + calculateTotal() : (acceptedTerms ? 'Complete All Info' : 'Accept Terms to Continue')"></span>
        </button>
        <button type="button" 
                @click="payByCash()"
                :disabled="!pickupName || !contactNumber || !pickupEmail || !acceptedTerms"
...
@push('scripts')
<!-- PayPal SDK -->
<script src="https://www.paypal.com/sdk/js?client-id=<?php echo $paypalClientId; ?>&currency=USD&intent=capture"></script>
<script>
