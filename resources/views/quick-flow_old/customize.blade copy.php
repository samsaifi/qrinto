...
@php
$imageTypes = [
'frame_image' => ['label'=>'Frame', 'url'=>$product->frame_image_url],
'sample_image' => ['label'=>'Sample', 'url'=>$product->sample_image_url],
'background_image' => ['label'=>'Background', 'url'=>$product->background_image_url],
'overlay_image' => ['label'=>'Overlay', 'url'=>$product->overlay_image_url],
];
            ...
            <div id="text-toolbar" class="hidden text-toolbar flex flex-wrap items-center gap-3">
                <div class="relative flex-1 min-w-[140px]">
                    <input type="text" id="text-input" placeholder="Add text..."
                        class="w-full" oninput="customizer.onTextInputChange(this.value)" onkeydown="if(event.key==='Enter') customizer.addText()">
                    <button id="clear-text-btn" onclick="customizer.clearSelection()"
                        class="hidden absolute right-3 top-1/2 -translate-y-1/2 text-slate-300 hover:text-slate-500 transition-colors">
                        <i data-lucide="x-circle" class="w-4 h-4"></i>
                    </button>
                </div>

                <div class="flex items-center gap-2 overflow-x-auto no-scrollbar">
                    <select id="font-family-select" onchange="customizer._updateSelectedStyle('fontFamily', this.value)" class="shrink-0 bg-transparent" style="width: 100px;">
                        <option>Arial</option>
                    ...
                    <select id="text-align-select" onchange="customizer._updateSelectedStyle('textAlign', this.value)" class="shrink-0 bg-transparent">
                        <option value="left">Left</option>
                        <option value="center">Center</option>
                        <option value="right">Right</option>
                        <option value="justify">Justify</option>
                    </select>
                    <select id="font-size-select" onchange="customizer._updateSelectedStyle('fontSize', parseInt(this.value))" class="shrink-0 bg-transparent">
                        @for($i=8; $i<=96; $i+=2)
                    ...
                    <div class="shrink-0">
                        <button id="add-text-btn" onclick="customizer.addText()" class="bg-brand-500 hover:bg-brand-600 text-white h-10 px-5 rounded-xl text-sm font-bold shadow-lg shadow-brand-100 transition-all active:scale-95">
                            + Add
                        </button>
                        <div id="editing-badge" class="hidden flex items-center gap-2 h-10 px-4 bg-brand-50 text-brand-600 rounded-xl border border-brand-100">
                            <i data-lucide="type" class="w-4 h-4"></i>
                            <span class="text-xs font-black uppercase tracking-widest">Editing</span>
                        </div>
                    </div>
                </div>
            </div>
    ...
    const customizer = {
        ...
        _initAllCanvases() {
            ...
            Object.keys(this.imageTypes).forEach(key => {
                ...
                const isPortrait = @json(($product - > pdf_orientation ?? 'portrait') === 'portrait');
                ...
                fc.on('object:scaling', (e) => {
                    if (e.target._isUserImage) {
                        ...
                        document.getElementById('zoom-slider').value = e.target.scaleX;
                    }
                });
        ...
        _updateSelectedStyle(property, value) {
        ...
        async handleFileUpload(input) {
            ...
            try {
                ...
                const res = await fetch('{{ route("flow.upload") }}', {
                    method: 'POST',
                    body: fd
                });
        ...
        addText() {
            ...
            const t = new fabric.Textbox(textVal, {
                ...
                textAlign: document.getElementById('text-align-select').value,
                _isUserText: true,
                objectCaching: false,
                cornerSize: 12,
                transparentCorners: false,
        ...
        submitAllCanvases() {
            ...
            if (!hasUpload || this.isSavingComposite) return;
            ...
            const uploadPromises = Object.keys(this.canvases).map(async key => {
                ...
                const res = await fetch('{{ route("flow.upload_composite") }}', {
                    method: 'POST',
                    headers: {
