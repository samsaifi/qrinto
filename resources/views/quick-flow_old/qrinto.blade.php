                ...
                <template x-for="size in selectedParent?.subtypes || []" :key="size.id">
                    <button @click="selectedSize = { ...size, label: `${selectedParent.name}: ${size.name}` }"
                        ...
                        <div class="text-right">
                            <p class="font-black text-brand-600" x-text="`$${size.price.toFixed(2)}`"></p>
    ...
    <div x-show="currentStep === 3" x-transition:enter="transition ease-out duration-300" class="space-y-6">
        ...
        <div class="bg-white rounded-3xl border border-slate-100 p-6 text-left space-y-6 shadow-sm">
            <div class="flex gap-4">
                ...
                <div class="text-right flex flex-col justify-center">
                    <p class="font-black text-brand-600 text-lg" x-text="`$${(selectedSize?.price * quantity).toFixed(2)}`"></p>
                </div>
            </div>
        ...
        <div class="space-y-3 pt-6">
            <button type="button" 
                ...
                <span x-text="isValid ? 'Pay Now — $' + (selectedSize?.price * quantity).toFixed(2) : 'Complete All Info'"></span>
            </button>
            <button type="button" 
            ...
            <div class="paypal-sheet" @click.stop>
                ...
                <div class="bg-slate-50 rounded-xl p-4 mb-4 flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-2xl font-black text-slate-900" x-text="'$' + (selectedSize?.price * quantity).toFixed(2)"></p>
                    </div>
                    <div class="text-right">
            ...
            initPaypal() {
                ...
                paypal.Buttons({
                    createOrder: async (data, actions) => {
                        const response = await fetch('{{ route("flow.qrinto.paypal.create") }}', {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                                'X-CSRF-TOKEN': '{{ csrf_token() }}'
                    ...
                    onApprove: async (data, actions) => {
                        ...
                        const response = await fetch('{{ route("flow.qrinto.paypal.capture") }}', {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                                'X-CSRF-TOKEN': '{{ csrf_token() }}'
