...
<div x-data="headerStoreSelector()" class="relative flex items-center shrink-0 group">
    ...
    <button type="button" @click="isOpen = !isOpen"
        ...
        <template x-if="storeId">
            <span class="absolute top-2 right-2 w-2.5 h-2.5 rounded-full  bg-gray-500 ring-2 ring-white"></span>
        </template>
        <template x-if="!storeId">
            <span
    ...
    <div x-show="isOpen || isHovered" @mouseenter="isHovered = true" @mouseleave="isHovered = false"
        ...
        style="display: none;" @click.outside="isOpen = false">
        ...
        <template x-if="!storeId">
            <div class="p-4 bg-white text-slate-900 space-y-3 relative z-10">
                <div class="flex items-center justify-between">
                    <span
                        class="text-[10px] font-black uppercase text-brand-700 bg-brand-50 border border-brand-200/80 px-2 py-0.5 rounded-md tracking-wider flex items-center gap-1">
                    ...
                    <div class="space-y-3">
                        <div class="p-3 rounded-xl bg-slate-50 border border-slate-200/80">
                            <div class="flex items-start gap-2.5">
                                <div
                                    class="w-8 h-8 rounded-lg bg-brand-500 text-white flex items-center justify-center shrink-0 mt-0.5">
                                    <i data-lucide="store" class="w-4 h-4"></i>
                                </div>
                                <div>
                                    <h4 class="font-black text-slate-900 text-sm leading-tight"
                                        x-text="suggestedStore.store_name"></h4>
                                    <p class="text-xs text-slate-500 font-medium mt-0.5"
                                        x-text="[suggestedStore.address, suggestedStore.city, suggestedStore.state].filter(Boolean).join(', ')">
                                    </p>
                                </div>
                            </div>
                        </div>
