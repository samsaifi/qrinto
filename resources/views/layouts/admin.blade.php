...
<body class="font-sans antialiased bg-surface-50" x-data="{ sidebarOpen: true }">
    ...
    <div class="flex min-h-screen">
        ...
        <aside :class="sidebarOpen ? 'w-64' : 'w-20'"
            class="fixed inset-y-0 left-0 {{ $isStoreAdmin ? 'bg-gradient-to-b from-surface-900 to-surface-950' : 'bg-surface-900' }} transition-all duration-300 z-50 flex flex-col">
            <!-- Logo -->
                ...
                <div class="flex items-center gap-3 px-5 py-3 border-b border-surface-800">
                    @if ($userStore->logo)
                        <div class="w-9 h-9 rounded-lg overflow-hidden flex-shrink-0 ring-2 ring-emerald-500/30">
                            <img src="{{ asset('storage/' . $userStore->logo) }}" alt="{{ $userStore->store_name }}"
                                class="w-full h-full object-cover">
                        </div>
                    @else
                        <div
                            class="w-9 h-9 rounded-lg bg-gradient-to-br from-emerald-500 to-teal-600 flex items-center justify-center flex-shrink-0">
                            <i data-lucide="store" class="w-4 h-4 text-white"></i>
                        </div>
                    @endif
                    <div x-show="sidebarOpen" x-transition class="min-w-0">
                        <span class="font-display font-bold text-white text-sm whitespace-nowrap block">Store
                            Panel</span>
                        <span class="text-xs  text-gray-400 truncate block max-w-[160px]"
                            title="{{ $userStore->store_name }}">{{ $userStore->store_name }}</span>
                    </div>
                </div>
            @else
                <div class="flex items-center gap-3 px-5 h-16 border-b border-surface-800">
                    <div
                        class="w-8 h-8 rounded-lg bg-gradient-to-br from-brand-500 to-brand-700 flex items-center justify-center flex-shrink-0">
            ...
            @if (auth()->user()->isAdmin())
                <div class="relative  flex w-full gap-4 rounded-lg border border-white/[0.06] bg-white/[0.04] p-[3px]">
                    ...
                    <!-- Slider -->
                    <div id="modeSlider"
                        class="absolute inset-y-[3px] left-[3px] w-[calc(50%-3px)] rounded-[7px] border border-[#7CDB55] bg-[#7CDB55]/[0.14] transition-all duration-200">
                    </div>

                    <!-- Admin -->
                    <button type="button" data-mode="admin"
                        class="mode-btn relative z-10 flex flex-1 items-center justify-center gap-1.5
                        rounded-md border-0 bg-transparent py-[7px]
                        text-[12.5px] font-medium text-[#EAF5E4]">
                        <span class="text-sm text-[#7CDB55]">▦</span>
                        <span>Admin panel</span>
                    </button>

                    <!-- Kiosk -->
                    <a href="{{ route('admin.kiosks.index') }}" data-mode="kiosk"
                        class="mode-btn relative z-10 flex flex-1 items-center justify-center gap-1.5
                        rounded-md border-0 bg-transparent hover:bg-brand-500 hover:text-white py-[7px]
                        text-[12.5px] font-medium text-[#7A837A]">
                    ...
                    <div x-data="{ open: {{ $catalogOpen ? 'true' : 'false' }} }">
                        <button @click="open = !open"
                            ...
                            <i data-lucide="shopping-bag"
                                class="w-5 h-5 flex-shrink-0 {{ $catalogOpen ? 'text-white' : 'text-brand-500' }}"></i>
                            <span x-show="sidebarOpen" x-transition
                                class="whitespace-nowrap flex-1 text-left">Catalog</span>
                        ...
                        <div x-show="open" x-transition:enter="transition ease-out duration-200"
                            ...
                            @foreach ($catalogItems as $item)
                                <a href="{{ route($item['route'], $item['params'] ?? []) }}"
                                    ...
                                    <i data-lucide="{{ $item['icon'] }}"
                                        class="w-4 h-4 flex-shrink-0 {{ request()->routeIs($item['match']) ? 'text-white' : 'text-brand-500' }}"></i>
                                    <span x-show="sidebarOpen" x-transition
                ...
                <div x-data="{ docOpen: {{ $docOpen ? 'true' : 'false' }} }">
                    <button @click="docOpen = !docOpen"
                        ...
                        <i data-lucide="book-open"
                            class="w-5 h-5 flex-shrink-0 {{ $docOpen ? 'text-white' : 'text-brand-500' }}"></i>
                        <span x-show="sidebarOpen" x-transition
                            class="whitespace-nowrap flex-1 text-left">Documentations</span>
                        <i x-show="sidebarOpen" data-lucide="chevron-down"
                            class="w-4 h-4 flex-shrink-0 transition-transform duration-200"
                            :class="docOpen ? 'rotate-180' : ''"></i>
                    </button>

                    <div x-show="docOpen" x-transition:enter="transition ease-out duration-200"
                        x-transition:enter-start="opacity-0 -translate-y-1"
                        x-transition:enter-end="opacity-100 translate-y-0"
                        x-transition:leave="transition ease-in duration-150"
                        x-transition:leave-start="opacity-100 translate-y-0"
                        ...
                        <div x-data="{ custOpen: {{ request()->is('admin/docs/mobile-custom-editing-ordering-guide*') || request()->is('admin/docs/desktop-custom-editing-ordering-guide*') ? 'true' : 'false' }} }">
                            <button @click="custOpen = !custOpen"
                                ...
                                <span x-show="sidebarOpen" x-transition
                                    class="whitespace-nowrap flex-1 text-left">Customer Flow</span>
                                <i x-show="sidebarOpen" data-lucide="chevron-down"
                                    class="w-3.5 h-3.5 flex-shrink-0 transition-transform duration-200"
                                    :class="custOpen ? 'rotate-180' : ''"></i>
                            </button>

                            <div x-show="custOpen"
                                class="ml-3 pl-2 border-l border-surface-700/60 space-y-0.5 mt-0.5">
                                <a href="{{ route('admin.docs.show', 'mobile-custom-editing-ordering-guide') }}"
                                    class="flex items-center gap-2 px-2.5 py-1.5 rounded-lg text-[11px] text-surface-400 hover:bg-surface-800 hover:text-white {{ request()->is('admin/docs/mobile-custom-editing-ordering-guide*') ? 'text-white font-bold bg-surface-800' : '' }}">
                                    <i data-lucide="smartphone"
                                        class="w-3.5 h-3.5 {{ request()->is('admin/docs/mobile-custom-editing-ordering-guide*') ? 'text-white' : 'text-pink-400' }}"></i>
                                    Mobile Ordering
                                </a>
                                <a href="{{ route('admin.docs.show', 'desktop-custom-editing-ordering-guide') }}"
                                    class="flex items-center gap-2 px-2.5 py-1.5 rounded-lg text-[11px] text-surface-400 hover:bg-surface-800 hover:text-white {{ request()->is('admin/docs/desktop-custom-editing-ordering-guide*') ? 'text-white font-bold bg-surface-800' : '' }}">
                                    <i data-lucide="monitor"
                                        class="w-3.5 h-3.5 {{ request()->is('admin/docs/desktop-custom-editing-ordering-guide*') ? 'text-white' : 'text-indigo-400' }}"></i>
                                    Desktop PC Studio
                                </a>
                            </div>
                        </div>
                        ...
                        <div x-data="{ catOpen: {{ request()->is('admin/docs/products*') || request()->is('admin/docs/categories*') || request()->is('admin/docs/card-types*') || request()->is('admin/docs/templates*') || request()->is('admin/docs/coupons*') || request()->is('admin/docs/events*') || request()->is('admin/docs/paper-types*') || request()->is('admin/docs/catalog*') ? 'true' : 'false' }} }">
                            ...
                            <div x-show="catOpen" class="ml-3 pl-2 border-l border-surface-700/60 space-y-0.5 mt-0.5">
                                <a href="{{ route('admin.docs.show', 'products') }}"
                                    class="flex items-center gap-2 px-2.5 py-1.5 rounded-lg text-[11px] text-surface-400 hover:bg-surface-800 hover:text-white {{ request()->is('admin/docs/products*') ? 'text-white font-bold bg-surface-800' : '' }}">
                                    <i data-lucide="box"
                                        class="w-3.5 h-3.5 {{ request()->is('admin/docs/products*') ? 'text-white' : 'text-brand-500' }}"></i>
                                    Products
                                </a>
                                <a href="{{ route('admin.docs.show', 'categories') }}"
                                    class="flex items-center gap-2 px-2.5 py-1.5 rounded-lg text-[11px] text-surface-400 hover:bg-surface-800 hover:text-white {{ request()->is('admin/docs/categories*') ? 'text-white font-bold bg-surface-800' : '' }}">
                                    <i data-lucide="grid-2x2"
                                        class="w-3.5 h-3.5 {{ request()->is('admin/docs/categories*') ? 'text-white' : 'text-brand-500' }}"></i>
                                    Categories
                                </a>
                                @if (auth()->user()->isAdmin())
                                    <a href="{{ route('admin.docs.show', 'card-types-sizes') }}"
                                        class="flex items-center gap-2 px-2.5 py-1.5 rounded-lg text-[11px] text-surface-400 hover:bg-surface-800 hover:text-white {{ request()->is('admin/docs/card-types*') ? 'text-white font-bold bg-surface-800' : '' }}">
                                        <i data-lucide="layers"
                                            class="w-3.5 h-3.5 {{ request()->is('admin/docs/card-types*') ? 'text-white' : 'text-brand-500' }}"></i>
                                        Card Types/Sizes
                                    </a>
                                    <a href="{{ route('admin.docs.show', 'templates') }}"
                                        class="flex items-center gap-2 px-2.5 py-1.5 rounded-lg text-[11px] text-surface-400 hover:bg-surface-800 hover:text-white {{ request()->is('admin/docs/templates*') ? 'text-white font-bold bg-surface-800' : '' }}">
                                        <i data-lucide="layout-template"
                                            class="w-3.5 h-3.5 {{ request()->is('admin/docs/templates*') ? 'text-white' : 'text-brand-500' }}"></i>
                                        Templates
                                    </a>
                                @endif
                                <a href="{{ route('admin.docs.show', 'coupons') }}"
                                    class="flex items-center gap-2 px-2.5 py-1.5 rounded-lg text-[11px] text-surface-400 hover:bg-surface-800 hover:text-white {{ request()->is('admin/docs/coupons*') ? 'text-white font-bold bg-surface-800' : '' }}">
                                    <i data-lucide="tag"
                                        class="w-3.5 h-3.5 {{ request()->is('admin/docs/coupons*') ? 'text-white' : 'text-brand-500' }}"></i>
                                    Coupons
                                </a>
                                <a href="{{ route('admin.docs.show', 'events') }}"
                                    class="flex items-center gap-2 px-2.5 py-1.5 rounded-lg text-[11px] text-surface-400 hover:bg-surface-800 hover:text-white {{ request()->is('admin/docs/events*') ? 'text-white font-bold bg-surface-800' : '' }}">
                                    <i data-lucide="calendar"
                                        class="w-3.5 h-3.5 {{ request()->is('admin/docs/events*') ? 'text-white' : 'text-brand-500' }}"></i>
                                    Events
                                </a>
                                <a href="{{ route('admin.docs.show', 'paper-types') }}"
                                    class="flex items-center gap-2 px-2.5 py-1.5 rounded-lg text-[11px] text-surface-400 hover:bg-surface-800 hover:text-white {{ request()->is('admin/docs/paper-types*') ? 'text-white font-bold bg-surface-800' : '' }}">
                                    <i data-lucide="scroll-text"
                                        class="w-3.5 h-3.5 {{ request()->is('admin/docs/paper-types*') ? 'text-white' : 'text-brand-500' }}"></i>
                                    Paper Types
                                </a>
                            </div>
                        </div>
                        ...
                        @if (auth()->user()->isAdmin() || auth()->user()->isStoreAdmin())
                            <a href="{{ route('admin.docs.show', 'stores') }}"
                                ...
                                <i data-lucide="store"
                                    class="w-4 h-4 flex-shrink-0 {{ request()->is('admin/docs/stores*') ? 'text-white' : 'text-brand-500' }}"></i>
                        ...
                        @if (auth()->user()->isAdmin())
                            <a href="{{ route('admin.docs.show', 'users') }}"
                                ...
                                <i data-lucide="users"
                                    class="w-4 h-4 flex-shrink-0 {{ request()->is('admin/docs/users*') ? 'text-white' : 'text-brand-500' }}"></i>
            ...
            <div class="px-3 py-4 border-t border-surface-800 space-y-1">
                <a href="{{ route('home') }}"
                    class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-surface-400 hover:bg-surface-800 hover:text-white transition">
        ...
        <div :class="sidebarOpen ? 'ml-64' : 'ml-20'" class="flex-1 transition-all duration-300">
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <div class="relative" x-data="{ docMenuOpen: false }">
                            <button @click="docMenuOpen = !docMenuOpen" @click.away="docMenuOpen = false"
                                ...
                                <i data-lucide="chevron-down"
                                    ...
                                    :class="docMenuOpen ? 'rotate-180' : ''"></i>
                            </button>

                            <div x-show="docMenuOpen" x-transition:enter="transition ease-out duration-200"
                        ...
                        @if ($isStoreAdmin && $userStore)
                            ...
                            <div
                                ...
                                @if ($userStore->logo)
                                    <div
                                        class="w-8 h-8 rounded-lg overflow-hidden ring-2 ring-emerald-200 flex-shrink-0">
                                        <img src="{{ asset('storage/' . $userStore->logo) }}"
                                            alt="{{ $userStore->store_name }}" class="w-full h-full object-cover">
                                    </div>
                                @else
                                    <div
                                        class="w-8 h-8 rounded-lg bg-gradient-to-br from-emerald-400 to-teal-600 flex items-center justify-center flex-shrink-0">
                                        <i data-lucide="store" class="w-4 h-4 text-white"></i>
                                    </div>
                                @endif
                                <div class="hidden sm:block">
                                    <p class="text-sm font-semibold text-surface-800 leading-tight">
                                        {{ auth()->user()->name }}</p>
                                    <p class="text-xs  text-gray-600 leading-tight">{{ $userStore->store_name }}</p>
                                </div>
                                <span
                                    class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold  bg-gray-100  text-gray-700 uppercase tracking-wider">Store</span>
                            </div>
                        @else
                            <div class="flex items-center gap-2 px-3 py-2 rounded-xl bg-surface-100">
                                <div
                                    class="w-7 h-7 rounded-lg bg-gradient-to-br from-brand-400 to-brand-600 flex items-center justify-center">
                                    <span
                                        class="text-white text-xs font-bold">{{ substr(auth()->user()->name, 0, 1) }}</span>
                                </div>
            ...
            <main class="p-6">
                @if (session('success'))
                    <div
                        class="mb-6 p-4 bg-accent-50 border border-accent-200 text-accent-800 rounded-xl flex items-center gap-3 text-sm">
                        <i data-lucide="check-circle" class="w-5 h-5 text-accent-600"></i> {{ session('success') }}
                    </div>
                @endif
                @if (session('error'))
                    <div
                        class="mb-6 p-4 bg-red-50 border border-red-200 text-red-800 rounded-xl flex items-center gap-3 text-sm">
