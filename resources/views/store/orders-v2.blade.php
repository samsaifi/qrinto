...
@section('content')
    ...
    <div id="order-error-banner" class="mb-4 px-4 py-3 rounded-xl bg-red-50 border border-red-200 text-red-800 text-sm font-medium hidden"></div>
                ...
                @if ($cards->count())
                    <div class="space-y-3">
                        @foreach ($cards as $order)
                            @include('store.partials.queue-row-v2', ['order' => $order, 'rPrefix' => $rPrefix, 'store' => $store])
                        @endforeach
                    </div>
                @else
                    <div class="v2-empty">
                        @if ($stageKey === 'new') New orders will land here the moment they're placed.
                        @elseif ($stageKey === 'printing') Nothing on the press right now.
                        @else Nothing waiting at the counter.
                        @endif
                    </div>
                @endif
            </section>
    ...
    {{-- Tray picker modal --}}
    <div x-data="printPickerV2()" x-cloak
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <p class="font-bold text-[13px]" :class="t.enabled ? 'text-slate-900' : 'text-slate-400'">
                                            ...
                                            <template x-if="t.recommended">
                                                <span class="ml-2 text-[10px] font-extrabold text-[#287d3c] bg-[#eaf3ea] border border-[#bfdcc4] rounded-full px-2 py-0.5 uppercase tracking-wider">Recommended</span>
                                            </template>
                                        </p>
                                        <p class="mono text-[11px] mt-0.5 truncate" :class="t.enabled ? 'text-slate-500' : 'text-slate-300'" :title="t.printer">
                    ...
                    <div class="pt-3 sticky bottom-0 bg-white pb-1 space-y-2">
                        <div x-show="pcState === 'error' || (pcState === 'ok' && pcPrinters.length === 0)"
                            ...
                            <div class="flex items-center gap-2 min-w-0">
                                ...
                                <p class="text-[12px] text-amber-800 leading-snug">Printer not visible? Install QZ Tray to detect this PC's printers.</p>
                            </div>
...
<style>
    .btn-ghost-v2 {
        ...
        color: #374151; background: #fff; border: 1px solid #e6e9e4;
        transition: all .15s ease;
    }
    .btn-ghost-v2:hover { border-color: #c8d0c3; }

    .v2-count {
        font-size: 12px; color: #6b7280; background: #e3e7de;
        padding: 1px 8px; border-radius: 10px; font-weight: 600;
    }
    .v2-empty {
        border: 1px dashed #e6e9e4; border-radius: 12px;
        padding: 16px 18px; color: #9aa0a6; font-size: 13px; background: #f7f9f5;
    }
    ...
    .v2-card.is-printing { border-color: #cfe6d9; }
    .v2-card.is-done { opacity: 0.7; }
    .v2-card.just-printed { animation: v2-printed-pop .5s cubic-bezier(.34,1.56,.64,1); }
    @keyframes v2-printed-pop {
        0% { transform: scale(0.98); opacity: 0.7; }
        100% { transform: scale(1); opacity: 1; }
    }

    .v2-col-id { width: 104px; flex-shrink: 0; font-size: 11px; color: #9aa0a6; line-height: 1.4; font-variant-numeric: tabular-nums; }
    .v2-col-main { flex: 1; min-width: 0; }
    .v2-col-main .v2-title { font-size: 15px; font-weight: 700; margin: 0 0 3px; color: #14171a; }
    .v2-col-main .v2-sub { font-size: 12.5px; color: #6b7280; margin: 0; }

    .v2-journey { display: flex; align-items: center; margin-top: 10px; gap: 0; }
    .v2-j-step { display: flex; align-items: center; gap: 6px; }
    .v2-j-dot {
        width: 16px; height: 16px; border-radius: 50%;
        border: 2px solid #e7ebe3; background: #fff;
        display: flex; align-items: center; justify-content: center;
        flex-shrink: 0; transition: all .3s ease;
    }
    .v2-j-dot svg { width: 9px; height: 9px; }
    .v2-j-step.done .v2-j-dot { background: #1c7a43; border-color: #1c7a43; }
    .v2-j-step.done .v2-j-dot svg { stroke: #fff; }
    .v2-j-step.current .v2-j-dot { border-color: #1c7a43; background: #fff; position: relative; }
    .v2-j-step.current .v2-j-dot::after { content: ''; width: 6px; height: 6px; border-radius: 50%; background: #1c7a43; }
    .v2-j-label { font-size: 10.5px; color: #9aa0a6; white-space: nowrap; }
    .v2-j-step.done .v2-j-label { color: #155f34; }
    .v2-j-step.current .v2-j-label { color: #14171a; font-weight: 700; }
    .v2-j-line { width: 22px; height: 2px; background: #e7ebe3; margin: 0 3px; }
    .v2-j-line.done { background: #1c7a43; }

    .v2-col-price { flex-shrink: 0; }
    .v2-pill {
        background: #fbf3d9; border: 1px solid #efdfa2; color: #8a6a15;
    ...
    .v2-col-action {
        ...
        align-items: flex-end; gap: 6px; min-width: 150px;
    }
    .v2-btn-primary {
        background: #1c7a43; color: #fff; border: none; border-radius: 9px;
        padding: 9px 16px; font-size: 13.5px; font-weight: 700; cursor: pointer;
        display: inline-flex; align-items: center; gap: 7px; white-space: nowrap;
        transition: background .15s ease;
    }
    .v2-btn-primary:hover { background: #155f34; }
    .v2-btn-primary svg { width: 15px; height: 15px; }
    .v2-btn-neutral {
        background: #fff; color: #374151; border: 1px solid #e5e7eb; border-radius: 9px;
        padding: 9px 16px; font-size: 13.5px; font-weight: 700; cursor: pointer;
        display: inline-flex; align-items: center; gap: 7px; white-space: nowrap;
    }
    .v2-btn-neutral:hover { background: #f9fafb; }
    .v2-btn-done {
        background: #c9d3cc; color: #5a625c; border: none; border-radius: 9px;
        padding: 9px 16px; font-size: 13.5px; font-weight: 700;
        display: inline-flex; align-items: center; gap: 7px; cursor: default;
    }

    .v2-link-row { display: flex; gap: 10px; align-items: center; }
    .v2-link {
        font-size: 12px; color: #6b7280; text-decoration: none; cursor: pointer;
        display: inline-flex; align-items: center; gap: 4px; transition: color .15s;
    }
    .v2-link:hover { color: #14171a; }
    .v2-undo {
        font-size: 12px; color: #9aa0a6; text-decoration: underline; cursor: pointer;
        background: none; border: none; padding: 0;
    }
    .v2-undo:hover { color: #6b7280; }

    .v2-print-widget { display: flex; align-items: center; gap: 10px; }
    .v2-ring-wrap { position: relative; width: 46px; height: 46px; flex-shrink: 0; }
    .v2-ring-wrap > svg { transform: rotate(-90deg); }
    .v2-ring-bg { fill: none; stroke: #e7ebe3; stroke-width: 4; }
    .v2-ring-fg { fill: none; stroke: #1c7a43; stroke-width: 4; stroke-linecap: round; transition: stroke-dashoffset .3s linear; }
    .v2-ring-icon {
        position: absolute; top: 0; left: 0; width: 100%; height: 100%;
        display: flex; align-items: center; justify-content: center;
    }
    .v2-ring-icon svg { width: 16px; height: 16px; stroke: #155f34; transform: none; }
    .v2-print-text { display: flex; flex-direction: column; gap: 1px; }
    .v2-print-text .p-label { font-size: 12.5px; font-weight: 700; color: #155f34; }
    .v2-print-text .p-time { font-size: 11.5px; color: #9aa0a6; }

    .v2-check-pop { display: flex; align-items: center; gap: 8px; }
    .v2-check-circle {
        width: 46px; height: 46px; border-radius: 50%; background: #dcf3e4;
        display: flex; align-items: center; justify-content: center;
        animation: v2-pop .35s cubic-bezier(.34,1.56,.64,1);
    }
    @keyframes v2-pop { 0% { transform: scale(0.5); opacity: 0; } 100% { transform: scale(1); opacity: 1; } }
    .v2-check-circle svg { width: 22px; height: 22px; stroke: #155f34; }
    .v2-check-label { font-size: 12.5px; font-weight: 700; color: #155f34; }
    ...
    @media (max-width: 720px) {
        .v2-card { flex-wrap: wrap; }
        .v2-col-action { align-items: flex-start; }
    }
</style>
@endpush
...
<script>
    ...
    const V2_LABELS = { new: 'New', printing: 'Printing', ready: 'Ready', done: 'Picked up' };
    ...
    function svgPrinter(size) {
        return '<svg width="'+size+'" height="'+size+'" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M6 9V3h12v6"/><rect x="4" y="9" width="16" height="9" rx="1"/><path d="M6 14h12v7H6z"/></svg>';
    }
    ...
    function printPickerV2() {
        return {
            visible: false, loading: false, sending: false,
            payload: null, choice: null, error: '',
            pcState: 'idle', pcPrinters: [], defaultPrinter: null,

            async open(prepareUrl) {
                this.visible = true; this.loading = true;
                this.payload = null; this.choice = null; this.error = '';
                try {
                    const res = await fetch(prepareUrl, { credentials: 'same-origin', headers: { 'Accept': 'application/json' } });
                    const data = await res.json();
                    if (!res.ok || !data.ok) { this.error = data.error || 'Could not prepare this order.'; this.loading = false; return; }
                    this.payload = data;
                    const first = data.matched_key || data.trays.find(t => t.enabled)?.key;
                    this.choice = first ? ('tray:' + first) : null;
                } catch (e) { this.error = 'Network error while preparing the order.'; }
                finally { this.loading = false; this.$nextTick(() => { if (window.lucide) lucide.createIcons(); }); }
                this.scanPc();
            },
            ...
            chosenTarget() {
                ...
                if (this.choice.startsWith('tray:')) {
                    ...
                    if (!t) return null;
                    return { printer: t.printer || 'Noritsu 931BL', label: t.label, tray_key: t.key, size: t.size, media: t.media, gsm: t.gsm, user_type: t.user_type };
                }
                return null;
            },
            ...
            async confirm() {
                ...
                if (ok) {
                    ...
                    if (card) { stopPrintingAnimation(orderId); showCardError(card, orderId); }
                    showBannerError('Print failed for ' + (orderNumber || 'order') + '. Check that QZ Tray is running and the printer is online.');
                }
            },
        };
    }
    ...
    function startPrintingAnimation(card, orderId) {
            ...
            '<div class="v2-print-widget" id="v2-pw-'+orderId+'">' +
                '<div class="v2-ring-wrap">' +
                    '<svg width="46" height="46" viewBox="0 0 46 46">' +
                        ...
                        '<circle class="v2-ring-fg" cx="23" cy="23" r="'+r+'" stroke-dasharray="'+c+'" stroke-dashoffset="'+c+'" id="v2-ring-'+orderId+'"/>' +
                    '</svg>' +
        ...
        const startTime = Date.now(), duration = 8000;
    ...
    function stopPrintingAnimation(orderId) {
        if (printingOrders[orderId]) { clearInterval(printingOrders[orderId]); delete printingOrders[orderId]; }
    }
    ...
    function showBannerError(msg) {
        ...
        if (banner) { banner.textContent = msg; banner.classList.remove('hidden'); setTimeout(() => banner.classList.add('hidden'), 10000); }
    }
    ...
    function buildJourneyHTML(currentStage) {
        ...
        return V2_STAGES.map((s, i) => {
            ...
            return line + '<div class="v2-j-step ' + state + '"><div class="v2-j-dot">' + dotInner + '</div><span class="v2-j-label">' + V2_LABELS[s] + '</span></div>';
        }).join('');
    }
