                    ...
                    <div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left w-full">
                        ...
                        <div class="flex items-center gap-4 p-3 bg-gray-50 rounded-xl mb-6">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-gray-500">Base Price: $<span x-text="basePrice"></span></p>
                            </div>
                        </div>
                                ...
                                @if($group->type === 'select')
                                    <select x-model="selectedOptions['{{ $group->id }}']" 
                                        ...
                                        @foreach($group->values as $value)
                                            <option value="{{ $value->id }}" data-price="{{ $value->price_modifier }}" data-type="{{ $value->price_type }}">
                                                {{ $value->label }} 
                                        ...
                                        @foreach($group->values as $value)
                                            <label class="cursor-pointer">
                                                <input type="radio" 
                                                       ...
                                                       @change="calculateTotal"
                                                       class="peer sr-only">
                                                <div class="px-4 py-2 bg-white border border-gray-200 rounded-lg peer-checked:border-brand-500 peer-checked:bg-brand-50 peer-checked:text-brand-700 hover:border-brand-300 transition-all font-medium text-sm flex items-center justify-center min-w-min">
                                                    @if($value->label)
                                                        <span>{{ $value->label }}</span>
                                                    @endif
                                                    @if($value->price_modifier > 0)
                                                        <span class="text-xs text-gray-500 ml-1">(+${{ number_format($value->price_modifier, 2) }})</span>
                                                    @endif
                                                </div>
                                            </label>
                                        @endforeach
                        ...
                        <div class="flex items-center justify-between p-4 bg-gray-50 rounded-xl border border-gray-100">
                            ...
                            <div class="text-right">
                                ...
                                <span class="text-2xl font-black text-brand-600">$<span x-text="totalPrice"></span></span>
                            </div>
                        </div>
