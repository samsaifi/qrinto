...
@section('meta_description', 'Customize your ' . $product->name . '. Upload photos, select size, material, and frame
    options. See live pricing.')
        ...
        <div class="grid lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2 space-y-6">
                    ...
                    <template x-for="(stepLabel, index) in stepLabels" :key="index">
                        <div class="flex items-center gap-2">
                            <button @click="goToStep(index)"
                                :class="currentStep === index ? 'bg-brand-600 text-white shadow-lg shadow-brand-200' : (
                                    currentStep > index ? 'bg-accent-500 text-white' :
                            ...
                            <span class="text-xs font-medium hidden sm:inline"
                                :class="currentStep === index ? 'text-brand-600' : 'text-surface-400'"
                                x-text="stepLabel"></span>
                            <template x-if="index < stepLabels.length - 1">
                                <div class="w-8 h-0.5 bg-surface-200 hidden sm:block">
                ...
                <div x-show="currentStep === 0" x-transition
                    class="bg-white rounded-2xl border border-surface-100 shadow-card p-6 lg:p-8">
                    <div class="flex items-center gap-3 mb-6">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-surface-500">Upload <span class="font-medium"
                                    x-text="minImages + '-' + maxImages"></span> high quality photos (JPEG, PNG, WebP)</p>
                        </div>
                    </div>
                    ...
                    <div class="border-2 border-dashed rounded-2xl p-8 text-center transition-colors"
                        ...
                        <label
                            class="inline-flex items-center gap-2 px-6 py-3 bg-brand-600 text-white font-semibold rounded-xl hover:bg-brand-700 cursor-pointer transition shadow-lg shadow-brand-200">
                    ...
                    <div x-show="uploadedImages.length > 0" class="mt-6">
                        <div class="flex justify-between items-center mb-3">
                            <h3 class="text-sm font-semibold text-surface-700">Uploaded Photos (<span
                                    x-text="uploadedImages.length"></span>/<span x-text="maxImages"></span>)</h3>
                        </div>
                        <div class="grid grid-cols-3 sm:grid-cols-4 gap-3">
                            <template x-for="(img, index) in uploadedImages" :key="img.id">
                                <div
                                    class="relative group aspect-square rounded-xl overflow-hidden border border-surface-200 shadow-sm">
                    ...
                    <div x-show="uploadError"
                        class="mt-4 p-3 bg-red-50 border border-red-200 rounded-xl text-sm text-red-600 flex items-center gap-2">
                ...
                @foreach ($product->optionGroups as $groupIndex => $group)
                    <div x-show="currentStep === {{ $groupIndex + 1 }}" x-transition
                        class="bg-white rounded-2xl border border-surface-100 shadow-card p-6 lg:p-8">
                        <div class="flex items-center gap-3 mb-6">
                            <div class="w-10 h-10 rounded-xl bg-brand-100 flex items-center justify-center">
                            ...
                            <div>
                                <h2 class="font-display font-semibold text-xl text-surface-900">Select {{ $group->name }}
                                </h2>
                        ...
                        @if ($group->display_type === 'buttons')
                            <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                                @foreach ($group->values->where('is_active', true) as $value)
                                    <button
                                        @click="selectOption('{{ $group->slug }}', {{ $value->id }}, '{{ $value->label }}')"
                                        :class="selectedOptions['{{ $group->slug }}'] === {{ $value->id }} ?
                                            'ring-2 ring-brand-500 bg-brand-50 border-brand-200' :
                                            'bg-white border-surface-200 hover:border-brand-300 hover:bg-brand-50/50'"
                                        class="relative p-4 rounded-xl border text-left transition-all group">
                                        <span
                                            class="text-sm font-semibold text-surface-800 block">{{ $value->label }}</span>
                                        @if ($value->description)
                                            <span
                                                class="text-xs text-surface-500 mt-1 block">{{ $value->description }}</span>
                                        @endif
                                        <span
                                            class="text-xs font-medium mt-2 block {{ $value->price_modifier == 0 ? 'text-accent-600' : 'text-brand-600' }}">
                                            {{ $value->formatted_price_modifier }}
                                        </span>
                                        <div x-show="selectedOptions['{{ $group->slug }}'] === {{ $value->id }}"
                                            class="absolute top-2 right-2 w-5 h-5 bg-brand-600 text-white rounded-full flex items-center justify-center text-xs">
                                            ✓</div>
                                    </button>
                                @endforeach
                            </div>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                @foreach ($group->values->where('is_active', true) as $value)
                                    <button
                                        ...
                                        @if ($value->image)
                                            <img src="{{ asset('storage/' . $value->image) }}" alt="{{ $value->label }}"
                                                class="w-12 h-12 rounded-lg object-cover mb-3">
                                        @endif
                                        <span
                                            class="text-base font-semibold text-surface-800 block">{{ $value->label }}</span>
                                        @if ($value->description)
                                            <span
                                                class="text-sm text-surface-500 mt-1 block">{{ $value->description }}</span>
                                        @endif
                                        <span
                                            class="text-sm font-bold mt-3 block {{ $value->price_modifier == 0 ? 'text-accent-600' : 'text-brand-600' }}">
                                            {{ $value->formatted_price_modifier }}
                                        </span>
                                    </button>
                                @endforeach
                        ...
                        @else
                            <select
                                @change="selectOption('{{ $group->slug }}', parseInt($event.target.value), $event.target.options[$event.target.selectedIndex].text)"
                                ...
                                @foreach ($group->values->where('is_active', true) as $value)
                                    <option value="{{ $value->id }}">{{ $value->label }}
                                        ({{ $value->formatted_price_modifier }})</option>
                                @endforeach
                            </select>
                        @endif
                    </div>
                ...
                <!-- Last Step: Quantity -->
                <div x-show="currentStep === totalSteps - 1" x-transition
                    class="bg-white rounded-2xl border border-surface-100 shadow-card p-6 lg:p-8">
                    <div class="flex items-center gap-3 mb-6">
                        <div class="w-10 h-10 rounded-xl bg-brand-100 flex items-center justify-center">
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <input type="number" x-model.number="quantity" @change="calculatePrice()" min="1"
                            max="100"
                ...
                <div class="flex justify-between items-center pt-4">
                    ...
                    <div></div>
                    <button @click="nextStep()" x-show="currentStep < totalSteps - 1" :disabled="!canProceed()"
                        :class="canProceed() ? 'bg-brand-600 hover:bg-brand-700 shadow-lg shadow-brand-200' :
                    ...
                    <button @click="addToCart()" x-show="currentStep === totalSteps - 1" :disabled="!canAddToCart()"
                        :class="canAddToCart() ?
                            'bg-gradient-to-r from-brand-600 to-brand-700 hover:from-brand-700 hover:to-brand-800 shadow-xl shadow-brand-200' :
                            'bg-surface-300 cursor-not-allowed'"
                        ...
                        <i data-lucide="shopping-cart" class="w-5 h-5"></i> Add to Cart - <span
                            x-text="formattedTotalPrice"></span>
                    </button>
                </div>
            </div>
            ...
            <div class="lg:col-span-1">
                <div class="sticky top-28 space-y-6">
                    ...
                    <div class="bg-white rounded-2xl border border-surface-100 shadow-card p-6">
                        ...
                        <div class="space-y-3 mb-6">
                            ...
                            <div class="flex justify-between text-sm">
                                ...
                                <span class="text-surface-800 font-medium"
                                    x-text="uploadedImages.length + ' uploaded'"></span>
                            </div>
                        ...
                        <div class="border-t border-surface-100 pt-4">
                            <div class="flex justify-between items-center">
                                ...
                                <span class="text-lg font-semibold text-surface-800"
                                    x-text="formattedUnitPrice">{{ \App\Services\CurrencyService::format($product->base_price, 0) }}</span>
                            </div>
                            <div class="flex justify-between items-center mt-2">
                                <span class="font-display font-bold text-surface-900">Total</span>
                                <span class="text-2xl font-bold text-brand-600"
                                    x-text="formattedTotalPrice">{{ \App\Services\CurrencyService::format($product->base_price, 0) }}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-surface-50 rounded-2xl p-5 space-y-3">
                        @foreach ([['icon' => 'lock', 'text' => 'Secure checkout & payment'], ['icon' => 'award', 'text' => 'Premium HD quality printing'], ['icon' => 'package', 'text' => 'Careful packaging & delivery'], ['icon' => 'refresh-cw', 'text' => 'Easy returns within 7 days']] as $trust)
