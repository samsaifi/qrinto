...
@section('content')
    <style>
        ...
        .price-current text-muted text-xs text-xs {
            color: #0ea5e9;
            font-weight: 700;
            font-size: 13px;
        }
    ...
    <div class="category-header px-6">
        <a href="javascript:history.back()"
            class="w-10 h-10 rounded-full bg-white border border-slate-200 shadow-sm flex items-center justify-center text-slate-500 hover:bg-slate-50 transition-colors shrink-0">
        ...
        <div>
            <h1 style="font-size: 24px; font-weight: 800; color: #334155; margin: 0;">{{ $type->name }}</h1>
            <p style="font-size: 15px; color: #0ea5e9; font-weight: 500; margin: 0;">Choose your size</p>
        </div>
    </div>
                ...
                <div class="size-group">
                    <div class="size-group-head">
                        ...
                        @if ($first->width && $first->height)
                            <span class="size-group-dim">{{ $first->width }} × {{ $first->height }}
                                {{ $first->unit }}</span>
                        @endif
                    </div>
                            ...
                            <div class="variant-body">
                                ...
                                @if ($sub->price)
                                    <div class="variant-price">
                                        <span class="price-current text-muted text-xs">Starting at
                                            {{ \App\Services\CurrencyService::format($sub->price) }}</span>
                                        @if ($sub->old_price)
                                            <span
                                                class="price-old">{{ \App\Services\CurrencyService::format($sub->old_price) }}</span>
                                        @endif
                                    </div>
                                @endif
                            </div>
