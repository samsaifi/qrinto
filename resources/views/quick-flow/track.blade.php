...
@section('content')
    <div class="space-y-6 pb-24 px-6 text-center pb-36 min-h-screen pt-4">
        ...
        <div class="flex justify-between items-start text-left mb-6">
            ...
            <div>
                <span
                    class="inline-flex items-center gap-1.5 font-bold text-blue-600 text-xs bg-blue-50 px-3 py-1.5 rounded-xl border border-blue-100">
        ...
        <div class="bg-white border-2 border-slate-50 rounded-[2rem] p-5 shadow-premium text-left mt-6">
            ...
            <div class="flex justify-between items-center py-2 border-b border-slate-50">
                ...
                <span
                    class="font-bold text-slate-900 text-sm">{{ $order->items->first()?->product_name ?? 'Custom Print' }}</span>
            </div>
            <div class="flex justify-between items-center py-2 border-b border-slate-50">
            ...
            <div class="flex justify-between items-center py-2 pt-3">
                ...
                <span
                    class="font-black text-mobile-600 text-[17px]">{{ \App\Services\CurrencyService::formatWithCurrency($order->total, $order->currency) }}</span>
            </div>
        </div>

        <!-- Sticky Bottom Create New -->
        <div
            class="fixed bottom-0 left-0 right-0 max-w-md mx-auto p-5 glass border-t border-slate-100 safe-bottom z-50 flex gap-3">
            <a href="{{ route('flow.index') }}"
                class="flex-1 bg-mobile-500 hover:bg-mobile-600 text-white font-extrabold py-4 rounded-2xl shadow-xl transition-all active:scale-[0.97] flex items-center justify-center gap-2 text-base">
