...
@section('content')
<div class="space-y-6 pb-24 text-center min-h-screen pt-4" x-data="storeAutocomplete()">

    @if(!session()->has('active_store_id'))
    <div class="mb-8 p-4 bg-red-50 border-2 border-red-100 rounded-[2rem] flex items-center gap-4 text-left animate-pulse">
        ...
        <div class="flex-1">
            ...
            <p class="text-[11px] font-bold text-red-600 leading-tight">You haven't selected a store yet. Please choose a branch below to continue.</p>
        </div>
    </div>
    @else
        @php
        ...
        @endphp
        @if($selectedStore)
        <div class="mb-8 p-5 bg-emerald-50 border-2 border-emerald-100 rounded-[2rem] flex items-center gap-4 text-left">
            <div class="w-14 h-14 bg-emerald-100 rounded-2xl flex items-center justify-center text-emerald-600 flex-shrink-0">
                <i data-lucide="check-circle" class="w-7 h-7"></i>
            </div>
            <div class="flex-1 overflow-hidden">
                <p class="text-[10px] font-black text-emerald-600 uppercase tracking-widest mb-1">Currently Selected Branch</p>
                <h3 class="font-black text-slate-900 text-lg leading-tight truncate">{{ $selectedStore->store_name }}</h3>
                <p class="text-[11px] font-bold text-slate-500 mt-1 flex items-center gap-1">
                    <i data-lucide="navigation" class="w-3 h-3"></i> {{ $selectedStore->city }}, {{ $selectedStore->state }} {{ $selectedStore->zip_code }}
                </p>
    ...
    <div class="mb-8 text-left bg-white border-2 border-slate-100 p-6 rounded-[2rem] shadow-sm" data-tour="nearby-stores" x-show="geolocationChecked || nearbyLoading" x-cloak>
        ...
        <div x-show="nearbyLoading" class="py-4 text-center">
            <div class="w-6 h-6 border-2 border-slate-200 border-t-brand-500 rounded-full animate-spin mx-auto mb-2"></div>
        ...
        <div x-show="!nearbyLoading && nearbyStores.length > 0" class="space-y-3">
            ...
            <template x-for="store in nearbyStores" :key="store.id">
                <form action="{{ route('flow.set-store') }}" method="POST">
                    ...
                    <button type="submit" class="w-full text-left bg-white border-2 border-slate-100 p-4 rounded-2xl shadow-sm hover:border-brand-500 hover:shadow-md transition-all group flex items-start gap-4 active:scale-[0.98]">
                        ...
                        <div class="flex-1">
                            ...
                            <p class="text-slate-500 text-xs mb-1 line-clamp-1 flex items-center gap-1">
                                <i data-lucide="navigation" class="w-3 h-3"></i> <span x-text="`${store.city || ''}, ${store.state || ''} ${store.zip_code || ''}`"></span>
                            </p>
                            <div class="flex flex-wrap gap-2 mt-2">
                                <span x-show="store.phone" class="inline-flex items-center gap-1 bg-slate-50 text-slate-600 text-[10px] font-bold px-2 py-1 rounded-md border border-slate-100">
                                    <i data-lucide="phone" class="w-3 h-3"></i> <span x-text="store.phone"></span>
                                </span>
                                <span x-show="store.distance !== undefined && store.distance !== null" class="inline-flex items-center gap-1 bg-blue-50 text-blue-600 text-[10px] font-bold px-2 py-1 rounded-md border border-blue-100">
                                    <i data-lucide="map" class="w-3 h-3"></i> <span x-text="Number(store.distance).toFixed(1) + ' miles away'"></span>
                                </span>
                            </div>
                        </div>
                        <div class="flex items-center justify-center h-10 w-6 text-slate-300 group-hover:text-brand-500 transition-colors">
        ...
        <div x-show="!nearbyLoading && nearbyError && nearbyStores.length === 0" class="py-2 text-center">
            <p class="text-amber-600 font-bold text-xs flex items-center justify-center gap-1.5">
                <i data-lucide="alert-circle" class="w-4 h-4"></i> Nearby stores are not available. Please use the search box to find a store.
            </p>
        </div>
    </div>
    ...
    <p class="text-slate-500 text-sm mb-6 px-4">Search by name, city, zip code, or any store details to select your printing location.</p>
    ...
    <div x-show="!hasSearched && stores.length === 0"
        ...
        <h3 class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-6 text-center">Customer Guide: How to Order</h3>

        <div class="grid grid-cols-1 gap-4">
            <!-- Step 1 -->
            <div class="bg-white p-5 rounded-[24px] border border-slate-100 shadow-sm flex gap-4 items-center group hover:border-brand-200 transition-all hover:shadow-md">
                <div class="w-12 h-12 bg-brand-50 rounded-2xl flex items-center justify-center flex-shrink-0 text-brand-600 group-hover:bg-brand-100 transition-colors">
                    <i data-lucide="search" class="w-6 h-6"></i>
                </div>
                <div>
                    <h4 class="font-black text-slate-900 text-sm mb-0.5">1. Select a Store near you</h4>
                    <p class="text-slate-500 text-xs leading-tight font-medium">Search and pick the nearest Qrinto branch to see their services.</p>
                </div>
            </div>
            ...
            <div class="bg-white p-5 rounded-[24px] border border-slate-100 shadow-sm flex gap-4 items-center group hover:border-brand-200 transition-all hover:shadow-md">
                ...
                <div>
                    ...
                    <p class="text-slate-500 text-xs leading-tight font-medium">Browse through categories like Photo Prints, Gifts, or Cards.</p>
                </div>
            </div>
            ...
            <div class="bg-white p-5 rounded-[24px] border border-slate-100 shadow-sm flex gap-4 items-center group hover:border-brand-200 transition-all hover:shadow-md">
                ...
                <div>
                    ...
                    <p class="text-slate-500 text-xs leading-tight font-medium">Upload your photos and use our editor to customize your design.</p>
                </div>
            </div>
            ...
            <div class="bg-white p-5 rounded-[24px] border border-slate-100 shadow-sm flex gap-4 items-center group hover:border-brand-200 transition-all hover:shadow-md">
                ...
                <div>
                    ...
                    <p class="text-slate-500 text-xs leading-tight font-medium">Checkout securely and collect your order from your chosen store.</p>
                </div>
            </div>
        </div>
    ...
    <div x-show="hasSearched || stores.length > 0" x-cloak class="mt-8 text-left transition-all">
        <h3 class="text-xs font-black text-slate-400 uppercase tracking-widest mb-4 border-b border-slate-100 pb-2">Search Results</h3>

        <div x-show="!isLoading && stores.length === 0 && query.length > 0" class="py-8 text-center bg-slate-50 rounded-2xl border-2 border-slate-100 border-dashed">
        ...
        <div x-show="stores.length > 0" class="space-y-3">
            <template x-for="store in stores" :key="store.id">
                <form action="{{ route('flow.set-store') }}" method="POST">
                    ...
                    <input type="hidden" name="store_id" :value="store.id">
                    <button type="submit" class="w-full text-left bg-white border-2 border-slate-100 p-4 rounded-2xl shadow-sm hover:border-brand-500 hover:shadow-md transition-all group flex items-start gap-4 active:scale-[0.98]">
                        <div class="w-10 h-10 bg-brand-50 rounded-xl flex items-center justify-center flex-shrink-0 overflow-hidden group-hover:bg-brand-100 transition-colors">
