{{-- Canvas Customizer - Dynamic aspect ratio from flowData, white background, no masking --}}
    ...
    <script>
        const customizer = {
            ...
            init() {
                ...
                // 2. Set Active Canvas (first one)
                this.activeCanvas = Object.keys(this.imageTypes)[0];
                ...
                // 7. Build template category filter + chips
            ...
            updateUI() {
                ...
                const hasImg = cv && cv.fabricCanvas.getObjects().some(o => o._isUserImage);
                ...
                if (zoomCtrl) zoomCtrl.classList.toggle('hidden', !hasImg);
            ...
            _initAllCanvases() {
                ...
                Object.keys(this.imageTypes).forEach(key => {
                    ...
                    // Dynamic canvas dimensions from flowData (session)
                    const sizeW = <?php echo (float) ($flowData['size_width'] ?? 4); ?>;
                    const sizeH = <?php echo (float) ($flowData['size_height'] ?? 6); ?>;
                    const isPortrait = <?php echo ($product->pdf_orientation ?? 'portrait') === 'portrait' ? 'true' : 'false'; ?>;
                    const adminW = Math.round((isPortrait ? sizeW : sizeH) * 100);
                    const adminH = Math.round((isPortrait ? sizeH : sizeW) * 100);
            ...
            handleRemove() {
                ...
                if (this.selectedObject) {
                    ...
                    // Update image tracking if an image was removed
                    if (wasImage) {
            ...
            submitAllCanvases() {
                ...
                if (!hasAnyContent) {
                    ...
                    btn.innerHTML =
                        '<i data-lucide="alert-circle" class="w-3.5 h-3.5"></i><span>Please add a photo or text first</span>';
                    if (window.lucide) lucide.createIcons();
                    setTimeout(() => {
                        btn.classList.remove('!bg-red-500');
                        btn.innerHTML =
                            '<i data-lucide="shopping-cart" class="w-3.5 h-3.5"></i><span>Confirm & Continue</span>';
                        if (window.lucide) lucide.createIcons();
                    }, 2500);
                    return;
                }
            ...
            clearAll() {
                ...
                Object.keys(this.canvases).forEach(key => {
                    ...
                    // Filter and remove user objects
                    objects.forEach(o => {
            ...
            _positionTextActionIcons() {
                ...
                const iconGroupWidth = 30; // One button + borders/padding/gap (only edit icon)
            ...
            async applyTemplate(id) {
                ...
                for (const key of keys) {
                    ...
                    for (const spec of (tpl.images || [])) {
                        await this._addTemplateImage(fc, spec, sf, key);
                    }
                    for (const spec of (tpl.svgs || [])) {
                        await this._addTemplateSvg(fc, spec, sf, key);
                    }
                ...
                if (activeCv?.fabricCanvas) activeCv.fabricCanvas.discardActiveObject().renderAll();
                this.selectedObject = null;
                this._hideTextActionIcons();
                this.updateUI();
            },
