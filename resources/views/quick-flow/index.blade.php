...
@section('content')
    <style>
        ...
        .flow-card:hover {
            border-color: #0ea5e9 box-shadow: 0 10px 30px rgba(111, 186, 59, 0.1);
    ...
    <div class="px-5 pt-5 pb-4  ">
        ...
        {{-- Store pill badge --}}
        <div
            class="inline-flex items-center gap-2 bg-mobile-100 border border-mobile-200 rounded-full pl-1.5 pr-3 py-1 mb-5">
            <img src="{{ asset('/storage/') . '/' . $activeStore->logo ?? asset('images/default-logo.png') }}"
                alt="{{ $activeStore->store_name }}" class="w-6 h-6 rounded-full object-cover border border-slate-200">
            <span class="text-[13px] font-medium text-mobile-800">{{ $activeStore->store_name }}</span>
        </div>

        {{-- Headline --}}
        <h1 class="text-[28px] font-black leading-tight text-black mb-1.5">
            What would you like to<br>
            <span class="text-mobile-500">create?</span>
        </h1>
    ...
    <div class="selection-grid px-6" data-tour="product-grid">
