...
@section('content')
    <div class="space-y-6" x-data="templateBookmarks()">
            ...
            @if ($type->price)
                <div class="text-right">
                    <div class="flex items-center gap-2 justify-end">
                        ...
                        @endif
                        <span
                            class="text-2xl font-black text-slate-900 font-display">{{ \App\Services\CurrencyService::format($type->price) }}</span>
                    </div>
                    <p
                        class="text-[10px] font-bold text-brand-600 uppercase tracking-widest bg-brand-50 px-2 py-0.5 rounded-full inline-block mt-1">
                        Starting Price</p>
                </div>
            @endif
        </div>
            ...
            @foreach ($categories as $cat)
                ...
                @if ($catProductCount > 0)
                    <button @click="activeCategory = {{ $cat->id }}"
                        :class="activeCategory === {{ $cat->id }} ? 'bg-brand-500 text-white border-brand-500' :
                            'bg-white text-slate-600 border-slate-200 hover:bg-slate-50'"
                        class="px-3 py-1 rounded-full border text-xs font-bold whitespace-nowrap transition-colors">
            ...
            @forelse($templates as $tpl)
                <a href="{{ route('flow.customize', $tpl->slug) }}" data-tpl-id="{{ $tpl->id }}"
                    ...
                    <button type="button" @click.prevent.stop="toggleBookmark($el)" data-tpl-id="{{ $tpl->id }}"
                        ...
                        :title="isBookmarked({{ $tpl->id }}) ? 'Remove bookmark' : 'Bookmark this template'">
                        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24"
                            stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"
                            :class="isBookmarked({{ $tpl->id }}) ? 'stroke-brand-500 fill-brand-500' :
