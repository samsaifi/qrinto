...
@section('content')
    <div class="space-y-6 pb-24 text-center min-h-screen pt-4 px-6" x-data="qrintoFlow()">
        ...
        <div class="flex justify-center items-center gap-2 mb-8">
            <template x-for="i in 3">
                <div class="h-1.5 rounded-full transition-all duration-500"
                    :class="currentStep >= i ? 'w-8 bg-mobile-600' : 'w-4 bg-slate-200'"></div>
            </template>
        </div>
        ...
        <div x-show="currentStep === 1" x-transition:enter="transition ease-out duration-300" class="space-y-6">
            <div
                ...
                <i data-lucide="upload-cloud" class="w-8 h-8 text-mobile-600"></i>
            </div>

            <h1 class="text-2xl font-black text-slate-900 mb-2">Upload Your Design</h1>
            <p class="text-slate-500 text-sm mb-8 px-4">Upload your print-ready file (JPG, PNG, or WebP). We'll print it
            ...
            <div class="relative group" @dragover.prevent="isDragging = true" @dragleave.prevent="isDragging = false"
                ...
                <div @click="$refs.fileInput.click()"
                    :class="[
                        isDragging ? 'border-mobile-500 bg-mobile-50 scale-[1.02]' : 'border-slate-200 bg-white',
                        previewUrl ? 'p-0 border-solid overflow-hidden' : 'p-12 border-dashed'
                    ]"
                    class="border-2 rounded-[32px] transition-all cursor-pointer hover:border-mobile-400 relative min-h-[300px] flex items-center justify-center">

                    <template x-if="!previewUrl">
                        <div class="space-y-4">
                            <div
                                class="w-12 h-12 bg-slate-50 rounded-2xl flex items-center justify-center mx-auto text-slate-400 group-hover:text-mobile-500 transition-colors">
                    ...
                    <template x-if="previewUrl">
                        <div class="w-full h-full">
                            ...
                            <div class="absolute top-4 right-4 w-10 h-10 bg-white/90 backdrop-blur shadow-xl text-red-500 rounded-full flex items-center justify-center transition-transform hover:scale-110 active:scale-95 z-20"
                ...
                <div x-show="isUploading"
                    class="absolute inset-0 bg-white/90 backdrop-blur-sm rounded-[32px] flex flex-col items-center justify-center z-10">
                    <div class="w-12 h-12 border-4 border-slate-100 border-t-mobile-600 rounded-full animate-spin mb-4">
                    </div>
            ...
            <div class="pt-8">
                <button @click="currentStep = 2" :disabled="!uploadId"
                    :class="!uploadId ? 'bg-slate-100 text-slate-400 cursor-not-allowed' :
                        'bg-mobile-600 text-white shadow-lg shadow-mobile-100 active:scale-95'"
        ...
        <div x-show="currentStep === 2" x-transition:enter="transition ease-out duration-300" class="space-y-6">
            <h1 class="text-2xl font-black text-slate-900 mb-2"
                x-text="!selectedParent ? 'Choose Category' : 'Select Size'"></h1>
            <p class="text-slate-500 text-sm mb-8 px-4"
                x-text="!selectedParent ? 'Pick the type of product you want to print.' : `Select the perfect dimensions for your ${selectedParent.name}.`">
            </p>

            <!-- Parent Selection -->
            <div x-show="!selectedParent" class="grid grid-cols-1 gap-3">
                <template x-for="parent in productTypes" :key="parent.id">
                    <button @click="selectedParent = parent"
                        class="flex items-center justify-between p-6 rounded-2xl border-2 border-slate-100 bg-white transition-all group hover:border-mobile-500 hover:shadow-md text-left active:scale-[0.98]">
                        <div class="flex items-center gap-4">
                            <div
                                class="w-12 h-12 bg-slate-50 text-slate-400 group-hover:bg-mobile-50 group-hover:text-mobile-600 rounded-2xl flex items-center justify-center transition-colors overflow-hidden">
                        ...
                        <div
                            class="w-10 h-10 rounded-full bg-slate-50 flex items-center justify-center text-slate-300 group-hover:bg-mobile-600 group-hover:text-white transition-all">
            ...
            <div x-show="selectedParent" class="space-y-4">
                <button @click="selectedParent = null; selectedSize = null"
                    class="flex items-center gap-2 text-xs font-black text-mobile-600 uppercase tracking-widest mb-4">
                    ...
                    <template x-for="size in selectedParent?.subtypes || []" :key="size.id">
                        <button @click="selectedSize = { ...size, label: `${selectedParent.name}: ${size.name}` }"
                            :class="selectedSize?.id === size.id ?
                                'border-mobile-500 bg-mobile-50 shadow-md ring-2 ring-mobile-100' :
                                'border-slate-100 bg-white'"
                            class="flex items-center justify-between p-5 rounded-2xl border-2 transition-all group text-left">
                            <div class="flex items-center gap-4">
                                <div :class="selectedSize?.id === size.id ? 'bg-mobile-600 text-white' : 'bg-slate-50 text-slate-400'"
                            ...
                            <div class="text-right">
                                <p class="font-black text-mobile-600" x-text="__price(size.price)"></p>
                                <template x-if="size.popular">
                                    <span
                                        class="text-[8px] font-black bg-mobile-100 text-mobile-600 px-1.5 py-0.5 rounded uppercase">Popular</span>
                                </template>
                            </div>
                        </button>
                    </template>
            ...
            <div class="pt-8 flex gap-3">
                <button @click="currentStep = 1"
                    class="flex-1 py-5 rounded-2xl bg-slate-100 text-slate-600 font-black text-sm uppercase tracking-widest">Back</button>
                <button @click="currentStep = 3" :disabled="!selectedSize"
                    :class="!selectedSize ? 'bg-slate-100 text-slate-400' :
                        'bg-mobile-600 text-white shadow-lg shadow-mobile-100 active:scale-95'"
        ...
        <div x-show="currentStep === 3" x-transition:enter="transition ease-out duration-300" class="space-y-6">
            ...
            <div class="bg-white rounded-3xl border border-slate-100 p-6 text-left space-y-6 shadow-sm">
                <div class="flex gap-4">
                    <img :src="previewUrl"
                        class="w-20 h-20 object-cover rounded-xl bg-slate-50 border border-slate-100">
                    <div class="flex-1 flex flex-col justify-center">
                        ...
                        <div class="flex items-center gap-3 mt-2">
                            <button @click="quantity = Math.max(1, quantity - 1)"
                                class="w-6 h-6 rounded bg-slate-100 flex items-center justify-center text-slate-600"><i
                    ...
                    <div class="text-right flex flex-col justify-center">
                        <p class="font-black text-mobile-600 text-lg" x-text="__price(selectedSize?.price * quantity)">
                        </p>
                    </div>
                </div>
                ...
                <div class="pt-4 border-t border-slate-50 space-y-4">
                    <div class="space-y-1">
                        ...
                        <input type="text" x-model="pickupName" placeholder="Full Name"
                            class="w-full bg-slate-50 border-0 rounded-xl py-3 px-4 text-sm font-bold focus:ring-2 focus:ring-mobile-500">
                    </div>
                    <div class="space-y-1">
                        <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Email
                            Address</label>
                        <input type="email" x-model="pickupEmail" placeholder="email@example.com"
                            class="w-full bg-slate-50 border-0 rounded-xl py-3 px-4 text-sm font-bold focus:ring-2 focus:ring-mobile-500">
                    </div>
                    <div class="space-y-1">
                        <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Phone Number</label>
                        <input type="tel" x-model="contactNumber" placeholder="000-000-0000"
                            class="w-full bg-slate-50 border-0 rounded-xl py-3 px-4 text-sm font-bold focus:ring-2 focus:ring-mobile-500">
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-3 pt-6">
                <button type="button" @click="openPaypal()" :disabled="isProcessing || !isValid"
                    :class="isProcessing || !isValid ? 'bg-slate-300 shadow-none scale-100' :
                        'bg-mobile-600 hover:bg-mobile-700 active:scale-[0.97] shadow-lg shadow-mobile-100'"
                ...
                <button type="button" @click="processCheckout('cash')" :disabled="isProcessing || !isValid"
                    :class="isProcessing || !isValid ? 'bg-slate-50 text-slate-400 border-slate-100 scale-100' :
                        'bg-white text-slate-700 border-slate-200 hover:border-slate-300 active:scale-[0.97]'"
                ...
                <button @click="currentStep = 2" :disabled="isProcessing"
                    ...
                    class="w-full py-6 text-slate-400 font-bold text-xs uppercase tracking-widest hover:text-slate-600 transition-colors">
