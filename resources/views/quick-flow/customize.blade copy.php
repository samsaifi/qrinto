...
<div id="customizer-app" class="container mx-auto mb-12">
    ...
    <div class="grid grid-cols-1 gap-4 items-start">
        ...
        <div class="fixed max-w-md mx-auto bottom-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-md border-t border-slate-100 shadow-[0_-10px_35px_rgba(0,0,0,0.08)] px-4 py-3 pb-safe">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <div id="upload-area" class="relative">
                        <label class="block cursor-pointer">
                            <div id="upload-zone" class="w-11 h-11 rounded-xl border border-slate-200 bg-slate-50 hover:bg-slate-100 active:scale-95 transition flex items-center justify-center shadow-sm">
                    ...
                    <button id="trigger-text-drawer" onclick="customizer.openTextDrawer()"
                        ...
                        <span id="text-edit-indicator" class="absolute -top-0.5 -right-0.5 w-2.5 h-2.5 bg-brand-500 rounded-full border-2 border-white hidden"></span>
                    </button>

                    <!-- Quick Color Picker Button -->
                    <div class="relative w-11 h-11 shrink-0">
                        <input type="color" id="main-color-input" oninput="customizer._updateSelectedStyle('fill', this.value)"
                            class="absolute inset-0 opacity-0 w-full h-full cursor-pointer z-10">
                        <div id="main-color-preview" class="w-full h-full rounded-xl border border-slate-200 bg-slate-50 shadow-sm flex items-center justify-center transition active:scale-95 overflow-hidden">
        ...
        <div id="drawer-backdrop" class="fixed max-w-md mx-auto inset-0 z-50 bg-slate-900/40 backdrop-blur-sm hidden transition-opacity duration-300 opacity-0" onclick="customizer.closeTextDrawer()"></div>

        <div id="text-drawer" class="fixed max-w-md mx-auto inset-x-0 bottom-0 z-50 bg-white rounded-t-[32px] shadow-[0_-15px_40px_rgba(0,0,0,0.12)] transform translate-y-full transition-transform duration-300 ease-out border-t border-slate-100 pb-safe">
            <!-- Pull Handle bar -->
            ...
            <div class="px-6 pb-3 flex items-center justify-between border-b border-slate-50">
                ...
                <button onclick="customizer.closeTextDrawer()" class="p-1 rounded-full hover:bg-slate-50 text-slate-400 hover:text-slate-600 transition">
            ...
            <div class="p-6 space-y-4 max-h-[60vh] overflow-y-auto">
                ...
                <div class="grid grid-cols-2 gap-3">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <select id="font-size-select" onchange="customizer._updateSelectedStyle('fontSize', parseInt(this.value))"
                            class="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2.5 text-xs font-semibold focus:outline-none focus:border-brand-500 focus:bg-white transition-all">
                            @for($i=8; $i<=96; $i+=2)
                                <option value="{{ $i }}" {{ $i == 16 ? 'selected' : '' }}>{{ $i }} px</option>
                                @endfor
                        </select>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-3 items-center">
                    <div class="space-y-1.5">
                        <label class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Color Palette</label>
                        <div class="relative w-full h-11">
                            <input type="color" id="text-color-input" oninput="customizer._updateSelectedStyle('fill', this.value)"
                                class="absolute inset-0 opacity-0 w-full h-full cursor-pointer z-10">
                            <div id="text-color-preview" class="w-full h-full rounded-xl border border-slate-200 shadow-sm flex items-center justify-between px-3 bg-slate-50" style="background: #000000">
                                <span class="text-xs font-bold text-white mix-blend-difference">Choose</span>
                                <i data-lucide="palette" class="w-4 h-4 text-white mix-blend-difference opacity-75"></i>
                            </div>
                        </div>
                    </div>
                ...
                <div class="pt-3">
                    <button id="add-text-btn" onclick="customizer.addTextAndClose()" class="w-full bg-brand-500 hover:bg-brand-600 text-white font-bold py-3.5 rounded-xl text-sm shadow-lg shadow-brand-100 transition-all active:scale-[0.98] flex items-center justify-center gap-2">
                        <i data-lucide="plus" class="w-4 h-4"></i> Add to Card
                    </button>
                    <div id="editing-badge" class="hidden w-full flex items-center justify-center gap-2 py-3.5 bg-brand-50 text-brand-600 rounded-xl border border-brand-100">
...
<script>
    const customizer = {
        ...
        init() {
            ...
            window.addEventListener('resize', this._debounce(() => { this._resizeAllCanvases(); this._positionTextActionIcons(); }, 150));
        ...
        _initAllCanvases() {
            ...
            Object.keys(this.imageTypes).forEach(key => {
                ...
                fc.on('object:modified', () => { this._saveCanvasState(key); this._positionTextActionIcons(); });
                fc.on('object:added', () => this._saveCanvasState(key));
                fc.on('object:removed', () => { this._saveCanvasState(key); this._hideTextActionIcons(); });
