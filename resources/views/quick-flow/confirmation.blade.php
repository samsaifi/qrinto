...
@php
    ...
    foreach($order->items as $orderItem) {
        ...
        $itemPreviews[] = [
            ...
            'preview_url' => !empty($orderItem->preview_image_path) ? asset('storage/' . $orderItem->preview_image_path) : (!empty($itemPages[0]) ? $itemPages[0] : null),
        ];
    }
@endphp

<div class="space-y-4 pb-28 pt-1">
    
    {{-- Success Hero Banner --}}
    <div class="bg-white border-2 border-slate-50 rounded-[2rem] p-5 shadow-premium text-center space-y-3 relative overflow-hidden">
        <div class="w-16 h-16 bg-emerald-500 text-white rounded-full flex items-center justify-center mx-auto shadow-lg shadow-emerald-500/25">
            <i data-lucide="check" class="w-8 h-8 stroke-[3]"></i>
        </div>
        <div>
            <span class="text-[10px] font-black text-emerald-600 uppercase tracking-[0.2em] bg-emerald-50 px-3 py-1 rounded-full border border-emerald-200/60 inline-block mb-1.5">
                Order Confirmed
            </span>
        ...
        <div class="flex items-center justify-center gap-2 pt-1 flex-wrap">
            <span class="bg-slate-900 text-white text-xs font-black px-3.5 py-1.5 rounded-xl flex items-center gap-1.5 shadow-sm">
                <i data-lucide="hash" class="w-3.5 h-3.5 text-brand-400"></i>
                <span>{{ $order->order_number }}</span>
            </span>
            @if($order->store)
            <span class="bg-slate-100 text-slate-700 text-xs font-bold px-3.5 py-1.5 rounded-xl border border-slate-200/80 flex items-center gap-1.5">
    ...
    <div class="bg-white border-2 border-slate-50 rounded-[2rem] p-5 shadow-premium space-y-4" x-data="{ activeIndex: 0 }">
        ...
        <div class="flex items-center gap-2 overflow-x-auto pb-1 scrollbar-none">
            ...
            <button type="button" @click="activeIndex = {{ $idx }}"
                :class="activeIndex === {{ $idx }} ? 'bg-brand-500 text-white border-brand-500 font-extrabold' : 'bg-slate-50 text-slate-600 border-slate-200 font-bold'"
        ...
        <div x-show="activeIndex === {{ $idx }}" x-transition class="space-y-4">
            <div class="bg-slate-50/80 border border-slate-100 rounded-2xl p-4 flex flex-col items-center justify-center text-center">
                {{-- Fanned Card Deck Preview --}}
                <div class="fanned-card-stage relative flex items-center justify-center w-36 h-36 shrink-0 select-none py-1 px-1 my-2">
                    @if(count($prevItem['images']) > 0)
                        @foreach($prevItem['images'] as $imgIdx => $imgUrl)
                            ...
                            <div class="fanned-card absolute top-1/2 left-1/2 rounded-xl overflow-hidden bg-white border border-white shadow-sm transition-all duration-300"
                                ...
                                <img src="{{ $imgUrl }}" alt="Page {{ $imgIdx + 1 }}" class="w-full h-full object-cover">
                                @if($count > 1)
                        ...
                        @endif
                    @else
                        <div class="w-24 h-32 rounded-xl bg-white border border-slate-200 flex flex-col items-center justify-center text-slate-300">
                ...
                <div class="mt-2">
                    @if(!empty($prevItem['type_name']))
                        <span class="text-[9px] font-black text-brand-600 uppercase tracking-widest block">{{ $prevItem['type_name'] }}</span>
                    @endif
                    <h3 class="font-extrabold text-sm text-slate-900 leading-tight mt-0.5">{{ $prevItem['name'] }}</h3>
    ...
    <div class="bg-white border-2 border-slate-50 rounded-[2rem] p-5 shadow-premium space-y-3">
        ...
        <div class="space-y-2.5 pt-1">
            <div class="bg-emerald-50 border border-emerald-200/80 p-3 rounded-xl flex items-center gap-3">
                ...
                <div class="flex-1">
                    ...
                    <p class="text-[10px] font-bold text-emerald-700">Received at store</p>
                </div>
            </div>

            <div class="bg-amber-50 border border-amber-200/80 p-3 rounded-xl flex items-center gap-3">
                <div class="w-7 h-7 rounded-full bg-amber-500 text-white flex items-center justify-center shrink-0 animate-pulse">
    ...
    <div class="bg-white border-2 border-slate-50 rounded-[2rem] p-5 shadow-premium space-y-3">
        ...
        <div class="space-y-2 text-xs font-bold pt-1">
            ...
            <div class="flex justify-between text-slate-500">
                ...
                <span class="{{ $order->payment_status === 'paid' ? 'text-emerald-600' : 'text-amber-600' }} uppercase font-extrabold">
    ...
    <div class="fixed bottom-0 left-0 right-0 max-w-md mx-auto p-4 glass border-t border-slate-100 safe-bottom z-50">
        <a href="{{ route('flow.track.order', $order->order_number) }}"
            class="w-full bg-brand-500 hover:bg-brand-600 text-white font-extrabold py-3.5 rounded-2xl shadow-xl transition-all active:scale-[0.97] flex items-center justify-center gap-2 text-base no-underline">
