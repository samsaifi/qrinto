...
@section('content')
<div class="max-w-2xl">
    <h1 class="font-display font-bold text-2xl text-surface-900 mb-8">{{ isset($event) ? 'Edit Event' : 'Create Event' }}</h1>
    ...
    <form action="{{ isset($event) ? route('admin.events.update', $event) : route('admin.events.store') }}"
        ...
        <div class="bg-white rounded-2xl border border-surface-100 shadow-card p-6 space-y-5">
            ...
            <div class="grid sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="date" name="start_date" value="{{ old('start_date', isset($event) && $event->start_date ? $event->start_date->format('Y-m-d') : '') }}"
                           class="w-full rounded-xl border-surface-200 focus:border-brand-500 focus:ring-brand-500">
                ...
                <div>
                    ...
                    <input type="date" name="end_date" value="{{ old('end_date', isset($event) && $event->end_date ? $event->end_date->format('Y-m-d') : '') }}"
                           class="w-full rounded-xl border-surface-200 focus:border-brand-500 focus:ring-brand-500">
            ...
            <div>
                    ...
                    <div class="relative flex gap-2 items-center">
                        <div class="relative flex-1">
                            <input id="icon_lucide" name="icon_lucide" type="text" autocomplete="off"
                                   ...
                                   class="w-full rounded-xl border-surface-200 focus:border-brand-500 focus:ring-brand-500">
                            <div id="icon-suggestions" class="hidden absolute z-50 left-0 right-0 top-full mt-1 bg-white border border-surface-200 rounded-xl shadow-lg max-h-56 overflow-y-auto"></div>
                        </div>
                        <div id="icon-preview" class="w-10 h-10 rounded-lg bg-surface-100 flex items-center justify-center flex-shrink-0">
                ...
                <input type="hidden" name="icon_svg" id="icon_svg_hidden" value="{{ old('icon_svg', $event->icon_svg ?? '') }}">
                @error('icon_svg') <p class="text-xs text-red-500 mt-1">{{ $message }}</p> @enderror
            </div>
            ...
            <div>
                ...
                @if(auth()->user()->isAdmin())
                    ...
                    @include('admin.events._multiselect', [
                        ...
                        'selectedIds' => old('stores', isset($event) ? $event->stores->pluck('id')->map(fn($id) => (string) $id)->toArray() : []),
                        'fieldName' => 'stores',
        ...
        <div class="flex gap-3">
            ...
            <a href="{{ route('admin.events.index') }}" class="px-6 py-3 bg-surface-100 text-surface-600 font-semibold rounded-xl hover:bg-surface-200 transition">Cancel</a>
        </div>
    </form>
</div>
@endsection
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const allNames = [
        'accessibility','activity','air-vent','airplay','alarm-check','alarm-clock','alarm-clock-off','alarm-minus','alarm-plus',
        'album','alert-circle','alert-octagon','alert-triangle','align-center','align-center-horizontal','align-center-vertical',
        ...
        'align-vertical-distribute-center','align-vertical-distribute-end','align-vertical-distribute-start',
        'align-vertical-justify-center','align-vertical-justify-end','align-vertical-justify-start',
        'align-vertical-space-around','align-vertical-space-between','ampersand','ampersands','anchor',
        'angry','annoyed','antenna','aperture','app-window','apple','archive','archive-restore','archive-x',
        ...
        'arrow-down-to-line','arrow-left','arrow-left-circle','arrow-left-right','arrow-left-to-line',
        ...
        'baby','backpack','badge','badge-alert','badge-check','badge-dollar-sign','badge-euro','badge-help',
        'badge-indian-rupee','badge-info','badge-japanese-yen','badge-minus','badge-percent','badge-plus',
        'badge-pound-sterling','badge-russian-ruble','badge-swiss-franc','badge-x','banknote','bar-chart',
        ...
        'battery-low','battery-medium','battery-warning','beaker','bean','bean-off','bed','bed-double',
        'bed-single','beef','beer','bell','bell-dot','bell-minus','bell-off','bell-plus','bell-ring',
        'bike','binary','biohazard','bird','bitcoin','blend','blinds','blocks','bluetooth',
        'bluetooth-connected','bluetooth-off','bluetooth-searching','bold','bolt','bomb','bone','book',
        'book-copy','book-down','book-key','book-lock','book-marked','book-minus','book-open',
        'book-open-check','book-open-text','book-plus','book-template','book-text','book-type','book-up',
        'book-user','book-x','bookmark','bookmark-minus','bookmark-plus','boom-box','bot','box',
        'box-select','boxes','braces','brackets','brain','brain-circuit','brain-cog','brick-wall',
        ...
        'calendar-clock','calendar-days','calendar-heart','calendar-minus','calendar-off','calendar-plus',
        ...
        'clock-10','clock-11','clock-12','clock-2','clock-3','clock-4','clock-5','clock-6','clock-7',
        'clock-8','clock-9','cloud','cloud-cog','cloud-drizzle','cloud-fog','cloud-hail','cloud-lightning',
        'cloud-moon','cloud-moon-rain','cloud-off','cloud-rain','cloud-rain-wind','cloud-snow',
        'cloud-sun','cloud-sun-rain','clover','club','code','code-2','codepen','codesandbox','coffee',
        'cog','coins','columns','combine','command','compass','component','computer','concierge-bell',
        'cone','construction','contact','contact-2','container','contrast','cookie','copy','copy-check',
        'copy-minus','copy-plus','copy-slash','copy-x','copyleft','corner-down-left','corner-down-right',
        'corner-left-down','corner-left-up','corner-right-down','corner-right-up','corner-up-left',
        'corner-up-right','cpu','creative-commons','credit-card','croissant','crop','cross','crosshair',
        'crown','cup-soda','currency','database','database-backup','database-zap','delete',
        'dessert','diamond','dice-1','dice-2','dice-3','dice-4','dice-5','dice-6','dices','diff',
        ...
        'facebook','factory','fan','fast-forward','feather','ferris-wheel','figma','file','file-archive',
        ...
        'file-image','file-input','file-json','file-json-2','file-key','file-key-2','file-line-chart',
        ...
        'file-signature','file-spreadsheet','file-stack','file-symlink','file-terminal','file-text',
        ...
        'flip-vertical','flip-vertical-2','flower','flower-2','focus','fold-horizontal','fold-vertical',
        ...
        'folder-open-dot','folder-output','folder-plus','folder-root','folder-search','folder-search-2',
        ...
        'gallery-vertical-end','gamepad','gamepad-2','gauge','gauge-circle','gavel','gem','ghost',
        ...
        'gitlab','glass-water','glasses','globe','globe-2','goal','grab','graduation-cap','grape',
        'grid-2x2','grid-3x3','grip','grip-horizontal','grip-vertical','group','guitar',
        'hammer','hand','hand-metal','hard-drive','hard-drive-download','hard-drive-upload','hash',
        'haze','hdmi-port','heading','heading-1','heading-2','heading-3','heading-4','heading-5',
        'heading-6','headphones','heart','heart-crack','heart-handshake','heart-off','heart-pulse',
        'heater','help-circle','helpline','hexagon','highlighter','history','home','hop','hop-off',
        'hotel','hourglass','ice-cream','ice-cream-2','image','image-minus','image-off','image-plus',
        'import','inbox','indent','indian-rupee','infinity','info','inspection-panel','instagram',
        ...
        'lasso','lasso-select','laugh','layers','layers-2','layers-3','layout','layout-dashboard',
        'layout-grid','layout-list','layout-panel-left','layout-panel-top','layout-template','leaf',
        ...
        'list-end','list-filter','list-minus','list-music','list-ordered','list-plus','list-restart',
        ...
        'luggage','magnet','mail','mail-check','mail-minus','mail-open','mail-plus','mail-question',
        ...
        'mic','mic-2','mic-off','microscope','microwave','milestone','milk','milk-off','minimize',
        'minimize-2','minus','minus-circle','minus-square','monitor','monitor-check',
        'monitor-dot','monitor-down','monitor-off','monitor-pause','monitor-play','monitor-smartphone',
        'monitor-speaker','monitor-stop','monitor-up','monitor-x','moon','moon-star','more-horizontal',
        ...
        'parking-meter','parking-square','parking-square-off','party-popper','pause','pause-circle',
        ...
        'plane-takeoff','play','play-circle','play-square','plug','plug-2','plug-zap','plug-zap-2',
        'plus','plus-circle','plus-square','pocket','pocket-knife','podcast','pointer','popcorn',
        'popsicle','pound-sterling','power','power-circle','power-off','power-square','presentation',
        ...
        'replace-all','reply','reply-all','rewind','ribbon','rocket','rocking-chair','roller-coaster',
        'rotate-3d','rotate-ccw','rotate-cw','route','route-off','router','rows','rows-2','rows-3',
        'rows-4','rss','ruler','russian-ruble',
        'sailboat','salad','sandwich','satellite','satellite-dish','save','save-all','scale',
        'scale-3d','scaling','scan','scan-barcode','scan-eye','scan-face','scan-line','scan-search',
        ...
        'separator-horizontal','separator-vertical','server','server-cog','server-crash','server-off',
        'settings','settings-2','shapes','share','share-2','sheet','shell','shield','shield-alert',
        ...
        'sigma','signal','signal-high','signal-low','signal-medium','signal-zero','siren','skip-back',
        'skip-forward','skull','slack','slash','slice','sliders','sliders-horizontal','smartphone',
        'smartphone-charging','smartphone-nfc','smile','smile-plus','snail','snowflake','sofa',
        'soup','space','spade','sparkle','sparkles','speaker','speech','spell-check','spell-check-2',
        ...
        'wallet','wallet-cards','wallet-minimal','wallpaper','wand','wand-2','warehouse','washing-machine',
        'watch','waves','waypoints','webcam','webhook','weight','wheat','wheat-off','whole-word',
    ...
    function renderSuggestions(query) {
        if (!query || !allNames.length) { dropdown.classList.add('hidden'); return; }
        ...
        activeIdx = -1;
        if (window.lucide) lucide.createIcons({ nodes: dropdown.querySelectorAll('[data-lucide]') });
    }
    ...
    input.addEventListener('keydown', e => {
        ...
        else if (e.key === 'Enter' && activeIdx >= 0) { e.preventDefault(); pickIcon(items[activeIdx].dataset.name); return; }
