...
<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
    ...
    <a href="{{ route('admin.events.create') }}" class="inline-flex items-center gap-2 px-5 py-2.5 bg-brand-600 text-white text-sm font-semibold rounded-xl hover:bg-brand-700 transition shadow-lg shadow-brand-200">
            ...
            <tbody class="divide-y divide-surface-50">
                ...
                <tr class="hover:bg-surface-50/60 transition-colors">
                    <td class="px-3 py-3 text-center">
                        @if($event->icon_svg)
                            <span class="inline-flex items-center justify-center w-8 h-8 rounded-lg" style="background-color: {{ $event->color ?? '#000000' }}20; color: {{ $event->color ?? '#000000' }}">
                                {!! $event->icon($event->icon_svg) !!}

                            </span>
                        @elseif($event->color)
                            <span class="inline-block w-5 h-5 rounded-full border border-surface-200" style="background-color: {{ $event->color }}"></span>
                        @else
                            <span class="text-xs text-surface-400">&mdash;</span>
                        @endif
                    </td>
                    <td class="px-5 py-3">
                        <a href="{{ route('admin.events.show', $event) }}" class="font-semibold text-sm text-surface-800 hover:text-brand-600 transition">{{ $event->title }}</a>
                    </td>
                    <td class="px-3 py-3 text-center">
                        <span class="inline-flex px-2 py-0.5 text-xs font-semibold rounded-md {{ $event->is_active ? 'bg-accent-100 text-accent-700' : 'bg-surface-200 text-surface-500' }}">
                    ...
                    <td class="px-3 py-3 text-center">
                        <div class="flex items-center justify-center gap-1">
                            <a href="{{ route('admin.events.show', $event) }}" class="inline-flex p-1.5 rounded-lg hover:bg-surface-100 text-surface-400 hover:text-surface-600 transition" title="View">
                                <i data-lucide="eye" class="w-4 h-4"></i>
                            </a>
                            <a href="{{ route('admin.events.edit', $event) }}" class="inline-flex p-1.5 rounded-lg hover:bg-brand-50 text-surface-400 hover:text-brand-600 transition" title="Edit">
                ...
                @empty
                <tr><td colspan="{{ auth()->user()->isAdmin() ? 7 : 6 }}" class="px-6 py-12 text-center text-surface-400">No events yet. <a href="{{ route('admin.events.create') }}" class="text-brand-600 font-medium">Add your first event</a>.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
