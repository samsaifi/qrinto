            ...
            <table class="w-full">
                <thead>
                    <tr class="bg-surface-50">
                        ...
                        <th class="px-6 py-3 text-left text-xs font-semibold text-surface-500 uppercase">Pickup User Name
                        </th>
                        <th class="px-6 py-3 text-left text-xs font-semibold text-surface-500 uppercase">Pickup User Email
                        </th>
                    ...
                    @forelse($orders as $order)
                        <tr class="hover:bg-surface-50 transition">
                            <td class="px-6 py-4"><span
                                    class="font-mono font-semibold text-brand-600">{{ $order->order_number }}</span></td>
                            ...
                            <td class="px-6 py-4 text-sm text-surface-600">
                                @if ($order->store)
                                    <span class="inline-flex items-center gap-1"><i data-lucide="store"
                                            class="w-3 h-3 text-surface-400"></i> {{ $order->store->store_name }}</span>
                                @else
                            ...
                            <td class="px-6 py-4">
                                <span
                                    class="px-2 py-0.5 text-xs font-semibold rounded {{ $order->payment_status === 'paid' ? 'bg-accent-100 text-accent-700' : 'bg-yellow-100 text-yellow-700' }}">
                                    {{ ucfirst($order->payment_status) }}
                                </span>
                            </td>
                            <td class="px-6 py-4 text-right font-semibold">
                                {{ \App\Services\CurrencyService::formatWithCurrency($order->total, $order->currency, 0) }}
                            </td>
                            ...
                            <td class="px-6 py-4 text-right flex items-center justify-end gap-3">
                                <button
                                    ...
                                    class="text-surface-500 hover:text-brand-600 transition" title="Duplicate Order">
                                    <i data-lucide="copy" class="w-4 h-4"></i>
                                </button>
                                <a href="{{ route('admin.orders.show', $order) }}"
                                    class="text-brand-600 hover:text-brand-700 text-sm font-medium">View →</a>
                            </td>
                        </tr>
                    @empty
                        <tr>
                ...
                <div class="mb-5">
                    <p class="text-sm text-surface-500 mb-4">
                        You are duplicating order <span id="duplicate-order-number"
                            class="font-mono font-bold text-brand-600"></span>. All customization, uploads, and details will
                        remain the same. Please specify the quantity for the new order.
                    </p>
