...
<div class="bg-white rounded-2xl border border-surface-200/70 shadow-xs p-4 mb-6">
    <form action="{{ route('admin.orders.index') }}" method="GET" class="flex flex-wrap gap-4 items-end">
        ...
        <div>
            ...
            <select name="status" class="text-xs px-3.5 py-2 rounded-xl border border-surface-200 bg-white focus:ring-2 focus:ring-brand-500 focus:outline-none">
        ...
        <div class="flex items-center gap-2">
            <button type="submit" class="px-4 py-2 bg-brand-600 text-white text-xs font-semibold rounded-xl hover:bg-brand-700 transition shadow-2xs">Filter</button>
            <a href="{{ route('admin.orders.index') }}" class="px-3 py-2 text-xs font-medium text-surface-500 hover:text-brand-600 transition">Clear</a>
        </div>
    </form>
</div>
...
<div class="bg-white rounded-2xl border border-surface-200/70 shadow-xs overflow-hidden">
    <div class="overflow-x-auto">
        <table class="w-full text-left text-xs align-middle">
            <thead>
                <tr class="bg-surface-50 border-b border-surface-100 text-surface-500 uppercase tracking-wider font-semibold">
            ...
            <tbody class="divide-y divide-surface-100 font-medium">
                ...
                @php
                    ...
                    $rawEmail = $order->shipping_address['email'] ?? ($order->guest_email ?? ($order->user->email ?? 'N/A'));
                ...
                <tr class="hover:bg-surface-50/70 transition h-14">
                    <!-- Order ID -->
                    <td class="px-4 py-3">
                        <a href="{{ route('admin.orders.show', $order) }}" class="font-mono text-xs font-bold text-brand-600 hover:text-brand-700 transition whitespace-nowrap block" title="{{ $order->order_number }}">
                            {{ $order->order_number }}
                        </a>
                    </td>
                    ...
                    <!-- Pickup User Name -->
                    <td class="px-4 py-3">
                        <span class="text-xs text-surface-900 font-semibold truncate block max-w-[125px]" title="{{ $formattedName }}">
                            {{ $formattedName }}
                        </span>
                    </td>
                    ...
                    <!-- Pickup User Email -->
                    <td class="px-4 py-3">
                        <span class="text-xs text-surface-600 truncate block max-w-[210px]" title="{{ $rawEmail }}">
                            {{ $rawEmail }}
                        </span>
                    </td>
                    ...
                    <td class="px-4 py-3 text-center">
                        <span class="inline-flex items-center justify-center w-6 h-6 rounded-full bg-surface-100 text-surface-700 font-bold text-xs">
                            {{ $order->items->count() }}
                        </span>
                    </td>
                    ...
                    <!-- Order Status -->
                    <td class="px-4 py-3 text-center">
                        <span class="px-2.5 py-1 text-[11px] font-bold rounded-lg inline-block whitespace-nowrap shadow-2xs" style="background-color: {{ ($order->status_color ?? '#6b7280') }}20; color: {{ $order->status_color ?? '#6b7280' }}">
                    ...
                    <td class="px-4 py-3">
                        <div class="flex items-center gap-1.5 min-w-0 max-w-[145px]">
                            @if($storeName)
                                ...
                                <span class="text-xs text-surface-700 truncate font-medium" title="{{ $storeName }}">{{ $storeName }}</span>
                            @else
                    ...
                    <!-- Payment Status -->
                    <td class="px-4 py-3 text-center">
                        <span class="px-2 py-0.5 text-[10px] uppercase font-bold rounded inline-block whitespace-nowrap {{ $order->payment_status === 'paid' ? 'bg-emerald-100 text-emerald-800 border border-emerald-200' : 'bg-amber-100 text-amber-800 border border-amber-200' }}">
                    ...
                    <td class="px-4 py-3 text-right">
                        <div class="flex items-center justify-end gap-2.5 whitespace-nowrap">
                            <button type="button" onclick="openDuplicateModal('{{ $order->id }}', '{{ $order->order_number }}', '{{ $order->items->first()?->quantity ?? 1 }}')" 
                                    class="p-1 text-surface-400 hover:text-brand-600 transition" 
                                    title="Duplicate Order">
                                <i data-lucide="copy" class="w-4 h-4"></i>
                            </button>
                            <a href="{{ route('admin.orders.show', $order) }}" class="text-brand-600 hover:text-brand-700 text-xs font-bold inline-flex items-center gap-1">
