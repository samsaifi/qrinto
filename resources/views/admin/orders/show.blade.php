...
@section('content')
    ...
    <div class="flex items-center justify-between mb-8">
        <div> @php $addr = $order->shipping_address; @endphp
            ...
            <p class="text-sm text-surface-500">Placed {{ $order->created_at->format('M d, Y h:i A') }} by
                {{ $addr['name'] ?? ($order->user->name ?? 'N/A') }}</p>
        </div>
        <div class="flex items-center gap-3">
            @if ($order->store)
                <a href="{{ route('admin.stores.edit', $order->store) }}"
                    class="px-4 py-2 text-sm font-semibold rounded-xl bg-brand-50 text-brand-700 border border-brand-200/60 flex items-center gap-2 hover:bg-brand-100 transition">
        ...
        <div class="lg:col-span-2 space-y-6">
                ...
                <div class="space-y-5">
                    @foreach ($order->items as $item)
                        @php
                            ...
                            $uploadIds =
                                $item->customization_data['canvas_mapped_ids'] ??
                                ($item->customization_data['upload_ids'] ?? []);
                            if (is_array($uploadIds) && !empty($uploadIds)) {
                                $uList = \App\Models\CustomerUpload::whereIn(
                                    'id',
                                    array_values(array_filter($uploadIds)),
                                )->get();
                            ...
                            $itemPrintUrl =
                                $itemPdfUrl ?: ($firstImgUrl ?: route('admin.orders.print.page', $order->id));
                        @endphp

                        <div
                            class="bg-surface-50/70 border border-surface-200/70 rounded-2xl p-4 sm:p-5 flex flex-col md:flex-row gap-5 items-start justify-between relative group hover:border-brand-300 transition-all">
                            ...
                            <div class="flex items-center justify-center shrink-0">
                                ...
                                <div
                                    class="fanned-card-stage relative flex items-center justify-center w-28 h-28 shrink-0 select-none py-1 px-1 bg-white/80 rounded-2xl border border-surface-200/80 shadow-2xs">
                                    @if ($imageCount > 0)
                                        @foreach ($itemImages as $idx => $imgUrl)
                                            ...
                                            <a href="{{ $imgUrl }}" target="_blank"
                                                ...
                                                @if ($count > 1)
                                                    <div
                                                        class="absolute bottom-0.5 right-0.5 bg-surface-900/90 text-white text-[8px] font-black px-1 rounded">
                                                        P{{ $idx + 1 }}
                                                    </div>
                                                @endif
                                            </a>
                                        ...
                                        @if ($count > 1)
                                            <div
                                                class="absolute bottom-1 left-1/2 -translate-x-1/2 bg-surface-900/90 text-white text-[9px] font-bold px-2 py-0.5 rounded-full shadow-md backdrop-blur-xs whitespace-nowrap z-40 flex items-center gap-1 border border-surface-700">
                                    ...
                                    @else
                                        <div
                                            class="w-full h-full rounded-xl overflow-hidden bg-surface-100 flex flex-col items-center justify-center text-surface-400">
                            ...
                            <div class="flex-1 min-w-0 space-y-2">
                                <div class="flex items-start justify-between gap-3">
                                    <div>
                                        <h3 class="font-bold text-surface-900 text-base leading-snug">
                                            {{ $item->product_name }}</h3>
                                        <p class="text-xs text-surface-500 font-semibold mt-0.5">
                                            {{ \App\Services\CurrencyService::formatWithCurrency($item->unit_price, $order->currency) }}
                                            × {{ $item->quantity }} pcs
                                        </p>
                                    </div>
                                </div>
                                ...
                                @if ($item->selected_options && count($item->selected_options) > 0)
                                    @php
                                        ...
                                        $stringOptions = array_filter(
                                            $item->selected_options,
                                            fn($val) => !is_numeric($val),
                                        );
                                        $selectedOptionValues = !empty($optionIds)
                                            ? \App\Models\ProductOptionValue::with('optionGroup')
                                    ...
                                    <div class="flex flex-wrap gap-2 pt-1">
                                        @foreach ($selectedOptionValues as $optVal)
                                            <span
                                                class="inline-flex items-center gap-1 text-[11px] font-semibold text-surface-700 bg-white px-2.5 py-1 rounded-lg border border-surface-200">
                                        ...
                                        @foreach ($stringOptions as $key => $val)
                                            @if ($val !== null && $val !== '' && strtolower($val) !== 'null')
                                                <span
                                                    class="inline-flex items-center gap-1 text-[11px] font-semibold text-surface-700 bg-white px-2.5 py-1 rounded-lg border border-surface-200">
                                                    <span
                                                        class="text-surface-400">{{ ucfirst(str_replace('_', ' ', $key)) }}:</span>
                                ...
                                @php
                                    ...
                                    if (
                                        empty($dim) &&
                                        !empty($item->customization_data['size_width']) &&
                                        !empty($item->customization_data['size_height'])
                                    ) {
                                        $dim =
                                            sprintf('%.2f', (float) $item->customization_data['size_width']) .
                                            ' × ' .
                                            sprintf('%.2f', (float) $item->customization_data['size_height']);
                                    }
                                    if (
                                        empty($dim) &&
                                        $item->product &&
                                        !empty($item->product->width) &&
                                        !empty($item->product->height)
                                    ) {
                                        $dim =
                                            sprintf('%.2f', (float) $item->product->width) .
                                            ' × ' .
                                            sprintf('%.2f', (float) $item->product->height);
                                    }
                                    if (
                                        empty($dim) &&
                                        $item->product &&
                                        $item->product->productType &&
                                        !empty($item->product->productType->width) &&
                                        !empty($item->product->productType->height)
                                    ) {
                                        $dim =
                                            sprintf('%.2f', (float) $item->product->productType->width) .
                                            ' × ' .
                                            sprintf('%.2f', (float) $item->product->productType->height);
                                    }
                                    ...
                                    // 2. Dynamic Unit
                                    $unit =
                                        $item->customization_data['unit'] ??
                                        ($item->product->unit ?? ($item->product->productType->unit ?? null));
                                    if (!empty($unit)) {
                                    ...
                                    $sizeLabel =
                                        $item->customization_data['size_label'] ??
                                        ($item->customization_data['size_name'] ?? null);
                                    if (empty($sizeLabel) && $item->product && $item->product->productType) {
                                        $sizeLabel =
                                            $item->product->productType->title ?? $item->product->productType->name;
                                    }
                                    if (!empty($sizeLabel)) {
                                    ...
                                    // 4. Dynamic Card Type
                                    $cardType =
                                        $item->customization_data['type_name'] ??
                                        ($item->customization_data['card_type'] ?? null);
                                    ...
                                    // 5. Dynamic Category
                                    $category =
                                        $item->customization_data['category_name'] ??
                                    ...
                                    if (!empty($item->customization_data) && is_array($item->customization_data)) {
                                        foreach ($item->customization_data as $k => $v) {
                                            ...
                                            if (
                                                ...
                                                $v === '' ||
                                                (is_string($v) &&
                                                    in_array(trim(strtolower($v)), ['', 'x', 'null', 'n/a', ' x '])) ||
                                                (is_array($v) && empty($v))
                                            ) {
                                                continue;
                                            }
                                        ...
                                        if (
                                            isset($item->customization_data['style']) &&
                                            is_array($item->customization_data['style'])
                                        ) {
                                            foreach ($item->customization_data['style'] as $styleKey => $styleVal) {
                                                if (
                                                    ...
                                                    $formattedKey = ucfirst(
                                                        preg_replace('/(?<!^)[A-Z]/', ' $0', $styleKey),
                                                    );
                                                    $formattedVal = is_bool($styleVal)
                                                        ? ($styleVal
                                                            ? 'Yes'
                                                            : 'No')
                                                        : $styleVal;
                                ...
                                @if (count($visibleCustomDetails) > 0)
                                    <div
                                        class="text-xs text-surface-600 bg-white p-3.5 rounded-xl border border-surface-200/80 space-y-1.5 shadow-2xs">
                                        @foreach ($visibleCustomDetails as $detailLabel => $detailValue)
                                            <p class="flex items-center gap-2 text-xs">
                                                <span
                                                    class="font-semibold text-surface-500 w-24 flex-shrink-0">{{ $detailLabel }}:</span>
                                ...
                                <div class="pt-2 flex items-center gap-2 flex-wrap">
                                    @if ($itemPdfUrl)
                                        <a href="{{ $itemPdfUrl }}"
                                            download="design-pdf-{{ $item->order_id }}-{{ $item->id }}.pdf"
                                            target="_blank"
                                    ...
                                    @if ($firstImgUrl)
                                        <a href="{{ $firstImgUrl }}"
                                            download="design-{{ $item->order_id }}-{{ $item->id }}.jpg"
                                            target="_blank"
                                            class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-rose-50 border border-rose-200 text-rose-700 text-xs font-bold rounded-xl hover:bg-rose-100 transition shadow-2xs">
                                    ...
                                    <a href="{{ route($rPrefix . 'orders.print.page', ['order' => $order->id, 'item' => $item->id]) }}"
                ...
                <div class="bg-white rounded-2xl border border-surface-100 shadow-card p-6">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        @if (isset($order->flow_data['category_name']))
                            <div>
                                <label
                                    class="block text-xs font-semibold text-surface-500 uppercase tracking-widest mb-1">Category</label>
                                <p class="text-sm font-medium text-surface-900">{{ $order->flow_data['category_name'] }}
                                </p>
                            </div>
                        @endif
                        @if (isset($order->flow_data['size_name']))
                            <div>
                                <label
                                    class="block text-xs font-semibold text-surface-500 uppercase tracking-widest mb-1">Selected
                                    Size</label>
                ...
                <div class="bg-white rounded-2xl border border-surface-100 shadow-card p-6">
                    ...
                    @if (($addr['type'] ?? '') === 'store_pickup')
                        ...
                        <p class="text-sm text-surface-600 leading-relaxed">
                            ...
                            {{ $order->store ? $order->store->store_name : 'No Store Selected' }}<br>
                            <strong>Pickup Name:</strong> {{ $addr['name'] ?? '' }}<br>
                            <strong>Pickup Email:</strong>
                            {{ $addr['pickup_email'] ?? ($order->guest_email ?? ($order->user->email ?? 'N/A')) }}<br>
                            ...
                            @else
                                <span
                                    class="text-surface-600 font-semibold">{{ ucfirst($order->payment_gateway ?? 'Unknown') }}</span>
                            @endif
                        </p>
                    @else
            ...
            @if ($order->special_instructions)
                <div class="bg-white rounded-2xl border border-surface-100 shadow-card p-6 border-l-4 border-l-amber-500">
                    ...
                    <p class="text-sm text-surface-600 leading-relaxed whitespace-pre-wrap">
            ...
            <div class="bg-white rounded-2xl border border-surface-100 shadow-card p-6">
                ...
                <form action="{{ route('admin.orders.updateStatus', $order) }}" method="POST" class="space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label
                                class="block text-xs font-semibold text-surface-500 uppercase tracking-widest mb-1.5">Order
                                Status</label>
                            <select name="status"
                                class="w-full rounded-xl border-surface-200 focus:border-brand-500 focus:ring-brand-500 text-sm">
                        ...
                        <div>
                            <label
                                class="block text-xs font-semibold text-surface-500 uppercase tracking-widest mb-1.5">Payment
                                Status</label>
                            <select name="payment_status"
                                class="w-full rounded-xl border-surface-200 focus:border-brand-500 focus:ring-brand-500 text-sm">
                                @foreach (['pending', 'paid', 'failed', 'refunded'] as $ps)
                                    <option value="{{ $ps }}"
                                        {{ $order->payment_status === $ps ? 'selected' : '' }}>{{ ucfirst($ps) }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div>
                        ...
                        <textarea name="admin_notes" rows="3"
                            class="w-full rounded-xl border-surface-200 focus:border-brand-500 focus:ring-brand-500">{{ $order->admin_notes }}</textarea>
                    </div>
                    <button type="submit"
                        class="w-full px-5 py-3 bg-brand-600 text-white font-semibold rounded-xl hover:bg-brand-700 transition shadow-lg shadow-brand-200">
                        Update Order
                    </button>
                </form>

                <form action="{{ route('admin.orders.undoStatus', $order) }}" method="POST" class="mt-3">
                    @csrf
                    <button type="submit" onclick="return confirm('Revert to previous order status? No customer retraction email will be sent.')"
                        class="w-full px-5 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold rounded-xl transition text-xs flex items-center justify-center gap-2 cursor-pointer border border-slate-200">
                        <svg class="w-4 h-4 text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h10a8 8 0 018 8v2M3 10l6 6m-6-6l6-6"/>
                        </svg>
                        <span>Undo Status Change</span>
                    </button>
                </form>
            </div>
            ...
            <div class="bg-white rounded-2xl border border-surface-100 shadow-card p-6">
                ...
                <div class="space-y-2 text-sm">
                    <div class="flex justify-between"><span
                            class="text-surface-500">Subtotal</span><span>{{ \App\Services\CurrencyService::formatWithCurrency($order->subtotal, $order->currency) }}</span>
                    </div>
                    @if ($order->discount > 0)
                        <div class="flex justify-between text-accent-600">
                            <span>Discount</span><span>-{{ \App\Services\CurrencyService::formatWithCurrency($order->discount, $order->currency) }}</span>
                        </div>
                    @endif
                    <div class="flex justify-between"><span
                            class="text-surface-500">Shipping</span><span>{{ \App\Services\CurrencyService::formatWithCurrency($order->shipping_cost, $order->currency) }}</span>
                    </div>
                    <div class="border-t border-surface-100 pt-2 flex justify-between font-bold text-lg">
                        <span>Total</span><span
                            class="text-brand-600">{{ \App\Services\CurrencyService::formatWithCurrency($order->total, $order->currency) }}</span>
                    </div>
                    <div class="flex justify-between mt-2">
                        <span class="text-surface-500">Payment</span>
                        <span
                            class="px-2 py-0.5 text-xs font-bold rounded {{ $order->payment_status === 'paid' ? 'bg-accent-100 text-accent-700' : 'bg-yellow-100 text-yellow-700' }}">
                            {{ ucfirst($order->payment_status) }}
                        </span>
                    </div>
                    @if ($order->payment_method)
                        <div class="flex justify-between"><span
                                class="text-surface-500">Method</span><span>{{ ucfirst($order->payment_method) }}</span>
                        </div>
                    @endif
                </div>
            </div>
                            ...
                            <div class="relative pl-4 border-l-2 border-surface-200">
                                ...
                                <p class="text-sm font-semibold text-surface-800">
                                    ...
                                    @if ($history->old_status && $history->old_status !== $history->new_status)
                                        <span class="text-xs font-normal text-surface-400"> (from
                                            {{ ucfirst($history->old_status) }})</span>
                                    @endif
                                </p>
                                <p class="text-xs text-surface-500 mt-1">
                                    {{ $history->created_at->format('M d, Y h:i A') }} @if ($history->user)
                                        by {{ $history->user->name }}
                                    @endif
                                </p>
                                @if ($history->notes)
                                    <p
                                        class="text-xs text-surface-600 mt-2 bg-surface-50 p-2 rounded border border-surface-100 italic">
                                        {{ $history->notes }}</p>
                                @endif
                            </div>
                        @endforeach
                ...
                <div class="bg-white rounded-2xl border border-surface-100 shadow-card p-6">
                    ...
                    <div class="space-y-4">
                        @foreach ($order->printLogs as $log)
                            <div
                                class="relative pl-4 border-l-2 {{ $log->status === 'sent' ? 'border-accent-400' : ($log->status === 'failed' ? 'border-red-400' : 'border-surface-200') }}">
                                <div
                                    class="absolute -left-[5px] top-1.5 w-2 h-2 rounded-full {{ $log->status === 'sent' ? 'bg-accent-500' : ($log->status === 'failed' ? 'bg-red-500' : 'bg-surface-400') }}">
                                </div>
                                <div class="flex items-center justify-between">
                                    ...
                                    <span
                                        class="px-2 py-0.5 text-[10px] font-bold rounded-lg uppercase {{ $log->status === 'sent' ? 'bg-accent-100 text-accent-700' : ($log->status === 'failed' ? 'bg-red-100 text-red-700' : 'bg-surface-100 text-surface-600') }}">
                                        {{ $log->status }}
                                    </span>
                                </div>
                                <p class="text-xs text-surface-500 mt-1">{{ $log->created_at->format('M d, h:i A') }}
                                    @if ($log->user)
                                        by {{ $log->user->name }}
                                    @endif
                                </p>
                                @if ($log->error_message)
                                    <p class="text-xs text-red-600 mt-2 bg-red-50 p-2 rounded border border-red-100">
                                        {{ $log->error_message }}</p>
                                @endif
                            </div>
                        @endforeach
...
<div id="printStoreModal" class="fixed inset-0 z-50 hidden">
    ...
    <div class="flex items-center justify-center min-h-screen p-4">
        <div
            class="relative bg-white rounded-2xl shadow-2xl max-w-lg w-full max-h-[80vh] flex flex-col animate-in z-10">
            {{-- Header --}}
            <div class="flex items-center justify-between px-6 py-5 border-b border-surface-100">
                <div class="flex items-center gap-3">
                    <div
                        class="w-10 h-10 rounded-xl bg-brand-50 border border-brand-100 flex items-center justify-center">
                ...
                <button onclick="closePrintModal()"
                    class="p-2 rounded-xl hover:bg-surface-100 text-surface-400 transition">
            ...
            <div class="px-6 py-4 border-b border-surface-50">
                <div class="relative">
                    <i data-lucide="search"
                        class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-surface-400"></i>
            ...
            <div class="flex-1 overflow-y-auto px-6 py-3" id="storeListContainer" style="max-height: 300px;">
                <div id="storeListLoading" class="py-8 text-center text-surface-400">
                    <div
                        class="w-6 h-6 border-2 border-brand-300 border-t-brand-600 rounded-full animate-spin mx-auto mb-2">
                    </div>
...
@push('scripts')
    ...
    <script>
        ...
        function sendPrint() {
            ...
            btn.disabled = true;
            btnText.innerHTML =
                '<span class="inline-flex items-center gap-2"><span class="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin"></span> Sending...</span>';
