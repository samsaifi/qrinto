...
@section('title', 'Custom Print Sizes')

@section('content')
<div class="flex items-center justify-between mb-8">
    <div>
        <h1 class="font-display font-bold text-2xl text-surface-900">Custom Print Sizes</h1>
        <p class="text-sm text-surface-500">Manage available sizes and pricing for custom print uploads</p>
    </div>
    <a href="{{ route('admin.custom-print-sizes.create') }}" class="inline-flex items-center gap-2 px-5 py-2.5 bg-brand-600 text-white font-semibold rounded-xl hover:bg-brand-700 transition shadow-lg shadow-brand-200">
        ...
        <table class="w-full">
            ...
            <tbody class="divide-y divide-surface-100">
                ...
                <tr class="hover:bg-surface-50 transition">
                    ...
                    <td class="px-6 py-4">
                        <span class="inline-flex items-center justify-center w-10 h-10 rounded-xl bg-brand-100 text-brand-700 font-bold text-sm">
                            {{ $size->key }}
                        </span>
                    </td>
                    ...
                    <td class="px-6 py-4">
                        <span class="font-bold text-surface-900">{{ \App\Services\CurrencyService::format($size->price) }}</span>
                    </td>
                    <td class="px-6 py-4">
                        @if($size->is_popular)
                    ...
                    <td class="px-6 py-4">
                        <span class="px-2.5 py-1 text-xs font-semibold rounded-lg {{ $size->is_active ? 'bg-accent-100 text-accent-700' : 'bg-surface-200 text-surface-500' }}">
                            {{ $size->is_active ? 'Active' : 'Inactive' }}
                        </span>
                    </td>
                    <td class="px-6 py-4 text-right">
                        <div class="flex items-center justify-end gap-2">
                            <a href="{{ route('admin.custom-print-sizes.edit', $size) }}" class="p-2 rounded-lg hover:bg-surface-100 text-surface-500 hover:text-brand-600 transition" title="Edit">
                                <i data-lucide="pencil" class="w-4 h-4"></i>
                            </a>
                            <form action="{{ route('admin.custom-print-sizes.destroy', $size) }}" method="POST" onsubmit="return confirm('Delete this size? This cannot be undone.')">
                                @csrf @method('DELETE')
                                <button type="submit" class="p-2 rounded-lg hover:bg-red-50 text-surface-500 hover:text-red-600 transition" title="Delete">
                ...
                <tr>
                    <td colspan="8" class="px-6 py-12 text-center">
                        <div class="flex flex-col items-center gap-3">
                            ...
                            <p class="text-surface-400">No custom print sizes configured yet.</p>
                            <a href="{{ route('admin.custom-print-sizes.create') }}" class="text-sm text-brand-600 hover:text-brand-700 font-semibold">Add your first size →</a>
                        </div>
                    </td>
                </tr>
                @endforelse
