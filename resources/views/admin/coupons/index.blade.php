...
@section('content')
    ...
    <div class="flex items-center justify-between mb-8">
        ...
        <a href="{{ route($rPrefix . 'coupons.create') }}"
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                ...
                <tbody class="divide-y divide-surface-100">
                    @forelse($coupons as $coupon)
                        <tr class="hover:bg-surface-50 transition">
                            ...
                            <td class="px-6 py-4 text-sm">
                                ...
                                @else
                                    <span
                                        class="text-brand-600 font-semibold">{{ \App\Services\CurrencyService::format($coupon->value, 0) }}</span>
                                @endif
                            </td>
                            <td class="px-6 py-4 text-sm text-surface-600">{{ $coupon->used_count ?? 0 }} /
                                {{ $coupon->usage_limit ?? '∞' }}</td>
                            <td class="px-6 py-4 text-sm text-surface-500">
                                {{ $coupon->expires_at ? $coupon->expires_at->format('M d, Y') : '-' }}</td>
                            <td class="px-6 py-4 text-sm text-surface-600">{{ $coupon->store->store_name ?? 'All Stores' }}
                            </td>
                            <td class="px-6 py-4 text-sm text-surface-600">{{ $coupon->user->name ?? '-' }}</td>
                            <td class="px-6 py-4">
                                <span
                                    class="px-2.5 py-1 text-xs font-semibold rounded-lg {{ $coupon->is_active ? 'bg-accent-100 text-accent-700' : 'bg-surface-200 text-surface-500' }}">
                                    {{ $coupon->is_active ? 'Active' : 'Inactive' }}
                                </span>
                            </td>
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <a href="{{ route($rPrefix . 'coupons.edit', $coupon) }}"
                                        class="p-2 rounded-lg hover:bg-surface-100 text-surface-500 hover:text-brand-600 transition">
                                        <i data-lucide="pencil" class="w-4 h-4"></i>
                                    </a>
                                    <form action="{{ route($rPrefix . 'coupons.destroy', $coupon) }}" method="POST"
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="px-6 py-12 text-center text-surface-400">No coupons yet.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
