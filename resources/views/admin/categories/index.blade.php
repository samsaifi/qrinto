...
@section('content')
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        ...
        <a href="{{ route($rPrefix . 'categories.create') }}"
    ...
    <div class="bg-white rounded-2xl border border-surface-100 shadow-card p-5 mb-6">
        <form action="{{ route($rPrefix . 'categories.index') }}" method="GET" class="flex flex-wrap gap-4 items-end">
            ...
            <a href="{{ route($rPrefix . 'categories.index') }}"
        ...
        <div class="overflow-x-auto">
            <table class="w-full min-w-[600px]">
                <thead>
                    <tr class="bg-surface-50 border-b border-surface-100">
                        <th
                            ...
                            Image</th>
                        <th class="px-5 py-3 text-left text-xs font-semibold text-surface-500 uppercase tracking-wider">Name
                        </th>
                        <th
                        ...
                        <th
                            ...
                            Parent</th>
                        <th
                            class="px-3 py-3 text-center text-xs font-semibold text-surface-500 uppercase tracking-wider hidden sm:table-cell w-24">
                            Products</th>
                        <th
                            class="px-3 py-3 text-left text-xs font-semibold text-surface-500 uppercase tracking-wider hidden md:table-cell">
                            Created By</th>
                ...
                <tbody class="divide-y divide-surface-50">
                    @forelse($categories as $category)
                        <tr class="hover:bg-surface-50/60 transition-colors">
                            <td class="px-3 py-3 text-center">
                                ...
                                @else
                                    <span
                                        class="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-surface-100 text-surface-400">
                            ...
                            <td class="px-3 py-3 text-center">
                                <span
                                    class="inline-flex px-2 py-0.5 text-xs font-semibold rounded-md {{ $category->is_active ? 'bg-accent-100 text-accent-700' : 'bg-surface-200 text-surface-500' }}">
                            ...
                            <td class="px-3 py-3 hidden md:table-cell">
                                @if ($category->parent)
                                    <span
                                        class="inline-flex px-2 py-0.5 text-xs font-medium rounded-md bg-brand-50 text-brand-700 border border-brand-200/60">{{ $category->parent->name }}</span>
                                @else
                            ...
                            <td class="px-3 py-3 text-center">
                                <div class="flex items-center justify-center gap-1">
                                    <a href="{{ route($rPrefix . 'categories.edit', $category) }}"
                                        class="inline-flex p-1.5 rounded-lg hover:bg-brand-50 text-surface-400 hover:text-brand-600 transition"
                                        title="Edit">
                                        <i data-lucide="pencil" class="w-4 h-4"></i>
                                    </a>
                                    <form action="{{ route($rPrefix . 'categories.destroy', $category) }}" method="POST"
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="px-6 py-12 text-center text-surface-400">No categories yet. <a
                                    href="{{ route($rPrefix . 'categories.create') }}"
                                    class="text-brand-600 font-medium">Add your first category</a>.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
