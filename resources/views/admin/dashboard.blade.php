...
@section('content')
    @if (auth()->user()->isStoreAdmin() && $store)
     ...
     ═══════════════════════════════════════════════════════ --}}
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8 animate-fade-in">
                ...
                <div class="relative px-6 py-8 sm:px-8 sm:py-10 flex flex-col justify-between h-full">
                    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
                        ...
                        <div class="flex items-center gap-5">
                            @if ($store->logo)
                                <div
                                    class="w-16 h-16 rounded-2xl overflow-hidden ring-4 ring-white/20 shadow-lg flex-shrink-0">
                                    <img src="{{ asset('storage/' . $store->logo) }}" alt="{{ $store->store_name }}"
                                        class="w-full h-full object-cover">
                                </div>
                            @else
                                <div
                                    class="w-16 h-16 rounded-2xl bg-white/15 backdrop-blur-sm flex items-center justify-center flex-shrink-0 ring-4 ring-white/10">
                            ...
                            <div>
                                ...
                                <h1 class="font-display font-bold text-2xl sm:text-3xl text-white mt-1">
                                    {{ auth()->user()->name }}</h1>
                        ...
                        <div class="flex items-center gap-3">
                            <a href="{{ route('admin.orders.index') }}"
                                class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white/15 backdrop-blur-sm text-white text-sm font-medium hover:bg-white/25 transition-all border border-white/10">
                                <i data-lucide="package" class="w-4 h-4"></i>
                                View Orders
                            </a>
                            <a href="{{ route('admin.stores.edit', $store->id) }}"
                                class="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white text-brand-700 text-sm font-semibold hover:bg-brand-50 transition-all shadow-lg">
                    ...
                    @if ($recentOrders->count() > 0)
                        ...
                        <div class="mt-6">
                            <div class="flex items-center justify-between mb-3">
                                <p
                                    class="text-white/80 text-xs font-semibold uppercase tracking-wider flex items-center gap-1.5">
                                    <i data-lucide="package" class="w-3.5 h-3.5"></i>
                                    Latest Order
                                </p>
                                <a href="{{ route('admin.orders.index') }}"
                                    class="text-white/60 text-xs hover:text-white transition">View all →</a>
                            </div>
                            <a href="{{ route('admin.orders.show', $latestOrder) }}"
                                class="block bg-white/10 backdrop-blur-sm rounded-xl border border-white/10 p-4 hover:bg-white/15 transition-all group">
                                <div class="flex items-start gap-4">
                                    ...
                                    <div class="flex -space-x-2 flex-shrink-0">
                                        @php
                                            ...
                                            foreach ($latestOrder->items as $item) {
                                                if (
                                                    !empty($item->uploaded_images) &&
                                                    is_array($item->uploaded_images)
                                                ) {
                                        ...
                                        @forelse($displayImages as $imgUrl)
                                            <div
                                                class="w-14 h-14 rounded-xl overflow-hidden ring-2 ring-white/20 shadow-lg flex-shrink-0 bg-white/10">
                                                <img src="{{ $imgUrl }}" alt="Order image"
                                                    class="w-full h-full object-cover">
                                            </div>
                                        @empty
                                            <div
                                                class="w-14 h-14 rounded-xl bg-white/10 ring-2 ring-white/10 flex items-center justify-center flex-shrink-0">
                                                <i data-lucide="image" class="w-6 h-6 text-white/30"></i>
                                            </div>
                                        @endforelse
                                        @if ($extraCount > 0)
                                            <div
                                                class="w-14 h-14 rounded-xl bg-white/20 ring-2 ring-white/10 flex items-center justify-center flex-shrink-0 backdrop-blur-sm">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <div class="flex items-center gap-2 mb-1">
                                            <span
                                                class="font-mono text-sm font-bold text-white group-hover:text-brand-200 transition">{{ $latestOrder->order_number }}</span>
                                            @php
                                                $statusColors = [
                                                ...
                                                $sClass =
                                                    $statusColors[$latestOrder->status] ?? 'bg-white/10 text-white/70';
                                            @endphp
                                            <span
                                                class="inline-flex px-2 py-0.5 rounded-full text-[10px] font-bold {{ $sClass }}">
                                        ...
                                        <p class="text-white/60 text-xs mb-1">
                                            ...
                                            · {{ $latestOrder->items->count() }}
                                            {{ Str::plural('item', $latestOrder->items->count()) }}
                                        </p>
                                        <div class="flex items-center justify-between">
                                            <span
                                                class="text-white font-bold text-lg">{{ \App\Services\CurrencyService::formatWithCurrency($latestOrder->total, $latestOrder->currency) }}</span>
                                            <span
                                                class="text-white/40 text-xs">{{ $latestOrder->created_at->diffForHumans() }}</span>
                                        </div>
                                    </div>
                                    ...
                                    <div class="flex-shrink-0 self-center">
                                        <i data-lucide="chevron-right"
                                            class="w-5 h-5 text-white/30 group-hover:text-white/60 transition"></i>
                                    </div>
                                </div>
                            </a>
                        </div>
            ...
            {{-- ── QR Code Card (1/3) ────────────────────────────── --}}
            <div
                class="lg:col-span-1 bg-white rounded-2xl border border-surface-100 shadow-card flex flex-col items-center justify-center p-6 text-center">
                {{-- Badge --}}
                <div
                    class="inline-flex items-center gap-1.5 px-4 py-1.5 bg-gradient-to-r from-brand-600 to-brand-700 rounded-full text-white text-[11px] font-bold uppercase tracking-wider mb-4 shadow-lg shadow-brand-200">
                ...
                {{-- QR Code Container --}}
                <div id="dashboard-qr-container"
                    class="bg-surface-50 rounded-xl p-3 border border-surface-100 inline-block mb-4">
                ...
                {{-- URL Display --}}
                <div class="w-full font-mono text-[11px] text-surface-400 bg-surface-50 px-3 py-2 rounded-lg border border-surface-100 truncate mb-4"
                    title="{{ url('store/' . $store->store_code) }}">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6 mb-8">
            ...
            @foreach ($stats as $stat)
                <div
                    class="bg-white rounded-2xl border border-surface-100 shadow-card p-6 hover:shadow-hover transition-shadow duration-300">
    ...
    @if (auth()->user()->isAdmin())
        ...
        <div class="bg-white rounded-2xl border border-surface-100 shadow-card">
                ...
                <div class="flex flex-wrap items-end gap-3">
                    <div class="flex flex-wrap items-center gap-1.5">
                        <button type="button" data-preset="all"
                            class="store-preset-btn active px-3 py-1.5 text-xs font-semibold rounded-lg bg-brand-600 text-white transition">All
                            Time</button>
                        <button type="button" data-preset="today"
                            class="store-preset-btn px-3 py-1.5 text-xs font-semibold rounded-lg bg-surface-100 text-surface-600 hover:bg-surface-200 transition">Today</button>
                        <button type="button" data-preset="week"
                            class="store-preset-btn px-3 py-1.5 text-xs font-semibold rounded-lg bg-surface-100 text-surface-600 hover:bg-surface-200 transition">This
                            Week</button>
                        <button type="button" data-preset="month"
                            class="store-preset-btn px-3 py-1.5 text-xs font-semibold rounded-lg bg-surface-100 text-surface-600 hover:bg-surface-200 transition">This
                            Month</button>
                        <button type="button" data-preset="year"
                            class="store-preset-btn px-3 py-1.5 text-xs font-semibold rounded-lg bg-surface-100 text-surface-600 hover:bg-surface-200 transition">This
                            Year</button>
                    </div>

                    <div class="flex items-center gap-2">
                        <input type="date" id="store-date-start"
                            class="px-3 py-1.5 text-xs border border-surface-200 rounded-lg focus:ring-2 focus:ring-brand-500 focus:border-brand-500 outline-none">
                        <span class="text-xs text-surface-400">to</span>
                        <input type="date" id="store-date-end"
                            class="px-3 py-1.5 text-xs border border-surface-200 rounded-lg focus:ring-2 focus:ring-brand-500 focus:border-brand-500 outline-none">
                        <button type="button" id="store-date-apply"
                            class="px-3 py-1.5 text-xs font-semibold rounded-lg bg-brand-100 text-brand-700 hover:bg-brand-200 transition">Apply</button>
                    </div>

                    <select id="store-filter-select"
                        class="px-3 py-1.5 text-xs border border-surface-200 rounded-lg focus:ring-2 focus:ring-brand-500 focus:border-brand-500 outline-none min-w-[160px]">
                    ...
                    <button type="button" id="store-export-excel"
                        class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-lg bg-accent-600 text-white hover:bg-accent-700 transition">
                        <i data-lucide="download" class="w-3.5 h-3.5"></i>
                        Download Excel
                    </button>
                </div>
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="bg-surface-50">
                            ...
                            <th class="px-4 py-3 text-left text-xs font-semibold text-surface-500 uppercase cursor-pointer select-none summary-sort-col"
                            ...
                            <th class="px-4 py-3 text-right text-xs font-semibold text-surface-500 uppercase cursor-pointer select-none summary-sort-col"
                                data-col="cash_amount">Cash Payment <span class="sort-arrow"></span></th>
                            <th class="px-4 py-3 text-right text-xs font-semibold text-surface-500 uppercase cursor-pointer select-none summary-sort-col"
                                data-col="pending_orders">Pending Orders <span class="sort-arrow"></span></th>
                            <th class="px-4 py-3 text-right text-xs font-semibold text-surface-500 uppercase cursor-pointer select-none summary-sort-col"
                                data-col="pending_amount">Pending Amt <span class="sort-arrow"></span></th>
                            <th class="px-4 py-3 text-right text-xs font-semibold text-surface-500 uppercase cursor-pointer select-none summary-sort-col"
                                data-col="paid_orders">Paid Orders <span class="sort-arrow"></span></th>
                            <th class="px-4 py-3 text-right text-xs font-semibold text-surface-500 uppercase cursor-pointer select-none summary-sort-col"
                                data-col="paid_amount">Paid Amt <span class="sort-arrow"></span></th>
                        </tr>
                    </thead>
            ...
            <div id="stores-summary-pagination"
            ...
            <div class="p-6 border-b border-surface-100">
                ...
                <div class="flex flex-wrap items-end gap-3">
                    <div class="flex items-center gap-1.5">
                        <button type="button" data-preset="all"
                            class="stats-preset-btn active px-3 py-1.5 text-xs font-semibold rounded-lg bg-brand-600 text-white transition">All
                            Time</button>
                        <button type="button" data-preset="today"
                            class="stats-preset-btn px-3 py-1.5 text-xs font-semibold rounded-lg bg-surface-100 text-surface-600 hover:bg-surface-200 transition">Today</button>
                        <button type="button" data-preset="week"
                            class="stats-preset-btn px-3 py-1.5 text-xs font-semibold rounded-lg bg-surface-100 text-surface-600 hover:bg-surface-200 transition">This
                            Week</button>
                        <button type="button" data-preset="month"
                            class="stats-preset-btn px-3 py-1.5 text-xs font-semibold rounded-lg bg-surface-100 text-surface-600 hover:bg-surface-200 transition">This
                            Month</button>
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <input type="date" id="stats-date-end"
                            class="px-3 py-1.5 text-xs border border-surface-200 rounded-lg focus:ring-2 focus:ring-brand-500 focus:border-brand-500 outline-none">
                        <button type="button" id="stats-date-apply"
                            class="px-3 py-1.5 text-xs font-semibold rounded-lg bg-brand-100 text-brand-700 hover:bg-brand-200 transition">Apply</button>
                    </div>

                    <select id="stats-store-select"
                        class="px-3 py-1.5 text-xs border border-surface-200 rounded-lg focus:ring-2 focus:ring-brand-500 focus:border-brand-500 outline-none min-w-[160px]">
                        <option value="">All Stores</option>
                    ...
                    <button type="button" id="stats-export-excel"
                        class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-lg bg-accent-600 text-white hover:bg-accent-700 transition">
                        <i data-lucide="download" class="w-3.5 h-3.5"></i>
                        Download Excel
                    </button>
                ...
                <table class="w-full">
                    <thead>
                        <tr class="bg-surface-50">
                            ...
                            <th class="px-4 py-3 text-right text-xs font-semibold text-surface-500 uppercase cursor-pointer select-none stats-sort-col"
                                data-col="products">Products <span class="sort-arrow">&#9662;</span></th>
                            <th class="px-4 py-3 text-right text-xs font-semibold text-surface-500 uppercase cursor-pointer select-none stats-sort-col"
                                data-col="categories">Categories <span class="sort-arrow"></span></th>
                            <th class="px-4 py-3 text-right text-xs font-semibold text-surface-500 uppercase cursor-pointer select-none stats-sort-col"
                                data-col="card_types">Card Types <span class="sort-arrow"></span></th>
                            <th class="px-4 py-3 text-right text-xs font-semibold text-surface-500 uppercase cursor-pointer select-none stats-sort-col"
                                data-col="templates">Templates <span class="sort-arrow"></span></th>
                            <th class="px-4 py-3 text-right text-xs font-semibold text-surface-500 uppercase cursor-pointer select-none stats-sort-col"
                                data-col="coupons">Coupons <span class="sort-arrow"></span></th>
                            <th class="px-4 py-3 text-right text-xs font-semibold text-surface-500 uppercase cursor-pointer select-none stats-sort-col"
                                data-col="events">Events <span class="sort-arrow"></span></th>
                            <th class="px-4 py-3 text-right text-xs font-semibold text-surface-500 uppercase cursor-pointer select-none stats-sort-col"
                                data-col="paper_types">Paper Types <span class="sort-arrow"></span></th>
                        </tr>
                    </thead>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function fetchStores(page) {
                ...
                <tr class="bg-surface-50 font-bold">
                    ...
                    <td class="px-4 py-3 text-right text-sm text-accent-800">${fmtNum(t.paid_amount)}</td>
                        ...
                        if (data.last_page <= 1) {
                            pag.innerHTML =
                                `<span class="text-xs text-surface-400">${data.total} store${data.total !== 1 ? 's' : ''}</span><span></span>`;
                        } else {
                            let btns = '';
                            for (let p = 1; p <= data.last_page; p++) {
                                btns +=
                                    `<button data-page="${p}" class="px-3 py-1 text-xs rounded-lg font-semibold ${p === data.current_page ? 'bg-brand-600 text-white' : 'bg-surface-100 text-surface-600 hover:bg-surface-200'} transition">${p}</button>`;
                            }
                            pag.innerHTML =
                                `<span class="text-xs text-surface-400">Page ${data.current_page} of ${data.last_page} (${data.total} stores)</span><div class="flex gap-1">${btns}</div>`;
                            pag.querySelectorAll('[data-page]').forEach(btn => {
                                btn.addEventListener('click', () => fetchStores(parseInt(btn.dataset
                                    .page)));
                            });
                        }
                    });
            ...
            document.getElementById('store-date-apply').addEventListener('click', () => {
                if (document.getElementById('store-date-start').value && document.getElementById(
                        'store-date-end').value) {
                    document.querySelectorAll('.store-preset-btn').forEach(b => {
                        b.classList.remove('active', 'bg-brand-600', 'text-white');
                        b.classList.add('bg-surface-100', 'text-surface-600');
                    });
            ...
            document.querySelectorAll('.summary-sort-col').forEach(th => {
                th.addEventListener('click', () => {
                    ...
                    document.querySelectorAll('.summary-sort-col .sort-arrow').forEach(a => a
                        .innerHTML = '');
            ...
            function fetchStats(page) {
                ...
                fetch(statsUrl + '?' + params.toString(), {
                    ...
                    .then(data => {
                        ...
                        if (!data.data.length) {
                            tbody.innerHTML =
                                '<tr><td colspan="9" class="px-6 py-12 text-center text-surface-400">No data found.</td></tr>';
                        } else {
                            const si = (data.current_page - 1) * 10;
                            tbody.innerHTML = data.data.map((s, i) => `
                    <tr class="hover:bg-surface-50 transition">
                ...
                <tr class="bg-surface-50 font-bold">
                    ...
                    <td class="px-4 py-3 text-right text-sm text-surface-900">${t.paper_types}</td>
                        ...
                        if (data.last_page <= 1) {
                            pag.innerHTML =
                                `<span class="text-xs text-surface-400">${data.total} row${data.total !== 1 ? 's' : ''}</span><span></span>`;
                        } else {
                            let btns = '';
                            for (let p = 1; p <= data.last_page; p++) {
                                btns +=
                                    `<button data-spage="${p}" class="px-3 py-1 text-xs rounded-lg font-semibold ${p === data.current_page ? 'bg-brand-600 text-white' : 'bg-surface-100 text-surface-600 hover:bg-surface-200'} transition">${p}</button>`;
                            }
                            pag.innerHTML =
                                `<span class="text-xs text-surface-400">Page ${data.current_page} of ${data.last_page} (${data.total} rows)</span><div class="flex gap-1">${btns}</div>`;
                            pag.querySelectorAll('[data-spage]').forEach(btn => {
                                btn.addEventListener('click', () => fetchStats(parseInt(btn.dataset
                                    .spage)));
                            });
                        }
                    });
            ...
            document.getElementById('stats-date-apply').addEventListener('click', () => {
                if (document.getElementById('stats-date-start').value && document.getElementById(
                        'stats-date-end').value) {
                    document.querySelectorAll('.stats-preset-btn').forEach(b => {
                        b.classList.remove('active', 'bg-brand-600', 'text-white');
                        b.classList.add('bg-surface-100', 'text-surface-600');
                    });
            ...
            document.querySelectorAll('.stats-sort-col').forEach(th => {
                th.addEventListener('click', () => {
                    ...
                    document.querySelectorAll('.stats-sort-col .sort-arrow').forEach(a => a
                        .innerHTML = '');
