...
@section('content')
    @php
        ...
        $rangeLabel =
            $preset === 'custom' && $dateStart && $dateEnd
                ? \Carbon\Carbon::parse($dateStart)->format('M j, Y') .
                    ' – ' .
                    \Carbon\Carbon::parse($dateEnd)->format('M j, Y')
    ...
    <div class="flex flex-wrap items-start justify-between gap-4 mb-6">
        <div>
            ...
            <p class="text-sm text-surface-500 mt-0.5">
                ...
                <span
                    class="inline-flex items-center gap-1 ml-2 px-2 py-0.5 rounded-full bg-brand-50 text-brand-700 border border-brand-200 text-[11px] font-semibold">
                    <i data-lucide="calendar" class="w-3 h-3"></i> {{ $rangeLabel }}
                </span>
                <span
                    class="inline-flex items-center gap-1 ml-1 px-2 py-0.5 rounded-full bg-surface-100 text-surface-700 border border-surface-200 text-[11px] font-semibold">
    ...
    <div class="bg-white rounded-2xl border border-surface-100 shadow-card p-5 mb-5">
        <form method="GET" action="{{ route('admin.print-logs.index') }}" x-data="{ preset: '{{ $preset }}' }" class="space-y-4">
            ...
            'custom' => 'Custom',
                        ...
                        <button type="button" @click="preset = '{{ $key }}'"
                            :class="preset === '{{ $key }}' ? 'bg-brand-600 text-white' :
                ...
                <div class="ml-auto">
                    ...
                    <select name="store_id"
                        class="rounded-xl border-surface-200 text-sm focus:border-brand-500 focus:ring-brand-500 min-w-[190px]">
                        <option value="all">All stores</option>
                        @foreach ($stores as $s)
                            <option value="{{ $s->id }}" @selected($storeId === (string) $s->id)>
                                {{ $s->store_name }}@if ($s->is_test)
                                    (Test)
                                @endif
                            </option>
                        @endforeach
    ...
    @php
        $cards = [
            [
                'label' => 'Total New',
                'value' => $analytics['new_orders'],
                'icon' => 'inbox',
                'tint' => 'bg-blue-50 text-blue-700 border-blue-200',
            ],
            [
                'label' => 'Total Printing',
                'value' => $analytics['printing_orders'],
                'icon' => 'printer',
                'tint' => 'bg-purple-50 text-purple-700 border-purple-200',
            ],
            [
                'label' => 'Total Ready for Pickup',
                'value' => $analytics['ready_orders'],
                'icon' => 'package-check',
                'tint' => 'bg-amber-50 text-amber-700 border-amber-200',
            ],
            [
                'label' => 'Total Picked Up',
                'value' => $analytics['picked_up_orders'],
                'icon' => 'check-circle-2',
                    ...
                    @forelse ($logs as $log)
                        <tr class="hover:bg-surface-50/60 transition">
                            <td class="px-6 py-3 text-sm text-surface-700 whitespace-nowrap">
                                ...
                                <div class="text-xs text-surface-400 font-mono">{{ $log->printed_at?->format('H:i:s') }}
                                </div>
                            </td>
                            <td class="px-6 py-3 text-sm">
                                <span
                                    class="font-mono font-semibold text-surface-800">{{ $log->order_number ?? '-' }}</span>
                            </td>
                            ...
                            <td class="px-6 py-3">
                                ...
                                @endphp
                                <span
                                    class="inline-flex items-center px-2.5 py-1 text-xs font-semibold rounded-lg border {{ $chip }}">
                                    {{ ucfirst($log->status) }}
                                </span>
                            </td>
