...
@section('content')
    <div class="max-w-2xl">
        <h1 class="font-display font-bold text-2xl text-surface-900 mb-8">
            {{ isset($paperType) ? 'Edit Paper Type' : 'Create Paper Type' }}</h1>

        <form
            action="{{ isset($paperType) ? route('admin.paper-types.update', $paperType) : route('admin.paper-types.store') }}"
            ...
            <div class="bg-white rounded-2xl border border-surface-100 shadow-card p-6 space-y-5">
                ...
                <label class="flex items-center gap-2 text-sm text-surface-700">
                    <input type="checkbox" name="is_active" value="1"
                        {{ old('is_active', $paperType->is_active ?? true) ? 'checked' : '' }}
                ...
                <div>
                    ...
                    @if (auth()->user()->isAdmin())
                        ...
                        @include('admin.paper-types._multiselect', [
                            ...
                            'selectedIds' => old(
                                'stores',
                                isset($paperType)
                                    ? $paperType->stores->pluck('id')->map(fn($id) => (string) $id)->toArray()
                                    : []),
                            'fieldName' => 'stores',
                    ...
                    @else
                        ...
                        <p class="text-xs text-surface-400 mt-1">Paper type will be assigned to your store automatically.
                        </p>
                    @endif
                </div>
            </div>
            ...
            <div class="flex gap-3">
                ...
                <a href="{{ route('admin.paper-types.index') }}"
