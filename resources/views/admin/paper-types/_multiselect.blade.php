@php
    $tagBg = match ($tagColor ?? 'brand') {
        'emerald' => ' bg-gray-50  text-gray-700',
        default => 'bg-brand-50 text-brand-700',
    };
    $highlightBg = match ($tagColor ?? 'brand') {
        'emerald' => ' bg-gray-50/50',
...
<div id="{{ $uid }}" x-data="{
    ...
    data-selected='@json(collect($selectedIds)->map(fn($id) => (string) $id)->values())' class="relative">
    {{-- Trigger --}}
    <div @click="open = !open" @click.outside="open = false"
        ...
        <i data-lucide="chevron-down" class="w-4 h-4 text-surface-400 ml-auto flex-shrink-0 transition-transform"
    ...
    <div x-show="open" x-transition.opacity.duration.150ms
        ...
        <div class="max-h-48 overflow-y-auto p-1">
            <label
                class="flex items-center gap-2 text-sm text-surface-700 px-2 py-1.5 rounded-lg hover:bg-surface-50 cursor-pointer border-b border-surface-100 mb-1"
                @click.stop>
            ...
            <template x-for="item in filteredItems" :key="item.id">
                <label
                    class="flex items-center gap-2 text-sm text-surface-700 px-2 py-1.5 rounded-lg hover:bg-surface-50 cursor-pointer"
            ...
            <p x-show="filteredItems.length === 0" class="text-sm text-surface-400 px-2 py-3 text-center">No results
                found.</p>
        </div>
    </div>
