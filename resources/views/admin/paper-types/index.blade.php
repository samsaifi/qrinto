...
@section('content')
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        ...
        <a href="{{ route('admin.paper-types.create') }}"
        ...
        <div class="overflow-x-auto">
            <table class="w-full min-w-[600px]">
                ...
                <tbody class="divide-y divide-surface-50">
                    @forelse($paperTypes as $paperType)
                        <tr class="hover:bg-surface-50/60 transition-colors">
                            ...
                            <td class="px-3 py-3 text-center">
                                <span
                                    class="inline-flex px-2 py-0.5 text-xs font-semibold rounded-md {{ $paperType->is_active ? 'bg-accent-100 text-accent-700' : 'bg-surface-200 text-surface-500' }}">
                            ...
                            <td class="px-3 py-3 hidden sm:table-cell">
                                <span
                                    class="text-sm text-surface-500">{{ $paperType->created_at->format('M d, Y') }}</span>
                            </td>
                            <td class="px-3 py-3 text-center">
                                <div class="flex items-center justify-center gap-1">
                                    <a href="{{ route('admin.paper-types.edit', $paperType) }}"
                                        class="inline-flex p-1.5 rounded-lg hover:bg-brand-50 text-surface-400 hover:text-brand-600 transition"
                                        title="Edit">
                                        <i data-lucide="pencil" class="w-4 h-4"></i>
                                    </a>
                                    <form action="{{ route('admin.paper-types.destroy', $paperType) }}" method="POST"
                                        onsubmit="return confirm('Delete this paper type?')">
                                        @csrf @method('DELETE')
                                        <button type="submit"
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-6 py-12 text-center text-surface-400">No paper types yet. <a
                                    href="{{ route('admin.paper-types.create') }}" class="text-brand-600 font-medium">Add
                                    your first paper type</a>.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
