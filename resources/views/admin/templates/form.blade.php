...
.prop-label { font-size:11px; font-weight:600; text-transform:uppercase; letter-spacing:.05em; color:#64748b; }
.prop-input { width:100%; padding:6px 10px; border:1px solid #e2e8f0; border-radius:8px; font-size:13px; outline:none; }
.prop-input:focus { border-color:#6366f1; box-shadow:0 0 0 3px rgba(99,102,241,.1); }
.builder-btn { display:inline-flex; align-items:center; gap:6px; padding:6px 12px; border-radius:8px; font-size:13px; font-weight:600; cursor:pointer; border:1px solid #e2e8f0; background:#fff; color:#475569; transition:all .15s; }
.builder-btn:hover { background:#f8fafc; border-color:#cbd5e1; }
.builder-btn.primary { background:#6366f1; border-color:#6366f1; color:#fff; }
.builder-btn.primary:hover { background:#4f46e5; }
.builder-btn.danger  { background:#fee2e2; border-color:#fca5a5; color:#dc2626; }
.builder-btn.danger:hover  { background:#fecaca; }
.align-btn { padding:5px 8px; border:1px solid #e2e8f0; border-radius:6px; cursor:pointer; background:#fff; line-height:1; }
...
<form id="template-form" method="POST"
    ...
    <div class="grid grid-cols-1 xl:grid-cols-5 gap-6">
            ...
            <div class="bg-white rounded-2xl border border-surface-100 shadow-card p-5">
                ...
                <div class="flex flex-wrap gap-2 mb-4" id="toolbar">
                    <button type="button" class="builder-btn" onclick="TB.addText()">
                        <svg viewBox="0 0 16 16" width="14" height="14" fill="currentColor"><path d="M1 2h14v2H9v10H7V4H1V2z"/></svg>
                        Text
                    </button>
                    <label class="builder-btn cursor-pointer">
                        <svg viewBox="0 0 16 16" width="14" height="14" fill="none" stroke="currentColor" stroke-width="1.5"><rect x="1" y="3" width="14" height="10" rx="1.5"/><path d="M1 11l4-4 3 3 2-2 4 4"/><circle cx="11.5" cy="6.5" r="1.5"/></svg>
                        Image
                        <input type="file" accept="image/jpeg,image/png,image/webp" class="sr-only" onchange="TB.uploadAndAdd(this,'image')">
                    </label>
                    <label class="builder-btn cursor-pointer">
                        <svg viewBox="0 0 16 16" width="14" height="14" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M8 1l2 5h5l-4 3 1.5 5L8 11l-4.5 3L5 9 1 6h5z"/></svg>
                        SVG
                        <input type="file" accept=".svg,image/svg+xml" class="sr-only" onchange="TB.uploadAndAdd(this,'svg')">
                    </label>
                    <div class="flex-1"></div>
                    <button type="button" class="builder-btn danger" onclick="TB.deleteSelected()">
                        <svg viewBox="0 0 16 16" width="14" height="14" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M2 4h12M5 4V2h6v2M6 7v5M10 7v5M3 4l1 10h8l1-10"/></svg>
                        Delete
                    </button>
                    <button type="button" class="builder-btn" onclick="TB.clearCanvas()" title="Clear all objects">
                        <svg viewBox="0 0 16 16" width="14" height="14" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M1 1l14 14M11 5a4 4 0 0 0-6 0L2 8a4 4 0 0 0 6 6l3-3M14 8a4 4 0 0 0-4-4"/></svg>
                        Clear
                    </button>
                </div>
        ...
        <div class="xl:col-span-2 space-y-4">
            ...
            <div class="bg-white rounded-2xl border border-surface-100 shadow-card p-5 space-y-4">
                ...
                <div id="icon-lucide-row" class="prop-row">
                    ...
                    <div class="flex gap-2 items-center">
                        ...
                        <div id="icon-preview" class="w-8 h-8 rounded-lg bg-surface-100 flex items-center justify-center flex-shrink-0">
                            <i id="icon-preview-el" data-lucide="layout-template" class="w-4 h-4 text-surface-500"></i>
                        </div>
                    </div>
                </div>
                <div id="icon-upload-row" class="prop-row hidden">
                    ...
                    @if(isset($template) && $template->icon_type === 'upload' && $template->icon_value)
                        <div class="flex items-center gap-3 mb-2">
                            <img src="{{ asset('storage/' . $template->icon_value) }}" alt="" class="w-8 h-8 object-contain">
                            <span class="text-xs text-surface-500">Current icon — upload new to replace</span>
                        </div>
                    @endif
                    <input id="icon_file" name="icon_file" type="file" accept=".svg,image/svg+xml" class="prop-input text-sm py-1.5">
                </div>
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div class="prop-row">
                        ...
                        <select id="apply_to" name="apply_to" class="prop-input">
                            <option value="active" {{ $cfgApplyTo === 'active' ? 'selected' : '' }}>Active page</option>
                ...
                <div class="flex gap-4">
                    ...
                    @endphp
                    <label class="flex items-center gap-2 cursor-pointer text-sm">
                        <input type="checkbox" name="replace" id="replace" value="1" class="rounded text-brand-600"
                               {{ $cfgReplace ? 'checked' : '' }}>
                        Replace existing layers
                    </label>
                    <label class="flex items-center gap-2 cursor-pointer text-sm">
                        <input type="checkbox" name="ignore_mask" id="ignore_mask" value="1" class="rounded text-brand-600"
                               {{ $cfgIgnoreMask ? 'checked' : '' }}>
                        Ignore mask clip
                    </label>
                </div>
            ...
            {{-- No selection hint --}}
            <div id="no-selection-hint" class="bg-surface-50 rounded-2xl border border-surface-100 p-5 text-center text-sm text-surface-400">
...
(function () {
    ...
    function clearCanvas() {
        ...
        canvas.getObjects().forEach(o => { if (!o._isGuide) canvas.remove(o); });
        canvas.discardActiveObject();
        canvas.renderAll();
        onSelectionCleared();
    }
    ...
    function bindMediaProps(obj, type) {
        ...
        q('#prop-locked').addEventListener('change', e => {
            ...
            obj.set({ selectable: !e.target.checked, evented: !e.target.checked });
            canvas.renderAll();
        });
    }
    ...
    function refreshIconPreview() {
        ...
        iconDebounce = setTimeout(() => { if (window.lucide) lucide.createIcons({ nodes: [el] }); }, 300);
    }
    ...
    function alignIcon(a) {
        if (a === 'left')   return '<rect x="1" y="2" width="12" height="2" rx="1"/><rect x="1" y="6" width="8" height="2" rx="1"/><rect x="1" y="10" width="10" height="2" rx="1"/>';
        if (a === 'right')  return '<rect x="1" y="2" width="12" height="2" rx="1"/><rect x="5" y="6" width="8" height="2" rx="1"/><rect x="3" y="10" width="10" height="2" rx="1"/>';
    ...
    window.TB = { addText, uploadAndAdd, deleteSelected, clearCanvas, toggleIconType };
})();
</script>
@endpush
