...
<div class="flex items-center justify-between mb-8">
    ...
    <a href="{{ route('admin.qrinto-sizes.create') }}" class="inline-flex items-center gap-2 px-5 py-2.5 bg-brand-600 text-white font-semibold rounded-xl hover:bg-brand-700 transition shadow-lg shadow-brand-200">
...
<div class="mb-6 p-4 bg-emerald-50 border border-emerald-200 text-emerald-800 rounded-xl flex items-center gap-3 text-sm">
    ...
    <span>These sizes and prices are displayed to customers on the <strong>Qrinto</strong> page when they upload their own designs.</span>
</div>
                ...
                <tr class="hover:bg-surface-50 transition">
                    ...
                    <td class="px-6 py-4 text-right">
                        <div class="flex items-center justify-end gap-2">
                            <a href="{{ route('admin.qrinto-sizes.edit', $size) }}" class="p-2 rounded-lg hover:bg-surface-100 text-surface-500 hover:text-brand-600 transition" title="Edit">
                                <i data-lucide="pencil" class="w-4 h-4"></i>
                            </a>
                            <form action="{{ route('admin.qrinto-sizes.destroy', $size) }}" method="POST" onsubmit="return confirm('Delete this size? This cannot be undone.')">
                                @csrf @method('DELETE')
                ...
                <tr>
                    <td colspan="8" class="px-6 py-12 text-center">
                        <div class="flex flex-col items-center gap-3">
                            ...
                            <a href="{{ route('admin.qrinto-sizes.create') }}" class="text-sm text-brand-600 hover:text-brand-700 font-semibold">Add your first size →</a>
                        </div>
                    </td>
                </tr>
                @endforelse
