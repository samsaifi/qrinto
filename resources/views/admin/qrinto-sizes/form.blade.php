...
@section('title', isset($size) ? 'Edit Size: ' . $size->label : 'Add Qrinto Size')

@section('content')
<div class="mb-8">
    <div class="flex items-center gap-3 mb-2">
        <a href="{{ route('admin.qrinto-sizes.index') }}" class="p-2 rounded-xl hover:bg-surface-100 text-surface-400 hover:text-surface-600 transition">
            <i data-lucide="arrow-left" class="w-5 h-5"></i>
        </a>
        <h1 class="font-display font-bold text-2xl text-surface-900">
            {{ isset($size) ? 'Edit Size: ' . $size->label : 'Add Qrinto Size' }}
        </h1>
    </div>
    <p class="text-sm text-surface-500 ml-12">{{ isset($size) ? 'Update the size details and pricing' : 'Add a new size option for qrintos' }}</p>
</div>

<div class="max-w-2xl">
    <form action="{{ isset($size) ? route('admin.qrinto-sizes.update', $size) : route('admin.qrinto-sizes.store') }}"
          method="POST"
        ...
        <div class="flex items-center justify-between pt-4 border-t border-surface-100">
            <a href="{{ route('admin.qrinto-sizes.index') }}" class="px-6 py-2.5 text-surface-500 hover:text-surface-700 font-medium transition">
                Cancel
            </a>
