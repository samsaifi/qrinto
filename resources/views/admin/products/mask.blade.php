...
'background_image' => ['label' => 'Background', 'icon' => 'layers', 'url' => $product->background_image_url],
...
<div class="flex gap-2 mb-4 flex-wrap">
    ...
    <button type="button"
        ...
        {{ $img['label'] }}
        @if(!$img['url'])
        <span class="text-[9px] bg-yellow-100 text-yellow-700 px-1.5 py-0.5 rounded-full font-bold">No Image</span>
        @endif
        <!-- Enabled toggle -->
        <label class="relative inline-flex items-center cursor-pointer ml-1" onclick="event.stopPropagation()">
            ...
            <div class="w-8 h-4 bg-surface-200 peer-checked:bg-brand-500 rounded-full peer-focus:ring-2 peer-focus:ring-brand-200 transition-all
                after:content-[''] after:absolute after:top-0.5 after:left-0.5 after:w-3 after:h-3 after:bg-white after:rounded-full after:transition-all peer-checked:after:translate-x-4"></div>
        </label>
    </button>
    @endforeach
</div>
...
<div style="display:flex; gap:12px;   min-height:450px; overflow-x:auto; padding-bottom:20px;">
    ...
    <div style="flex:none; position:relative;" id="canvasPanel"
        ...
        <!-- Toolbar -->
        <div style="display:flex; align-items:center; justify-content:space-between; padding:8px 16px; background:#f8fafc; border-bottom:1px solid #f1f5f9;">
            <div style="display:flex; align-items:center; gap:10px;">
                <span style="font-size:11px; font-weight:600; color:#64748b; text-transform:uppercase; letter-spacing:0.05em;" id="canvasLabel">Canvas — Frame Image</span>
                <span id="canvasInfo" style="font-size:11px; color:#94a3b8;"></span>
            </div>

            <!-- Polygon drawing hint -->
            <div id="polygonHelp" style="display:none; align-items:center; gap:8px; background:#eff6ff; padding:4px 12px; border-radius:6px; border:1px solid #bfdbfe;">
                <div class="w-2 h-2 rounded-full bg-blue-500 animate-pulse"></div>
                <span style="font-size:11px; color:#1e3a8a; font-weight:500;">Click to add points. Double-click or press Enter to finish.</span>
                <button type="button" onclick="finishPolygon()" style="background:#3b82f6; color:white; border:none; padding:2px 8px; font-size:10px; border-radius:4px; cursor:pointer; font-weight:bold;">Finish</button>
                <button type="button" onclick="cancelPolygon()" style="background:white; color:#ef4444; border:1px solid #fca5a5; padding:1px 7px; font-size:10px; border-radius:4px; cursor:pointer;">Cancel</button>
            </div>
        ...
        <div id="canvasDisabledOverlay"
            ...
            <p style="font-size:13px; font-weight:600; color:#64748b; margin-top:8px;">Canvas disabled — toggle to enable</p>
        </div>
    </div>
    ...
    <div style="width:220px; flex-shrink:0; position:sticky; top:0;"
        ...
        <div style="padding:10px; display:grid; grid-template-columns:1fr 1fr; gap:8px;">
            ...
            <button type="button" onclick="startPolygonDraw()" class="shape-btn group col-span-2 bg-blue-50/50">
                <svg class="w-8 h-8" viewBox="0 0 40 40">
                    <path d="M 10,30 L 5,10 L 25,5 L 35,20 L 25,35 Z" fill="rgba(59,130,246,0.4)" stroke="rgb(59,130,246)" stroke-width="2" stroke-linejoin="round" />
                </svg>
        ...
        <div style="padding:10px; border-top:1px solid #f1f5f9;">
            <h3 style="font-size:11px; font-weight:600; color:#64748b; text-transform:uppercase; letter-spacing:0.05em; margin-bottom:8px;">Add Text</h3>
            ...
            <div style="display:flex; gap:6px; margin-bottom:6px;">
                <select id="maskFontSize" style="flex:1; padding:5px 6px; font-size:11px; border:1px solid #e2e8f0; border-radius:6px; background:#fff; outline:none;">
            ...
            <select id="maskFontFamily" style="width:100%; padding:5px 6px; font-size:11px; border:1px solid #e2e8f0; border-radius:6px; background:#fff; outline:none; margin-bottom:8px;">
                <option>Arial</option>
        ...
        <div id="propsPanel" style="border-top:1px solid #f1f5f9; padding:10px; display:none;">
            <h3 style="font-size:11px; font-weight:600; color:#64748b; text-transform:uppercase; letter-spacing:0.05em; margin-bottom:8px;">Properties</h3>
            <!-- Shape props -->
            <div id="shapeProps" style="display:none;">
                <div style="margin-bottom:6px;">
                    ...
                    <input type="color" id="propFillColor" value="#0064ff" onchange="applyShapeProp('fill',this.value)"
                        style="width:100%; height:28px; border:1px solid #e2e8f0; border-radius:6px; cursor:pointer; padding:2px;">
                </div>
                <div style="margin-bottom:6px;">
                    <label style="font-size:10px; color:#94a3b8; display:block; margin-bottom:3px;">Border Color</label>
                    <input type="color" id="propStrokeColor" value="#0050dc" onchange="applyShapeProp('stroke',this.value)"
                        style="width:100%; height:28px; border:1px solid #e2e8f0; border-radius:6px; cursor:pointer; padding:2px;">
                </div>
                <div style="margin-bottom:6px;">
                    <label style="font-size:10px; color:#94a3b8; display:block; margin-bottom:3px;">Opacity</label>
                    <input type="range" id="propOpacity" min="0" max="100" value="45" oninput="applyOpacity(this.value)" style="width:100%; cursor:pointer;">
            ...
            <div id="textProps" style="display:none;">
                <div style="margin-bottom:6px;">
                    ...
                    <input type="color" id="propTextColor" value="#0064ff" onchange="applyTextProp('fill',this.value)"
                        style="width:100%; height:28px; border:1px solid #e2e8f0; border-radius:6px; cursor:pointer; padding:2px;">
                </div>
                <div style="margin-bottom:6px;">
                ...
                <div style="margin-bottom:6px;">
                    <label style="font-size:10px; color:#94a3b8; display:block; margin-bottom:3px;">Bold / Italic</label>
                    <div style="display:flex; gap:4px;">
                        <button type="button" id="propBold" onclick="toggleBold()" style="flex:1; padding:4px; font-size:12px; font-weight:700; border:1px solid #e2e8f0; border-radius:6px; background:#fff; cursor:pointer;">B</button>
                        <button type="button" id="propItalic" onclick="toggleItalic()" style="flex:1; padding:4px; font-size:12px; font-style:italic; border:1px solid #e2e8f0; border-radius:6px; background:#fff; cursor:pointer;">I</button>
                    </div>
                </div>
            </div>
            <!-- Position readout -->
            <div style="margin-top:8px; display:grid; grid-template-columns:1fr 1fr; gap:4px; font-size:11px;">
                <div style="background:#f8fafc; border-radius:6px; padding:4px 8px;"><span style="color:#94a3b8;">X:</span> <span id="posX" style="font-family:monospace; color:#334155;">—</span></div>
                <div style="background:#f8fafc; border-radius:6px; padding:4px 8px;"><span style="color:#94a3b8;">Y:</span> <span id="posY" style="font-family:monospace; color:#334155;">—</span></div>
                <div style="background:#f8fafc; border-radius:6px; padding:4px 8px;"><span style="color:#94a3b8;">W:</span> <span id="posW" style="font-family:monospace; color:#334155;">—</span></div>
                <div style="background:#f8fafc; border-radius:6px; padding:4px 8px;"><span style="color:#94a3b8;">H:</span> <span id="posH" style="font-family:monospace; color:#334155;">—</span></div>
            </div>
        </div>
    </div><!-- /sidebar -->
</div>
    ...
    (function() {
        ...
        Object.keys(imageData).forEach(key => {
            canvasStates[key] = {
                ...
                enabled: (allSavedMasks?.[key]?.enabled !== undefined) ? allSavedMasks[key].enabled : true,
        ...
        Object.keys(imageData).forEach(key => {
            ...
            probe.onload = function() {
                if (probe.width > 0) {
                    canvasStates[key].canvasHeight = Math.round(BASE_WIDTH * probe.height / probe.width);
                }
            };
            probe.src = url;
        });
        ...
        function loadActiveTab() {
            ...
            if (imgUrl) {
                fabric.Image.fromURL(imgUrl, img => {
                    ...
                    // Update UI info
                    document.getElementById('canvasDimBadge').textContent = BASE_WIDTH + ' × ' + newCanvasH + ' px';
                    document.getElementById('canvasPanel').style.width = (BASE_WIDTH + 2 * PADDING + 2) + 'px';
        ...
        function showPropsPanel(obj) {
            ...
            if (obj.maskType === 'text') {
                ...
                document.getElementById('propBold').style.background = obj.fontWeight === 'bold' ? '#dbeafe' : '#fff';
                document.getElementById('propItalic').style.background = obj.fontStyle === 'italic' ? '#dbeafe' : '#fff';
        ...
        window.finishPolygon = function() {
            ...
            canvas.getObjects().forEach(o => {
                if (o.id === 'poly_point' || o.id === 'poly_line' || o.id === 'poly_shape') canvas.remove(o);
            });
            maskCounter++;
            const id = 'mask_' + maskCounter;
        ...
        function updateMaskList() {
            ...
            masks.forEach((m, i) => {
                ...
                div.className = 'mask-item flex items-center justify-between px-3 py-2 bg-surface-50 rounded-lg cursor-pointer hover:bg-blue-50 transition group';
